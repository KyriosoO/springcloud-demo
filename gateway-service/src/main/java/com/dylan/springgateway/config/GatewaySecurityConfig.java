...
public class GatewaySecurityConfig {
	...
	public GlobalFilter authTokenFilter(JwtDecoder jwtDecoder) {
		return (exchange, chain) -> {
			...
			// 2️ 获取 Token：优先从 Cookie 读取，其次从 Authorization Header
			HttpCookie cookie = exchange.getRequest().getCookies().getFirst("AUTH_TOKEN");
			String token = null;
			if (cookie != null) {
				token = cookie.getValue();
			} else {
				// 回退：检查 Authorization: Bearer <token> header
			...
			ServerHttpRequest mutatedRequest = exchange.getRequest().mutate()
					.header(HttpHeaders.AUTHORIZATION, "Bearer " + token).header("X-USER-ID", userId)
					.build();
