...
$expectedRunId = 'knowledge-egress-v2-20260812-candidate-01'
$expectedAuthorizationReference = 'P3_00:GATE-043'
$candidateManifest = Join-Path $repository 'agent-runtime\tests\integration\knowledge\evidence\knowledge-egress-v2-20260812-candidate-01.manifest.json'
if (
    ...
    throw 'knowledge.egress_v2_authorization_binding_invalid'
}
...
if ($actualManifestSha256 -ne $AuthorizedManifestSha256) {
    throw 'knowledge.egress_v2_manifest_hash_mismatch'
}

$runtimeRoot = Join-Path $repository 'agent-runtime'
$evidenceRoot = Join-Path $runtimeRoot 'tests\integration\knowledge\evidence'
$consumedPath = Join-Path $evidenceRoot 'gate043-knowledge-egress-v2-20260812-candidate-01.consumed.json'
...
try {
    ...
    python -m pytest `
        ...
        tests/integration/knowledge/test_egress_v2_stability_candidate_manifest.py `
        tests/integration/knowledge/test_knowledge_egress_v2_stability_harness.py `
        tests/integration/knowledge/test_egress_attempt_journal.py `
        tests/integration/knowledge/test_evidence_stage.py `
        tests/contract/knowledge/test_summary_task_v2.py `
        -q --tb=short
    if ($LASTEXITCODE -ne 0) {
...
$environmentNames = @(
    ...
    'RUN_KNOWLEDGE_EGRESS_V2_STABILITY',
...
$runRoot = Join-Path $tempBase "codex-k-egress-v2-$([Guid]::NewGuid().ToString('N'))"
...
try {
    ...
    $env:RUN_KNOWLEDGE_EGRESS_V2_STABILITY = '1'
    ...
    Push-Location $runtimeRoot
    try {
        python -m pytest tests/integration/knowledge/test_real_knowledge_egress_v2_stability.py -q -s --tb=short
