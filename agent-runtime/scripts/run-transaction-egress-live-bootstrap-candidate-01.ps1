...
param(
    ...
    [string]$RunId = 'transaction-egress-live-bootstrap-v1-20260817-candidate-01',
    [string]$AuthorizationReference = 'P3_00:GATE-026',
    [string]$RepositoryRoot = (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))
)

$ErrorActionPreference = 'Stop'
$expectedRunId = 'transaction-egress-live-bootstrap-v1-20260817-candidate-01'
...
if ($repository -cne 'D:\codex' -or $RunId -cne $expectedRunId -or
        ...
        $env:TRANSACTION_EGRESS_BOOTSTRAP_LIVE_AUTHORIZED -cne '1') {
    throw 'transaction.egress_live_bootstrap_authorization_binding_invalid'
}
...
try {
    ...
    & $python -m tests.integration.adapters.transaction.live_bootstrap_v1 `
        --repository-root $repository --manifest-sha256 $ManifestSha256
    if ($LASTEXITCODE -ne 0) {
        throw 'transaction.egress_live_bootstrap_failed'
    }
}
finally {
