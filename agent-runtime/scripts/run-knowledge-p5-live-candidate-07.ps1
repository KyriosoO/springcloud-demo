...
$manifestPath = Join-Path $runtimeRoot 'tests\evaluation\knowledge\live\evidence\knowledge-p5-live-v4-20260828-candidate-07.manifest.json'
$authorizationRelativePath = 'agent-runtime/tests/evaluation/knowledge/live/evidence/knowledge-p5-live-v4-20260828-candidate-07.authorization.json'
...
if (
    ...
    [string]$authorization.workPackageId -ne 'WP-K-EFFECT-LIVE-07' -or
...
try {
    ...
    $env:PYTHONPATH = 'src'
    python -m pytest `
        tests/evaluation/knowledge/test_live_p5_candidate_07_contracts.py `
...
try {
    ...
    $env:P5_KNOWLEDGE_CANDIDATE = 'candidate-07'
    ...
    $launcherEvidence = [ordered]@{
        schemaVersion = 1
        status = 'passed'
        workPackageId = [string]$manifest.workPackageId
