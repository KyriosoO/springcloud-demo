...
param(
    ...
    [string]$RunId = 'transaction-egress-v2-20260817-candidate-02',
    [string]$AuthorizationReference = 'P3_00:GATE-026',
    [string]$RepositoryRoot = (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))
)

$ErrorActionPreference = 'Stop'
$expectedRunId = 'transaction-egress-v2-20260817-candidate-02'
...
if ($actualManifestSha256 -cne $ManifestSha256 -or $manifest.schemaVersion -ne 2 -or
...
$names = @('RUN_TRANSACTION_EGRESS_CANDIDATE_02','TRANSACTION_EGRESS_MANIFEST_SHA256','TRANSACTION_EGRESS_LIFECYCLE_OUTPUT','TRANSACTION_EGRESS_RESULT_OUTPUT')
$snapshot = @{}
foreach ($name in $names) { $snapshot[$name] = [Environment]::GetEnvironmentVariable($name, 'Process') }
try {
    $env:RUN_TRANSACTION_EGRESS_CANDIDATE_02 = '1'
    ...
    $process = Start-Process -FilePath $python -ArgumentList @(
        '-m','pytest','tests/integration/adapters/transaction/test_real_transaction_egress_candidate_v2.py','-q','--tb=no'
