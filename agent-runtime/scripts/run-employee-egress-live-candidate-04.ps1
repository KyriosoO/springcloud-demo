...
$runId = 'employee-egress-v4-20260817-candidate-04'
...
$taskDirectory = Join-Path $repository 'agent-runtime\.codex-live\employee-egress-v4'
$mavenLog = Join-Path $taskDirectory 'maven.log'
$pythonLog = Join-Path $taskDirectory 'python.log'
$python = (Get-Command python.exe -ErrorAction Stop).Source
$manifestSha = [Environment]::GetEnvironmentVariable('EMPLOYEE_EGRESS_V4_EXPECTED_MANIFEST_SHA256', 'Process')

if ($env:EMPLOYEE_EGRESS_V4_LIVE_AUTHORIZED -ne '1' -or
        $manifestSha -notmatch '^[0-9a-f]{64}$') {
    throw 'employee.egress_candidate_v4_live_not_authorized'
}
foreach ($name in @(
    'COMMON_SECURITY_JWT_HMAC_KEY_ACTIVE',
    'EMPLOYEE_EGRESS_V4_ADMIN_JWT',
    'LLM_API_KEY'
)) {
    if ([string]::IsNullOrWhiteSpace([Environment]::GetEnvironmentVariable($name, 'Process'))) {
        throw "employee.egress_candidate_v4_env_missing:$name"
    }
}
foreach ($path in @($manifestPath, $authorizationPath)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw 'employee.egress_candidate_v4_binding_missing'
    }
}
foreach ($path in @($lifecyclePath, $consumedPath, $pendingPath, $stagingPath, $resultPath)) {
    if (Test-Path -LiteralPath $path) {
        throw 'employee.egress_candidate_v4_output_exists'
    }
}
if ((Get-FileHash -LiteralPath $manifestPath -Algorithm SHA256).Hash.ToLowerInvariant() -cne $manifestSha) {
    throw 'employee.egress_candidate_v4_manifest_mismatch'
}

$env:PYTHONPATH = (Join-Path $repository 'agent-runtime\src') + ';' + (Join-Path $repository 'agent-runtime')
$env:EMPLOYEE_EGRESS_V4_PREFLIGHT_SHA = $manifestSha
& $python -c @'
...
manifest_sha = os.environ["EMPLOYEE_EGRESS_V4_PREFLIGHT_SHA"]
...
'@
Remove-Item Env:\EMPLOYEE_EGRESS_V4_PREFLIGHT_SHA -ErrorAction SilentlyContinue
if ($LASTEXITCODE -ne 0) {
    Remove-Item Env:\PYTHONPATH -ErrorAction SilentlyContinue
    throw 'employee.egress_candidate_v4_preflight_failed'
}
...
$logLeakCount = 0
try {
    $env:RUN_EMPLOYEE_EGRESS_CANDIDATE_V4 = '1'
    $env:EMPLOYEE_EGRESS_V4_MANIFEST_SHA256 = $manifestSha
    $env:EMPLOYEE_EGRESS_V4_LIFECYCLE = $lifecyclePath
    $env:EMPLOYEE_EGRESS_V4_CONSUMED = $consumedPath
    $env:EMPLOYEE_EGRESS_V4_PENDING = $pendingPath
    $env:EMPLOYEE_EGRESS_V4_STAGING = $stagingPath
    $env:EMPLOYEE_EGRESS_V4_PYTHON = $python
    $env:EMPLOYEE_EGRESS_V4_PYTHON_LOG = $pythonLog
    $env:EMPLOYEE_EGRESS_V4_REPOSITORY = $repository

    & mvn -f (Join-Path $repository 'serviceCenter\pom.xml') `
        -pl ':employee-service' -am `
        '-Dtest=EmployeeEgressCandidateV4LiveIntegrationTest' `
    ...
    $sensitive = @(
        ...
        [Environment]::GetEnvironmentVariable('EMPLOYEE_EGRESS_V4_ADMIN_JWT', 'Process'),
    ...
    foreach ($name in @(
        'RUN_EMPLOYEE_EGRESS_CANDIDATE_V4', 'EMPLOYEE_EGRESS_V4_MANIFEST_SHA256',
        'EMPLOYEE_EGRESS_V4_LIFECYCLE', 'EMPLOYEE_EGRESS_V4_CONSUMED',
        'EMPLOYEE_EGRESS_V4_PENDING', 'EMPLOYEE_EGRESS_V4_STAGING',
        'EMPLOYEE_EGRESS_V4_PYTHON', 'EMPLOYEE_EGRESS_V4_PYTHON_LOG',
        'EMPLOYEE_EGRESS_V4_REPOSITORY'
    )) {
        Remove-Item "Env:\$name" -ErrorAction SilentlyContinue
    }
}

$env:EMPLOYEE_EGRESS_V4_FINAL_SHA = $manifestSha
$env:EMPLOYEE_EGRESS_V4_FINAL_EXIT = [string]$hostExitCode
$env:EMPLOYEE_EGRESS_V4_FINAL_LEAKS = [string]$logLeakCount
& $python -c @'
from pathlib import Path
import os
from tests.integration.adapters.employee.egress_candidate_v4 import (
...
manifest_sha = os.environ["EMPLOYEE_EGRESS_V4_FINAL_SHA"]
...
host_exit = int(os.environ["EMPLOYEE_EGRESS_V4_FINAL_EXIT"])
log_leaks = int(os.environ["EMPLOYEE_EGRESS_V4_FINAL_LEAKS"])
...
Remove-Item Env:\PYTHONPATH -ErrorAction SilentlyContinue
Remove-Item Env:\EMPLOYEE_EGRESS_V4_FINAL_SHA -ErrorAction SilentlyContinue
Remove-Item Env:\EMPLOYEE_EGRESS_V4_FINAL_EXIT -ErrorAction SilentlyContinue
Remove-Item Env:\EMPLOYEE_EGRESS_V4_FINAL_LEAKS -ErrorAction SilentlyContinue
if ($finalizerExitCode -ne 0 -or -not (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
    throw 'employee.egress_candidate_v4_finalize_failed'
}
...
if ($result.status -cne 'passed' -or $result.lifecycle.recordCount -ne 76 -or
    ...
    throw 'employee.egress_candidate_v4_not_passed'
}
[PSCustomObject]@{
