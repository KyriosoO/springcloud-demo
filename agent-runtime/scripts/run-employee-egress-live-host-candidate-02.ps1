...
param(
    ...
    [string]$RunId = 'employee-egress-live-bootstrap-v2-20260818-candidate-02',
    [string]$AuthorizationReference = 'P3_00:GATE-024',
    [string]$RepositoryRoot = (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))
)

$ErrorActionPreference = 'Stop'
$expectedRunId = 'employee-egress-live-bootstrap-v2-20260818-candidate-02'
...
if ($repository -cne 'D:\codex' -or $RunId -cne $expectedRunId -or
        ...
        $env:EMPLOYEE_EGRESS_BOOTSTRAP_V2_LIVE_AUTHORIZED -cne '1') {
    throw 'employee.egress_live_bootstrap_v2_authorization_binding_invalid'
}
...
try {
    ...
    & $python -m tests.integration.adapters.employee.live_bootstrap_v2 `
        --repository-root $repository --manifest-sha256 $ManifestSha256
    if ($LASTEXITCODE -ne 0) {
        throw 'employee.egress_live_bootstrap_v2_failed'
    }
}
finally {
