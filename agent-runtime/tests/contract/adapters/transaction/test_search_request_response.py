...
def _request(size: int = 20) -> TransactionSearchWireRequest:
    return TransactionSearchWireRequest(
        condition=TransactionSearchCondition(
            ...
            amount=None, amount_gt=Decimal("100.0100"), amount_lt=None,
        ),
...
def test_transaction_request_maps_snake_to_camel_and_exact_decimal_to_number() -> None:
    ...
    assert request.json_body.content == b'{"condition":{"amountGt":100.01,"transType":"PAY"},"page":1,"size":20,"sorts":[{"direction":"DESC","field":"amount"}]}'
    assert b'"100.01"' not in request.json_body.content


def test_transaction_response_reads_decimal_without_float_and_ignores_only_known_wide_date_fields() -> None:
    body = b'{"rows":[{"transId":"T0001","transType":"PAY","amount":100.0100,"transDate":"2026-01-01"}],"total":1,"totalExact":true,"page":1,"size":20}'
    ...
    assert response.rows[0].amount == Decimal("100.0100")
