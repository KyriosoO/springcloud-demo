...
from tests.evaluation.knowledge.live_runner import _repository_state_excluding_output
    ...
    calls = iter(
        (
            ...
            SimpleNamespace(
                stdout=(
                    authorization_entry
                    + "\n?? agent-runtime/tests/evaluation/knowledge/results/run-01/result.json\n"
                )
            ),
        )
    )
    monkeypatch.setattr("tests.evaluation.knowledge.live_runner.subprocess.run", lambda *args, **kwargs: next(calls))

    commit, entries = _repository_state_excluding_output(
        repository_root=REPOSITORY_ROOT,
        output_dir=REPOSITORY_ROOT / "agent-runtime/tests/evaluation/knowledge/results/run-01",
        allowed_entries=(authorization_entry,),
    )
...
def test_final_snapshot_does_not_allow_modified_or_unrelated_entries(
    ...
    calls = iter(
        (
            SimpleNamespace(stdout="a" * 40 + "\n"),
            SimpleNamespace(stdout=f" M {authorization_path}\n?? unrelated.txt\n"),
        )
    )
    monkeypatch.setattr("tests.evaluation.knowledge.live_runner.subprocess.run", lambda *args, **kwargs: next(calls))

    _, entries = _repository_state_excluding_output(
        repository_root=REPOSITORY_ROOT,
        output_dir=REPOSITORY_ROOT / "agent-runtime/tests/evaluation/knowledge/results/run-01",
        allowed_entries=(f"?? {authorization_path}",),
    )

    assert entries == (f" M {authorization_path}", "?? unrelated.txt")
