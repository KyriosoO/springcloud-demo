...
def test_candidate_06_prepared_state_has_no_authorization_or_result() -> None:
    assert not AUTHORIZATION_PATH.exists()
    assert not RESULT_ROOT.exists()


@pytest.mark.asyncio
async def test_candidate_06_requires_separate_authorization_before_key_or_outbound(tmp_path: Path) -> None:
    with pytest.raises(LiveEvaluationBootstrapError, match="evaluation.live_authorization_missing"):
        await build_live_from_environment(
            environ={
                ...
                "P5_KNOWLEDGE_CANDIDATE": "candidate-06",
            },
...
def test_candidate_06_prepared_assets_contain_no_secret_or_live_result() -> None:
    ...
    assert "authorization.consumed.json" not in {
        path.name for path in RESULT_ROOT.glob("*")
    }
