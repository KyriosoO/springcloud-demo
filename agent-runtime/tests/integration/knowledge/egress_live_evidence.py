...
def validate_live_evidence(value: object) -> dict[str, Any]:
    ...
    if (
        ...
        or evidence["runId"] != "knowledge-egress-v1-20260812-candidate-02"
        or evidence["authorizationReference"] != "P3_00:GATE-039"
    ...
    if budget != {"authorizedSummaryCalls": 3, "actualSummaryCalls": 3, "retryCount": 0}:
        raise KnowledgeEgressEvidenceError("knowledge.egress_evidence_invalid")
    ...
    for expected_case_id, item in zip(CASE_IDS, cases, strict=True):
        ...
        if (
            ...
            or not 1 <= case["pointCount"] <= 5
            or case["quoteValidation"] != "passed"
        ):
            raise KnowledgeEgressEvidenceError("knowledge.egress_evidence_invalid")
