...
def _require_prepared_assets() -> None:
    if not MANIFEST.is_file() or not AUTHORIZATION.is_file():
        pytest.skip("employee bootstrap manifest is generated after wrapper source commit")


def test_employee_bootstrap_manifest_authorization_assets_and_history_are_frozen() -> None:
...
def test_employee_bootstrap_assets_exist_at_frozen_source_commit() -> None:
    _require_prepared_assets()
    manifest = load_strict_json(MANIFEST)
...
def test_employee_bootstrap_is_prepared_and_has_no_outer_or_inner_outputs() -> None:
    _require_prepared_assets()
