...
JAVA = REPOSITORY / "employee-service/src/test/java/com/dylan/employee/live/EmployeeEgressInputQualificationV5LiveIntegrationTest.java"
LAUNCHER = REPOSITORY / "agent-runtime/scripts/run-employee-egress-input-qualification-candidate-05.ps1"
...
def test_result_rejects_success_count_and_reason_presence_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualificationV5Error, match="invalid"):
...
def test_outputs_are_exclusive(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualificationV5Error, match="output_exists"):
        _execute(tmp_path, repository)
...
def test_host_finalizer_preserves_cleanup_and_fails_closed_on_log_leak(tmp_path: Path) -> None:
    ...
    assert len(validate_lifecycle(lifecycle, manifest_sha256="a" * 64)) == 16


def test_live_finalizer_maps_host_failure_and_validates_before_result(tmp_path: Path) -> None:
    execution = _execute(tmp_path, InMemoryEmployeeFixtureRepository())
    lifecycle = tmp_path / "lifecycle.jsonl"
    lines = lifecycle.read_text(encoding="utf-8").splitlines()
    lifecycle.write_text("\n".join(lines[:-3]) + "\n", encoding="utf-8")
    (tmp_path / "result.json").unlink()
    pending = tmp_path / "live-pending.json"
    write_exclusive_json(
        pending,
        {
            "schemaVersion": 5,
    ...
    result_path = tmp_path / "live-result.json"
    result = finalize_live_candidate(
        lifecycle_path=lifecycle,
        pending_path=pending,
    ...
    assert (result["status"], result["reason"]) == ("failed", "host_execution_failed")
    assert len(validate_lifecycle(lifecycle, manifest_sha256="a" * 64)) == 16
    assert result_path.is_file()


def test_live_finalizer_rejects_invalid_pending_or_lifecycle_without_result(
    tmp_path: Path,
) -> None:
    execution = _execute(tmp_path / "invalid-pending-source", InMemoryEmployeeFixtureRepository())
    lifecycle = tmp_path / "invalid-pending-source" / "lifecycle.jsonl"
    lines = lifecycle.read_text(encoding="utf-8").splitlines()
    lifecycle.write_text("\n".join(lines[:-3]) + "\n", encoding="utf-8")
    result_path = tmp_path / "invalid-pending-result.json"
    ...
    malformed_source = tmp_path / "invalid-lifecycle-source"
    malformed = _execute(malformed_source, InMemoryEmployeeFixtureRepository())
    bad_lifecycle = malformed_source / "lifecycle.jsonl"
    bad_lines = bad_lifecycle.read_text(encoding="utf-8").splitlines()
    bad_lifecycle.write_text("\n".join(bad_lines[:-4]) + "\n", encoding="utf-8")
    (malformed_source / "result.json").unlink()
    pending = tmp_path / "valid-pending.json"
    write_exclusive_json(
        pending,
        {
            "schemaVersion": 5,
            ...
            "fieldPresence": malformed.result["fieldPresence"],
            "counts": malformed.result["counts"],
        },
    )
    malformed_result = tmp_path / "invalid-lifecycle-result.json"
    with pytest.raises(QualificationV5Error, match="invalid"):
        finalize_live_candidate(
            lifecycle_path=bad_lifecycle,
            pending_path=pending,
    ...
    assert not malformed_result.exists()


def test_history_schemas_and_static_live_seams_are_strict() -> None:
    verify_history(REPOSITORY)
    lifecycle_schema = json.loads(
        (EVIDENCE / "employee-egress-input-qualification-v5-lifecycle.schema.json").read_text()
    )
    result_schema = json.loads(
        (EVIDENCE / "employee-egress-input-qualification-v5-result.schema.json").read_text()
    )
    ...
    for token in (
        ...
        "value.size() != 5",
        "EmployeeEgressInputQualificationV5LiveIntegrationTest",
    ):
        assert token in source
    launcher = LAUNCHER.read_text(encoding="utf-8")
    assert "RUN_EMPLOYEE_EGRESS_INPUT_QUALIFY_V5" in launcher
    ...
    assert result_schema["properties"]["history"]["minItems"] == 17
    assert result_schema["properties"]["history"]["maxItems"] == 17
