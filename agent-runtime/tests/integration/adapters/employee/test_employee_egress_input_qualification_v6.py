...
JAVA = REPOSITORY / "employee-service/src/test/java/com/dylan/employee/live/EmployeeEgressInputQualificationV6LiveIntegrationTest.java"
LAUNCHER = REPOSITORY / "agent-runtime/scripts/run-employee-egress-input-qualification-candidate-06.ps1"
...
def test_result_rejects_success_count_and_reason_presence_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualificationV6Error, match="invalid"):
...
def test_outputs_are_exclusive(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualificationV6Error, match="output_exists"):
        _execute(tmp_path, repository)
...
def test_history_schemas_and_static_live_seams_are_strict() -> None:
    ...
    result_schema = json.loads(
        (EVIDENCE / "employee-egress-input-qualification-v6-result.schema.json").read_text()
    )
    ...
    for token in (
        ...
        "value.size() != 5",
        'value.path("codec").size() != 4',
        "EmployeeEgressInputQualificationV6LiveIntegrationTest",
    ):
        assert token in source
    launcher = LAUNCHER.read_text(encoding="utf-8")
    assert "RUN_EMPLOYEE_EGRESS_INPUT_QUALIFY_V6" in launcher
    ...
    assert result_schema["properties"]["history"]["minItems"] == 23
    assert result_schema["properties"]["history"]["maxItems"] == 23
