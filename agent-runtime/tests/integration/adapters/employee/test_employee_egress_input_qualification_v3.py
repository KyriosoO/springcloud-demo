...
JAVA = REPOSITORY / "employee-service/src/test/java/com/dylan/employee/live/EmployeeEgressInputQualificationV3LiveIntegrationTest.java"
LAUNCHER = REPOSITORY / "agent-runtime/scripts/run-employee-egress-input-qualification-candidate-03.ps1"
...
def test_result_rejects_success_count_and_reason_presence_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualificationV3Error, match="invalid"):
...
def test_outputs_are_exclusive(tmp_path: Path) -> None:
    ...
    with pytest.raises(QualificationV3Error, match="output_exists"):
        _execute(tmp_path, repository)
...
def test_history_schemas_and_static_live_seams_are_strict() -> None:
    ...
    lifecycle_schema = json.loads(
        (EVIDENCE / "employee-egress-input-qualification-v3-lifecycle.schema.json").read_text()
    )
    result_schema = json.loads(
        (EVIDENCE / "employee-egress-input-qualification-v3-result.schema.json").read_text()
    )
    ...
    for token in (
        ...
        "value.size() != 5",
        "EmployeeEgressInputQualificationV3LiveIntegrationTest",
    ):
        assert token in source
    launcher = LAUNCHER.read_text(encoding="utf-8")
    assert "RUN_EMPLOYEE_EGRESS_INPUT_QUALIFY_V3" in launcher
