...
PREPARATION_WORK_PACKAGE_ID: Final = "WP-EMP-EGRESS-INPUT-QUALIFY-04-PREP"
...
SEED: Final = "employee-qualification-candidate-04"
...
ASSET_PATHS: Final = frozenset(
    {
        "agent-runtime/tests/integration/adapters/employee/egress_input_qualification_v4.py",
        "agent-runtime/tests/integration/adapters/employee/test_employee_egress_input_qualification_v4.py",
        "agent-runtime/tests/integration/adapters/employee/test_employee_egress_input_qualification_v4_host.py",
        "agent-runtime/tests/integration/adapters/employee/test_real_employee_egress_input_qualification_v4.py",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v4-lifecycle.schema.json",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v4-result.schema.json",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v4-host-lifecycle.schema.json",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v4-pre-sql-failure.schema.json",
        "agent-runtime/tests/integration/adapters/employee/egress_input_qualification_v4_host.py",
        "agent-runtime/scripts/run-employee-egress-input-qualification-candidate-04.ps1",
        "employee-service/src/test/java/com/dylan/employee/live/EmployeeEgressInputQualificationV4LiveIntegrationTest.java",
    }
)
...
class QualificationV4Error(ValueError):
    pass


def _invalid() -> NoReturn:
    raise QualificationV4Error("employee.qualification_v4_invalid")
...
def verify_history(repository_root: Path) -> None:
    for _, relative, expected in HISTORY:
        ...
        if not path.is_file() or sha256_file(path) != expected:
            raise QualificationV4Error("employee.qualification_v4_history_mismatch")
...
def execute_fake_candidate(
    *,
    ...
    if lifecycle_path.exists() or result_path.exists():
        raise QualificationV4Error("employee.qualification_v4_output_exists")
    ...
    except QualificationV4Error as failure:
    ...
    _pair(journal, "host_validation", lambda: None)
    journal.record(
        phase="run",
    ...
    return CandidateExecution(status=status, reason=reason, result=result)
...
def _pair(journal: LifecycleJournal, phase: str, operation: Callable[[], None]) -> None:
    ...
    try:
        ...
        raise QualificationV4Error(reason) from failure
...
def _precheck(
    ...
    if counts["preexisting"] != 0:
        raise QualificationV4Error("identifier_conflict")
...
def _insert(
    ...
    if counts["inserted"] != 1:
        raise QualificationV4Error("insert_count_invalid")
...
def _verify(
    ...
    if counts["verified"] != 1:
        raise QualificationV4Error("fingerprint_mismatch")
...
def _delete(
    ...
    if counts["deleted"] != 1:
        raise QualificationV4Error("cleanup_count_invalid")
...
def _verify_deleted(
    ...
    if counts["remaining"] != 0:
        raise QualificationV4Error("cleanup_verification_failed")
...
def _count(value: object, *, reason: str) -> int:
    if type(value) is not int or value not in (0, 1):
        raise QualificationV4Error(reason)
    return value
...
def finalize_live_candidate(
    *,
    ...
    if set(pending) != {"schemaVersion", "status", "reason", "fieldPresence", "counts"} or pending["schemaVersion"] != 4:
        _invalid()
