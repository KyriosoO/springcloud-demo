...
pytestmark = pytest.mark.skipif(
    os.environ.get("RUN_EMPLOYEE_EGRESS_INPUT_QUALIFY_V3") != "1",
    reason="requires a separately authorized GATE-049 candidate-03 execution",
)
...
def _required(name: str) -> str:
    ...
    if value is None or not value.strip():
        raise RuntimeError(f"employee.qualification_v3_env_missing:{name}")
    return value.strip()


def _base_url() -> str:
    value = _required("EMPLOYEE_EGRESS_INPUT_QUALIFY_V3_BASE_URL")
    ...
    if (
        ...
        raise RuntimeError("employee.qualification_v3_endpoint_invalid")
    return value.rstrip("/")


class QualificationTransportV3:
    ...
    async def send(self, request: FakeDomainHttpRequest) -> FakeDomainHttpResponse:
        if self.calls != 0 or request.request.method != "GET":
            raise RuntimeError("employee.qualification_v3_request_count_invalid")
...
def _context(token: str) -> CapabilityExecutionContext:
    return CapabilityExecutionContext(
        request_id="employee-qualification-v3",
        correlation_id="employee-qualification-v3",
        original_question="查询该员工的职位和工作地",
        subject_id="employee-qualification-v3-reader",
...
async def test_single_synthetic_employee_detail_writes_only_finite_staging(
    ...
    if "LLM_API_KEY" in os.environ:
        raise RuntimeError("employee.qualification_v3_model_key_visible")
    identifier = _required("EMPLOYEE_EGRESS_INPUT_QUALIFY_V3_IDENTIFIER")
    token = _required("EMPLOYEE_EGRESS_INPUT_QUALIFY_V3_ADMIN_JWT")
    manifest_sha = _required("EMPLOYEE_EGRESS_INPUT_QUALIFY_V3_MANIFEST_SHA256")
    lifecycle_path = Path(_required("EMPLOYEE_EGRESS_INPUT_QUALIFY_V3_LIFECYCLE"))
    staging_path = Path(_required("EMPLOYEE_EGRESS_INPUT_QUALIFY_V3_STAGING"))
    ...
    transport = QualificationTransportV3(client, journal)
    handler = BoundBusinessActionHandler(
        ...
        config_snapshot_id=hashlib.sha256(b"employee-qualification-v3").hexdigest(),
    ...
    try:
        ...
        if identifier in captured.out + captured.err or token in captured.out + captured.err:
            raise RuntimeError("employee.qualification_v3_log_leak")
        write_exclusive_json(
            staging_path,
            {
                "schemaVersion": 3,
