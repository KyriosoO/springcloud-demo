...
PREPARATION_WORK_PACKAGE_ID: Final = "WP-EMP-EGRESS-INPUT-QUALIFY-05-PREP"
WORK_PACKAGE_ID: Final = "WP-EMP-EGRESS-INPUT-QUALIFY-05"
GATE_ID: Final = "GATE-049"
RUN_ID: Final = "employee-egress-input-qualification-v5-20260816-candidate-05"
AUTHORIZATION_REFERENCE: Final = "P3_00:GATE-049"
SEED: Final = "employee-qualification-candidate-05"
PREPARED_AT: Final = "2026-08-16T06:00:00Z"
...
ASSET_PATHS: Final = frozenset(
    {
        "agent-runtime/tests/integration/adapters/employee/egress_input_qualification_v5.py",
        "agent-runtime/tests/integration/adapters/employee/test_employee_egress_input_qualification_v5.py",
        "agent-runtime/tests/integration/adapters/employee/test_employee_egress_input_qualification_v5_host.py",
        "agent-runtime/tests/integration/adapters/employee/test_employee_egress_input_qualification_v5_history.py",
        "agent-runtime/tests/integration/adapters/employee/test_real_employee_egress_input_qualification_v5.py",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v5-lifecycle.schema.json",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v5-result.schema.json",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v5-host-lifecycle.schema.json",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v5-pre-sql-failure.schema.json",
        "agent-runtime/tests/integration/adapters/employee/egress_input_qualification_v5_host.py",
        "agent-runtime/scripts/run-employee-egress-input-qualification-candidate-05.ps1",
        "employee-service/src/test/java/com/dylan/employee/live/EmployeeEgressInputQualificationV5LiveIntegrationTest.java",
    }
)
...
class QualificationV5Error(ValueError):
    pass


def _invalid() -> NoReturn:
    raise QualificationV5Error("employee.qualification_v5_invalid")
...
def verify_history(repository_root: Path) -> None:
    for _, relative, expected in HISTORY:
        ...
        if not path.is_file() or sha256_file(path) != expected:
            raise QualificationV5Error("employee.qualification_v5_history_mismatch")
...
def execute_fake_candidate(
    *,
    ...
    if lifecycle_path.exists() or result_path.exists():
        raise QualificationV5Error("employee.qualification_v5_output_exists")
    ...
    except QualificationV5Error as failure:
...
def _pair(journal: LifecycleJournal, phase: str, operation: Callable[[], None]) -> None:
    ...
    try:
        ...
        raise QualificationV5Error(reason) from failure
...
def _precheck(
    ...
    if counts["preexisting"] != 0:
        raise QualificationV5Error("identifier_conflict")
...
def _insert(
    ...
    if counts["inserted"] != 1:
        raise QualificationV5Error("insert_count_invalid")
...
def _verify(
    ...
    if counts["verified"] != 1:
        raise QualificationV5Error("fingerprint_mismatch")
...
def _delete(
    ...
    if counts["deleted"] != 1:
        raise QualificationV5Error("cleanup_count_invalid")
...
def _verify_deleted(
    ...
    if counts["remaining"] != 0:
        raise QualificationV5Error("cleanup_verification_failed")
...
def _count(value: object, *, reason: str) -> int:
    if type(value) is not int or value not in (0, 1):
        raise QualificationV5Error(reason)
    return value
...
def finalize_live_candidate(
    *,
    ...
    if set(pending) != {"schemaVersion", "status", "reason", "fieldPresence", "counts"} or pending["schemaVersion"] != 5:
        _invalid()
