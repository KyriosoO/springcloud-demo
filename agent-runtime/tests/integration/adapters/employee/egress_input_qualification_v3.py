...
PREPARATION_WORK_PACKAGE_ID: Final = "WP-EMP-EGRESS-INPUT-QUALIFY-03-PREP"
WORK_PACKAGE_ID: Final = "WP-EMP-EGRESS-INPUT-QUALIFY-03"
GATE_ID: Final = "GATE-049"
RUN_ID: Final = "employee-egress-input-qualification-v3-20260814-candidate-03"
AUTHORIZATION_REFERENCE: Final = "P3_00:GATE-049"
SEED: Final = "employee-qualification-candidate-03"
PREPARED_AT: Final = "2026-08-14T12:00:00Z"
...
ASSET_PATHS: Final = frozenset(
    {
        "agent-runtime/tests/integration/adapters/employee/egress_input_qualification_v3.py",
        "agent-runtime/tests/integration/adapters/employee/test_employee_egress_input_qualification_v3.py",
        "agent-runtime/tests/integration/adapters/employee/test_real_employee_egress_input_qualification_v3.py",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v3-lifecycle.schema.json",
        "agent-runtime/tests/integration/adapters/employee/evidence/employee-egress-input-qualification-v3-result.schema.json",
        "agent-runtime/scripts/run-employee-egress-input-qualification-candidate-03.ps1",
        "employee-service/src/test/java/com/dylan/employee/live/EmployeeEgressInputQualificationV3LiveIntegrationTest.java",
    }
)
...
class QualificationV3Error(ValueError):
    pass


def _invalid() -> NoReturn:
    raise QualificationV3Error("employee.qualification_v3_invalid")
...
def verify_history(repository_root: Path) -> None:
    for _, relative, expected in HISTORY:
        ...
        if not path.is_file() or sha256_file(path) != expected:
            raise QualificationV3Error("employee.qualification_v3_history_mismatch")
...
def execute_fake_candidate(
    *,
    ...
    if lifecycle_path.exists() or result_path.exists():
        raise QualificationV3Error("employee.qualification_v3_output_exists")
    ...
    except QualificationV3Error as failure:
...
def _pair(journal: LifecycleJournal, phase: str, operation: Callable[[], None]) -> None:
    ...
    try:
        ...
        raise QualificationV3Error(reason) from failure
...
def _precheck(
    ...
    if counts["preexisting"] != 0:
        raise QualificationV3Error("identifier_conflict")
...
def _insert(
    ...
    if counts["inserted"] != 1:
        raise QualificationV3Error("insert_count_invalid")
...
def _verify(
    ...
    if counts["verified"] != 1:
        raise QualificationV3Error("fingerprint_mismatch")
...
def _delete(
    ...
    if counts["deleted"] != 1:
        raise QualificationV3Error("cleanup_count_invalid")
...
def _verify_deleted(
    ...
    if counts["remaining"] != 0:
        raise QualificationV3Error("cleanup_verification_failed")
...
def validate_manifest(value: object, *, repository_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or value["outputPaths"] != {"lifecycle": f"agent-runtime/tests/integration/adapters/employee/evidence/{RUN_ID}.lifecycle.jsonl", "result": f"agent-runtime/tests/integration/adapters/employee/evidence/{RUN_ID}.result.json"}
...
def _count(value: object, *, reason: str) -> int:
    if type(value) is not int or value not in (0, 1):
        raise QualificationV3Error(reason)
    return value
...
def finalize_live_candidate(
    *,
    ...
    if set(pending) != {"schemaVersion", "status", "reason", "fieldPresence", "counts"} or pending["schemaVersion"] != 3:
        _invalid()
