...
def test_manifest_authorization_assets_and_history_are_frozen() -> None:
    ...
    authorization = validate_authorization(
        raw_authorization, manifest_sha256=manifest_sha
    )
...
def test_manifest_binds_answer_v2_and_current_runtime_bootstrap() -> None:
    ...
    assert "agent-runtime/tests/integration/adapters/employee/egress_candidate_v2.py" in assets
    ...
    assert 'request.task_version != "answer-generation-v2"' in live_test
...
def test_five_schemas_are_strict_and_bind_candidate() -> None:
    ...
    for name in names:
        schema = json.loads(
            (EVIDENCE / f"employee-egress-v4-{name}.schema.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    lifecycle = json.loads(
        (EVIDENCE / "employee-egress-v4-lifecycle.schema.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_java_host_is_disabled_and_has_exact_sql_and_cleanup_contract() -> None:
    ...
    assert '@EnabledIfEnvironmentVariable(named = "RUN_EMPLOYEE_EGRESS_CANDIDATE_V4"' in source
...
def test_launcher_has_valid_ast_and_pre_model_fail_closed_checks() -> None:
    launcher = REPOSITORY / "agent-runtime/scripts/run-employee-egress-live-candidate-04.ps1"
    command = (
        "$e=$null;$t=$null;"
    ...
    assert "EMPLOYEE_EGRESS_V4_LIVE_AUTHORIZED" in source
