...
LIFECYCLE_SCHEMA = EVIDENCE / "transaction-egress-candidate-v3-lifecycle.schema.json"
RESULT_SCHEMA = EVIDENCE / "transaction-egress-candidate-v3-result.schema.json"
HOST_JOURNAL_SCHEMA = EVIDENCE / "transaction-egress-candidate-v3-host-preflight.schema.json"
HOST_RESULT_SCHEMA = EVIDENCE / "transaction-egress-candidate-v3-host-result.schema.json"
HOST_JOURNAL = EVIDENCE / f"{RUN_ID}.host-preflight.jsonl"
HOST_RESULT = EVIDENCE / f"{RUN_ID}.host-result.json"
...
def test_candidate_manifest_authorization_and_all_assets_are_frozen(tmp_path: Path) -> None:
    ...
    assert HOST_RESULT.exists()
    assert lifecycle_path_for(EVIDENCE).exists()
    assert not consumed_path_for(EVIDENCE).exists()
    assert result_path_for(EVIDENCE).exists()
...
def test_manifest_binds_answer_v2_current_bootstrap_and_retired_history() -> None:
    ...
    assert any(path.endswith("candidate-02.manifest.json") for path in history)
    assert any(path.endswith("candidate-02.authorization.json") for path in history)
    assert any(path.endswith("candidate-02.initialization-failure.json") for path in history)

    live_source = (
        RUNTIME
        / "tests/integration/adapters/transaction/test_real_transaction_egress_candidate_v3.py"
    ).read_text(encoding="utf-8")
    candidate_source = (
        RUNTIME / "tests/integration/adapters/transaction/egress_candidate_v3.py"
    ).read_text(encoding="utf-8")
...
def test_strict_schemas_match_three_states_budgets_and_no_extra_fields() -> None:
    ...
    host_journal = json.loads(HOST_JOURNAL_SCHEMA.read_text(encoding="utf-8"))
    host_result = json.loads(HOST_RESULT_SCHEMA.read_text(encoding="utf-8"))

    assert lifecycle["additionalProperties"] is False
    assert lifecycle["properties"]["schemaVersion"] == {"const": 3}
    assert lifecycle["properties"]["retryCount"] == {"const": 0}
    assert lifecycle["properties"]["resumeCount"] == {"const": 0}
    assert result["additionalProperties"] is False
    assert result["properties"]["schemaVersion"] == {"const": 3}
    ...
    assert host_journal["additionalProperties"] is False
    assert host_journal["properties"]["runId"] == {"const": RUN_ID}
    assert host_journal["properties"]["sequence"]["maximum"] == 4
    assert host_result["additionalProperties"] is False
    assert host_result["properties"]["status"]["enum"] == [
        "passed",
        "failed_unconsumed",
    ]
    assert all(
        value == {"const": 0}
        for value in host_result["properties"]["counts"]["properties"].values()
    )
    assert "collectionValidated" in host_result["required"]


def test_launcher_binds_all_assets_before_reading_secrets_and_never_starts_services() -> None:
    path = RUNTIME / "scripts/run-transaction-egress-live-candidate-03.ps1"
    ...
    host_preflight = script.index("& $python $hostPreflight")
    secret_read = script.index("GetEnvironmentVariable('LLM_API_KEY'")
    selector = script.index("SELECT TRANS_TYPE FROM t_transaction")
    pytest_entry = script.index("test_real_transaction_egress_candidate_v3.py")

    assert binding < asset_binding < host_preflight < secret_read < selector < pytest_entry
    assert "$maximumPaidAnswerCalls = 30" in script
    assert "test_real_transaction_egress_candidate_v3.py" in script
    assert "RUN_TRANSACTION_EGRESS_CANDIDATE_03" in script
    assert script.count("SELECT TRANS_TYPE FROM t_transaction") == 1
    assert script.count("executeQuery()") == 1
    assert "TRANSACTION_EGRESS_LIVE_DB_URL" in script
    assert "$env:PYTHONPATH = \"$runtimeSource;$runtime\"" in script
    assert "host-preflight.jsonl" in script
    assert "host-result.json" in script
    assert "collectionValidated" in script
...
def test_live_entry_is_opt_in_and_has_no_direct_evidence_overwrite() -> None:
    path = RUNTIME / "tests/integration/adapters/transaction/test_real_transaction_egress_candidate_v3.py"
    source = path.read_text(encoding="utf-8")
    tree = ast.parse(source)

    assert "RUN_TRANSACTION_EGRESS_CANDIDATE_03" in source
...
def test_prepared_assets_do_not_persist_query_or_transaction_values() -> None:
    prepared = (
        ...
        + HOST_JOURNAL_SCHEMA.read_text(encoding="utf-8")
        + HOST_RESULT_SCHEMA.read_text(encoding="utf-8")
    )
