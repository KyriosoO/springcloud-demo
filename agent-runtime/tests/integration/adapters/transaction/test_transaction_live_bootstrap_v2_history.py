...
def _historical_binding(tmp_path: Path) -> BootstrapBinding:
    return bind_historical_bootstrap(
        ...
        executable_paths=EXECUTABLE_ASSET_PATHS,
    )
...
def test_transaction_v2_manifest_authorization_and_histories_are_frozen(
    ...
    assert {row["path"] for row in manifest["executableHashes"]} == EXECUTABLE_ASSET_PATHS
    assert sha256_file(MANIFEST) == load_strict_json(AUTHORIZATION)["manifestSha256"]


def test_transaction_v2_assets_exist_at_frozen_source_commit() -> None:
    _require_prepared_assets()
    manifest = load_strict_json(MANIFEST)
...
def test_transaction_v2_is_prepared_without_outer_or_inner_outputs() -> None:
    _require_prepared_assets()
