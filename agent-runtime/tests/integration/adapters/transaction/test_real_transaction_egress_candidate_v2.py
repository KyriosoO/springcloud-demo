...
pytestmark = pytest.mark.skipif(
    os.environ.get("RUN_TRANSACTION_EGRESS_CANDIDATE_02") != "1",
    reason="requires exact GATE-026 candidate-02 opt-in",
)
...
def _context(token: str) -> CapabilityExecutionContext:
    return CapabilityExecutionContext(
        request_id="transaction-egress-candidate-02",
        correlation_id="transaction-egress-candidate-02",
...
def _domain_literals(value: object) -> tuple[str, ...]:
    values: list[str] = []
    if isinstance(value, Mapping):
        for item in value.values():
            values.extend(_domain_literals(item))
...
@pytest.mark.asyncio
async def test_gate026_candidate02_one_search_and_thirty_bounded_answers(
    ...
    valid_answers = 0
    forbidden_literals: tuple[str, ...] = (transaction_type, user_jwt)
    ...
    try:
        ...
        if result.status is not CapabilityStatus.SUCCESS or result.domain_result is None:
            ...
            forbidden_literals = forbidden_literals + (api_key,) + _domain_literals(result.domain_result)
