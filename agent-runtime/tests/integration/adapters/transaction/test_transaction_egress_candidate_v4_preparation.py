...
LIFECYCLE_SCHEMA = EVIDENCE / "transaction-egress-candidate-v4-lifecycle.schema.json"
RESULT_SCHEMA = EVIDENCE / "transaction-egress-candidate-v4-result.schema.json"
HOST_JOURNAL_SCHEMA = EVIDENCE / "transaction-egress-candidate-v4-host-preflight.schema.json"
HOST_RESULT_SCHEMA = EVIDENCE / "transaction-egress-candidate-v4-host-result.schema.json"
HOST_JOURNAL = EVIDENCE / f"{RUN_ID}.host-preflight.jsonl"
HOST_RESULT = EVIDENCE / f"{RUN_ID}.host-result.json"
FROZEN_SOURCE_COMMIT = "680cd25ac0475f301260123c8ce6229ed05dc8c9"
...
def test_manifest_binds_answer_v2_current_bootstrap_and_retired_history() -> None:
    ...
    candidate_source = (
        RUNTIME / "tests/integration/adapters/transaction/egress_candidate_v4.py"
    ).read_text(encoding="utf-8")
...
def test_launcher_binds_all_assets_before_reading_secrets_and_never_starts_services() -> None:
    path = RUNTIME / "scripts/run-transaction-egress-live-candidate-04.ps1"
    ...
    pytest_entry = script.index("test_real_transaction_egress_candidate_v4.py")

    assert binding < asset_binding < host_preflight < secret_read < selector < pytest_entry
    assert "$maximumPaidAnswerCalls = 30" in script
    assert "test_real_transaction_egress_candidate_v4.py" in script
    assert "RUN_TRANSACTION_EGRESS_CANDIDATE_04" in script
...
def test_live_entry_is_opt_in_and_has_no_direct_evidence_overwrite() -> None:
    path = RUNTIME / "tests/integration/adapters/transaction/test_real_transaction_egress_candidate_v4.py"
    source = path.read_text(encoding="utf-8")
    tree = ast.parse(source)

    assert "RUN_TRANSACTION_EGRESS_CANDIDATE_04" in source
