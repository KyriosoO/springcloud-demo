...
LIFECYCLE_SCHEMA = EVIDENCE / "transaction-egress-candidate-v1-lifecycle.schema.json"
RESULT_SCHEMA = EVIDENCE / "transaction-egress-candidate-v1-result.schema.json"
...
def test_launcher_binds_all_assets_before_reading_secrets_and_never_starts_services() -> None:
    path = RUNTIME / "scripts/run-transaction-egress-live-candidate-01.ps1"
    ...
    assert "test_real_transaction_egress_candidate.py" in script
    assert "RUN_TRANSACTION_EGRESS_CANDIDATE_01" in script
...
def test_live_entry_is_opt_in_and_has_no_direct_evidence_overwrite() -> None:
    path = RUNTIME / "tests/integration/adapters/transaction/test_real_transaction_egress_candidate.py"
    source = path.read_text(encoding="utf-8")
    tree = ast.parse(source)

    assert "RUN_TRANSACTION_EGRESS_CANDIDATE_01" in source
