...
def execute(expected_sha):
    ...
    save(ROOT / "authorization.json", dict(runId=RUN_ID, frozenHead=frozen["frozenHead"],
        manifestSha256=expected_sha, reference=REFERENCE, limits=LIMITS, cumulativeLimits=dict(e2e=35, model=92)))
    ...
    try:
        ...
        with bindings(), services.local_services(emit, include_agent=True) as (token, binding):
            ...
            require(len(rows) == 4 and all(r["passed"] and r["httpStatus"] == 200 for r in rows), "case_failed")
