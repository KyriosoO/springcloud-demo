...
from tests.system_e2e.knowledge_human_review import FIELDS, ReviewPortal, ReviewSession, packet

RUN_ID = "knowledge-representative-human-uat-v1-20260911-04"
ROOT = base.REPO / "agent-runtime/target" / RUN_ID
REFERENCE = "UAT_01:14.58"
...
def batch():
    ...
    with patch.multiple(base, RUN_ID=RUN_ID, ROOT=ROOT, REFERENCE=REFERENCE,
                        LIMITS=LIMITS, SELECTION=original_cases.SELECTION):
        yield
...
def manifest():
    ...
    names = base.service_run.git("ls-files", "agent-runtime/tests/system_e2e/knowledge_human_review.*",
        ...
        "agent-runtime/tests/system_e2e/test_knowledge_representative_human_uat_v1.py").splitlines()
    value["assets"].update({p: base.digest((base.REPO / p).read_bytes()) for p in names})
    value.update(knownBefore=dict(e2e=35, model=92), cumulativeLimits=dict(e2e=45, model=122),
...
class HumanBudget(base.Budget):
    def __init__(self, *args, session):
        super().__init__(*args)
        self.session = session

    def check_reviewer(self):
        if self.session.closed or self.session.remaining <= 0:
            self.stopped = True
            raise ValueError("review.stopped")

    def begin(self, case):
        self.check_reviewer()
        return super().begin(case)
...
def assess(case, response, observation, budget, session):
    result = base.assess(case, response, observation, budget)
    result["automaticPassed"] = result["passed"]
    if not result["passed"]:
        result["humanReview"] = dict(status="not_assessed", reason="automatic_failed")
        return result
    try:
        review = session.review(packet(case, response, budget.summary_input))
    except ValueError:
        review = dict(status="not_assessed", reason="review_unavailable")
    result["humanReview"] = review
    result["manualUsefulness"] = "not_assessed" if review["status"] != "assessed" else (
        "passed" if all(review[k] for k in FIELDS) else "failed")
    result["passed"] = result["manualUsefulness"] == "passed"
    if not result["passed"]:
        budget.stopped = True
    return result
...
def execute(expected_sha, session):
    ...
    base.save(ROOT / "authorization.json", dict(runId=RUN_ID, frozenHead=frozen["frozenHead"],
        manifestSha256=expected_sha, reference=REFERENCE, limits=LIMITS, cumulativeLimits=dict(e2e=45, model=122)))
    ...
    with batch():
        try:
            ...
            with base.bindings(), base.services.local_services(emit, include_agent=True) as (token, binding):
                ...
                base.require(len(rows) == 10 and all(r["passed"] and r["httpStatus"] == 200 for r in rows), "case_failed")
            base.require(manifest() == frozen, "final_binding_changed")
