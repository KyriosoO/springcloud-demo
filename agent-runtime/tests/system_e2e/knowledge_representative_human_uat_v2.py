...
from tests.system_e2e.knowledge_model_failure_probe_v2 import DETAILS, observe_failures

RUN_ID = "knowledge-representative-human-uat-v2-20260911-05"
ROOT = base.REPO / "agent-runtime/target" / RUN_ID
REFERENCE = "UAT_01:14.59"
SELECTION = tuple(item for item in original_cases.SELECTION if item[0] != "KRB-015")
...
def manifest():
    ...
    names = base.service_run.git("ls-files", "agent-runtime/tests/system_e2e/knowledge_human_review.*",
        ...
        "agent-runtime/tests/system_e2e/test_knowledge_representative_human_uat_v2.py",
    ...
    value.update(knownBefore=dict(e2e=37, model=97), cumulativeLimits=dict(e2e=46, model=124),
    ...
    value["failureDiagnostics"] = dict(version=2, details=list(DETAILS))
    return value
...
def execute(expected_sha, session):
    ...
    base.save(ROOT / "authorization.json", dict(runId=RUN_ID, frozenHead=frozen["frozenHead"],
        manifestSha256=expected_sha, reference=REFERENCE, limits=LIMITS, cumulativeLimits=dict(e2e=46, model=124)))
    ...
    with batch():
        try:
            ...
            with base.bindings(), base.services.local_services(emit, include_agent=True) as (token, binding):
                ...
                base.require(len(rows) == 9 and all(r["passed"] and r["httpStatus"] == 200 for r in rows), "case_failed")
