...
from tests.system_e2e import knowledge_representative_human_uat_v2 as runner
...
def test_scope_and_restoration_do_not_mutate_frozen_runner(tmp_path):
    ...
    with runner.batch(), base.bindings():
        ...
        assert [c["caseId"] for c in base.cases()] == [
            "KRB-006", "KRB-004", "KRB-010", "KRB-011", "KRB-012", "KRB-017", "KRB-019", "KRB-021", "KRB-023"]
...
def test_complete_or_stop_then_cleanup_terminal_and_no_resume(tmp_path, monkeypatch, failure):
    ...
    attempted = 1 if failure in {"automatic", "human", "timeout", "cancel"} else 9
    assert len(result["cases"]) == attempted and len(result["notExecuted"]) == 9 - attempted
