...
def local_services(emit, *, include_agent=False):
    for port in ((18090, 19401, 18080) if include_agent else (18090, 19401)):
        ...
        with tempfile.TemporaryDirectory(prefix="codex-kstage-b-services-") as directory:
            ...
            try:
                ...
                launches = (
                    ...
                    ("es-query-service", 19401, ["-cp", str(classes) + os.pathsep + classpath,
                ...
                with httpx.Client(trust_env=False, follow_redirects=False, timeout=3) as client:
                    ...
                    for process, url in zip(processes[:2], ("http://127.0.0.1:18090/public/test", "http://127.0.0.1:19401/actuator/health"), strict=True):
