...
- `REQ_00` v2.0 继续作为需求权威；当前设计基线为 L0 v2.3、L1 Core/Knowledge/Business v2.8/v1.6/v2.5，14 份 L2 的当前版本见下表。实施计划为 P3 v2.23，Business/Knowledge 验收计划分别为 UAT_00 v1.17、UAT_01 v1.6。
...
| `L0_00` | [单体 Agent L0 总体架构](design/L0_00_SINGLE_AGENT_ARCHITECTURE.md) | v2.3 / Approved | 系统边界、唯一链路、权限与分域决策 |

## 3. L1 架构

| 编号 | 文档 | 版本/状态 | 治理范围 |
|---|---|---|---|
| `L1_00` | [核心与运行架构](design/L1_00_SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md) | v2.8 / Approved | Runtime、filters planning bridge、可选 Knowledge、Core 与共享 Registry/组合根 |
| `L1_01` | [知识查询能力架构](design/L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) | v1.6 / Approved | 问题改写、多域、多路召回/重排、证据与摘要、P5 |
...
| 编号 | 文档 | 版本/状态 |
|---|---|---|
| `L2_01_00` | [Knowledge 流程与配置](design/L2_01_00_SINGLE_AGENT_KNOWLEDGE_QUERY_FLOW_CONFIGURATION_DETAILED_DESIGN.md) | v1.7 / Approved |
| `L2_01_01` | [Knowledge 检索与本地模型](design/L2_01_01_SINGLE_AGENT_KNOWLEDGE_RETRIEVAL_LOCAL_MODEL_DETAILED_DESIGN.md) | v1.6 / Approved |
| `L2_01_02` | [Knowledge 证据、出域、摘要与效果验证](design/L2_01_02_SINGLE_AGENT_KNOWLEDGE_EVIDENCE_EGRESS_SUMMARY_EFFECTIVENESS_DETAILED_DESIGN.md) | v1.7 / Approved |
...
- 当前实施计划：[P3_00 单体 Agent 查询能力代码实施计划](plans/P3_00_SINGLE_AGENT_CODE_IMPLEMENTATION_PLAN.md)，v2.23 / Reviewed；既有 25 个工作包 Done，本轮新增 7 个收口包中，文档纠偏、正式 Python 测试入口与 candidate-05 诊断已 Done，Summary V4/效果口径 v2 优化 Ready，后续候选、live 和最终收口按依赖推进。
- 当前验收计划：[UAT_00 单体 Agent 查询用户验收计划](plans/UAT_00_SINGLE_AGENT_ACCEPTANCE_TEST_PLAN.md)，v1.17 / Reviewed；18 项真实 v4 QueryPlan 与 17 项等价自动化风险闭合，共 35/35 可追踪。Knowledge 由 [UAT_01](plans/UAT_01_SINGLE_AGENT_KNOWLEDGE_ACCEPTANCE_TEST_PLAN.md) v1.6 独立治理，功能 37/37 Passed，效果仍为 `partially_effective`。
...
Employee/Transaction 需求、设计与 35/35 UAT 已完成且不得回退。Knowledge 默认关闭的生产接线、功能 UAT 37/37、域目录 v2 与 Summary V3 已实施；candidate-04/05 历史资产保持不可变，最新效果仍为 `partially_effective`。正式 Python 隔离入口与 candidate-05 根因诊断已关闭；下一步只实施评审通过的 Summary V4 与效果口径 v2，再准备 candidate-06 非 live 冻结及其精确授权效果 UAT。在新候选 Q1～Q4 全部通过前不得宣称 Knowledge 整体效果达标或七项收口完成。
