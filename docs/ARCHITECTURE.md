...
- Employee/Transaction 相关需求以 v2.0 为权威基线；本轮状态纠偏后 L0 为 v2.1、`L1_00` 为 v2.2、`L1_02` 为 v2.4，Core/Model L2 为 v2.1/v2.2，Business 公共/Employee/Transaction L2 为 v2.4，实施计划为 v2.16、正式验收计划为 v1.14。Knowledge 与未涉及的公共文档保持原版本。
...
| `L0_00` | [单体 Agent L0 总体架构](design/L0_00_SINGLE_AGENT_ARCHITECTURE.md) | v2.1 / Approved | 系统边界、唯一链路、权限与分域决策 |

## 3. L1 架构

| 编号 | 文档 | 版本/状态 | 治理范围 |
|---|---|---|---|
| `L1_00` | [核心与运行架构](design/L1_00_SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md) | v2.2 / Approved | Runtime、filters planning bridge、完整意图边界、Core 与三动作组合根 |
| `L1_01` | [知识查询能力架构](design/L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) | v1.0 / Approved | 问题改写、多域、多路召回/重排、证据与摘要、P5 |
| `L1_02` | [业务查询适配架构](design/L1_02_SINGLE_AGENT_BUSINESS_QUERY_ADAPTER_ARCHITECTURE.md) | v2.4 / Approved | 三动作 Adapter、字段级配置、operator-specific 文本安全、端点级角色转换、最终授权与列表结果 |
...
| `L2_00_01` | [Core 执行与能力注册](design/L2_00_01_SINGLE_AGENT_CORE_EXECUTION_CAPABILITY_REGISTRATION_DETAILED_DESIGN.md) | v2.1 / Approved |
| `L2_00_02` | [DeepSeek 接入与受控生成](design/L2_00_02_SINGLE_AGENT_DEEPSEEK_MODEL_ACCESS_CONTROLLED_GENERATION_DETAILED_DESIGN.md) | v2.2 / Approved |
...
| 编号 | 文档 | 版本/状态 |
|---|---|---|
| `L2_02_00` | [Business 公共约束、配置与出域](design/L2_02_00_SINGLE_AGENT_BUSINESS_QUERY_COMMON_CONSTRAINTS_CONFIGURATION_EGRESS_DETAILED_DESIGN.md) | v2.4 / Approved |
| `L2_02_01` | [Employee Adapter 与授权](design/L2_02_01_SINGLE_AGENT_EMPLOYEE_ADAPTER_AUTHORIZATION_DETAILED_DESIGN.md) | v2.4 / Approved |
| `L2_02_02` | [Transaction Adapter 与授权](design/L2_02_02_SINGLE_AGENT_TRANSACTION_ADAPTER_AUTHORIZATION_DETAILED_DESIGN.md) | v2.4 / Approved |
...
- 当前实施计划：[P3_00 单体 Agent 查询能力代码实施计划](plans/P3_00_SINGLE_AGENT_CODE_IMPLEMENTATION_PLAN.md)，v2.16 / Reviewed；16 个工作包 Done，相关门禁 Closed。
- 当前验收计划：[UAT_00 单体 Agent 查询用户验收计划](plans/UAT_00_SINGLE_AGENT_ACCEPTANCE_TEST_PLAN.md)，v1.14 / Reviewed；18 项真实 v4 QueryPlan 与 17 项等价自动化风险闭合，共 35/35 可追踪。
...
Employee/Transaction 需求与设计已通过独立分层和跨层评审，实施/UAT 计划为 Reviewed；Knowledge 及未涉及的公共设计保留原有 Approved 基线。统一配置、三个列表动作、生产组合根、Employee 最终授权、Transaction 日期/文本策略和 v4 完整意图 Prompt 均已实施。正式 run03 UAT 18/18 通过：Employee search 6 次、semantic 1 次、Transaction search 7 次，四项 unsupported 均零业务调用；其他 endpoint、answer、Knowledge、retry/resume 和敏感持久化均为 0。结果 SHA-256=`b49832426147dc14d56e571fea11b0345e16602d8cb5e2ea2eeb3dacb3326dd8`；两次历史失败结果保持不可变。
