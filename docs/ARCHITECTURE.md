...
- [单体 Agent 查询能力 L0 总体架构设计](design/SINGLE_AGENT_ARCHITECTURE.md)（v0.3，已评审、有条件通过、未实施、未生效）

## 架构文档状态

- L0 总体架构：五轮独立评审已完成且无剩余 S0/S1；须由项目维护者确认 `SA-GATE-001` 后，方可作为 L1 编写基线。
- L1 领域或模块架构：尚未建立；规划为核心与运行、知识查询能力、业务查询适配三份 L1。
