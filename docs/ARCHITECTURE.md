...
- [`L2_02_00` 单体 Agent 业务查询公共约束、配置与出域详细设计 L2](design/L2_02_00_SINGLE_AGENT_BUSINESS_QUERY_COMMON_CONSTRAINTS_CONFIGURATION_EGRESS_DETAILED_DESIGN.md)（v0.3，Approved、五轮独立评审及第三批聚焦一致性复核通过；`BQ-GATE-002` 及真实业务/数据出域门禁 Open，未实施、未生效）
- [`L2_02_01` 单体 Agent Employee Adapter 与业务授权联调详细设计 L2](design/L2_02_01_SINGLE_AGENT_EMPLOYEE_ADAPTER_AUTHORIZATION_DETAILED_DESIGN.md)（v0.3，Approved、五轮独立评审及直接依赖聚焦一致性复核通过；Provider/真实 Employee/出域门禁 Open，未实施、未生效）
- [`L2_02_02` 单体 Agent Transaction Adapter 与业务授权联调详细设计 L2](design/L2_02_02_SINGLE_AGENT_TRANSACTION_ADAPTER_AUTHORIZATION_DETAILED_DESIGN.md)（v0.2，Approved、五轮独立评审通过；Provider/真实 Transaction/出域门禁 Open，未实施、未生效）
...
| `L2_02_00` | 单体 Agent 业务查询公共约束、配置与出域 L2 | `L1_02` | 已创建（v0.3 Approved，五轮评审及聚焦一致性复核通过；实施/集成门禁 Open） |
| `L2_02_01` | 单体 Agent Employee Adapter 与业务授权联调 L2 | `L1_02` | 已创建（v0.3 Approved，五轮评审及直接依赖聚焦一致性复核通过；Provider/集成门禁 Open） |
| `L2_02_02` | 单体 Agent Transaction Adapter 与业务授权联调 L2 | `L1_02` | 已创建（v0.2 Approved，五轮独立评审通过；Provider/集成门禁 Open） |
...
- 第二批四份 L2 已依次完成最多五轮独立评审—修复—复核并 Approved：`L2_00_00` v0.2 固化 Spring 接入、跨进程协议、总时限与取消；`L2_00_02` v0.4 固化 DeepSeek Provider、输入闸门、结构化动作和受控生成；`L2_01_00` v0.2 固化 Knowledge 单动作流程、问题改写、多域选择与检索计划；`L2_02_00` 当前 v0.3 在原五轮评审基础上完成业务响应 codec 请求关联的聚焦一致性复核。连同先行完成的 `L2_00_01`，这五份 L2 均未实施、未生效，各自实施/集成门禁保持 Open。
- 第三批三份 L2 已依次完成五轮独立评审—修复—复核并 Approved：`L2_01_01` v0.2 固化 Knowledge typed ES/BGE、授权前置、两级 Profile 映射、RRF/Rerank 与候选契约；`L2_02_01` v0.3 固化首期仅 `employee.detail`，并完成对公共 codec v0.3 的聚焦一致性复核；`L2_02_02` v0.2 固化首期仅 `transaction.search`，排除金额条件、Date、聚合及写入口。三份均未实施、未生效，所有切片实施、Provider、真实集成和数据出域门禁保持 Open。
...
- `L2_00_00` v0.2、`L2_00_02` v0.4、`L2_01_00` v0.2、`L2_02_00` v0.3 当前均为 Approved；独立评审和严格校验不代表实施门禁关闭、实现完成、真实 Provider/检索/业务链路可用或真实数据允许出域。
- `L2_01_01` v0.2、`L2_02_01` v0.3、`L2_02_02` v0.2 当前均为 Approved；五轮独立评审只证明详细设计达到实施就绪质量，不构成代码实施授权、Provider 契约确认、真实链路可用或真实数据出域证据。
