...
- `REQ_00` v2.2 继续作为需求权威；当前设计基线为 L0 v2.6、L1 Core/Knowledge/Business v3.4/v1.9/v2.8，10 份 L2 的当前版本见下表。实施计划为 P3 v2.33，Business/Knowledge 验收计划分别为 UAT_00 v1.24、UAT_01 v1.12。本索引不复制工作包、Gate、candidate、哈希或动态测试总数。
...
| 编号 | 文档 | 版本/状态 | 治理范围 |
|---|---|---|---|
| `L1_00` | [核心与运行架构](design/L1_00_SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md) | v3.4 / Approved | Runtime、filters planning bridge、可选 Knowledge、Core 与共享 Registry/组合根 |
...
- 当前实施计划：[P3_00 单体 Agent 查询能力实施与收口计划](plans/P3_00_SINGLE_AGENT_CODE_IMPLEMENTATION_PLAN.md)，v2.33 / Reviewed；工作包、Gate、当前验证和提交状态只在该计划维护。
- 当前验收计划：[UAT_00 单体 Agent 结构化查询用户验收计划](plans/UAT_00_SINGLE_AGENT_ACCEPTANCE_TEST_PLAN.md)，v1.24 / Reviewed，固定治理35个既有 Business 用例并新增15个已通过的 Employee 自然语言扩展用例；Knowledge 由 [UAT_01](plans/UAT_01_SINGLE_AGENT_KNOWLEDGE_ACCEPTANCE_TEST_PLAN.md) v1.12 独立治理。
