...
- Employee/Transaction 相关需求、L0/L1/L2 与实施计划以 v2.0 为新权威基线；Employee 端点级授权、真实 ES partial hits 以及 Transaction 生产 Spring Date 响应合同纠偏后，`L1_02` 为 v2.2、Business 公共 L2 为 v2.2、Employee L2 为 v2.3、Transaction L2 为 v2.2，实施计划为 v2.9、正式验收计划为 v1.7。Knowledge 与未涉及的公共文档保持原版本。
...
| `L2_02_02` | [Transaction Adapter 与授权](design/L2_02_02_SINGLE_AGENT_TRANSACTION_ADAPTER_AUTHORIZATION_DETAILED_DESIGN.md) | v2.2 / Approved |
...
- 当前实施计划：[P3_00 单体 Agent 查询能力代码实施计划](plans/P3_00_SINGLE_AGENT_CODE_IMPLEMENTATION_PLAN.md)，v2.9 / Reviewed。计划是调度/证据索引，不得反向修改设计权威。
- 当前验收计划：[UAT_00 单体 Agent 查询用户验收计划](plans/UAT_00_SINGLE_AGENT_ACCEPTANCE_TEST_PLAN.md)，v1.7 / Reviewed；公共接入冒烟通过，Employee/Transaction 正式 UAT 已具备执行条件。
...
Employee/Transaction 需求与设计已通过独立分层和跨层评审，实施/UAT 计划为 Reviewed；Knowledge 及未涉及的公共设计保留原有 Approved 基线。新版统一配置、三个列表动作、生产组合根及 Employee 两个 ES POST endpoint 专用共享 JWT role converter 均已实施；Transaction 生产 Spring UTC 日期字符串与历史 epoch 已完成严格兼容。受控真实联调 6 个场景和公共接入 Java 20 项均已通过；正式 Employee/Transaction UAT 已具备执行条件但尚未完成。
