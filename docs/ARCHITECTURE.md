...
- `REQ_00` v2.4；L0 v2.8、L1 Core/Knowledge/Business v3.5/v1.19/v2.8，10份L2当前版本见下表。实施计划P3 v2.49；验收计划UAT_00 v1.24、UAT_01 v1.27；路线图ROADMAP_01 v0.8。阶段B新增语义仍须按评审和实施事实分开管理，不继承历史通过结论。本索引不复制工作包、Gate、candidate、哈希或动态测试总数。
...
| `L1_01` | [知识查询能力架构](design/L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) | v1.19 / Approved | 在线查询及离线语料构建平面、证据与受控发布 |
...
| 编号 | 文档 | 版本/状态 |
|---|---|---|
| `L2_01_00` | [Knowledge 流程与配置](design/L2_01_00_SINGLE_AGENT_KNOWLEDGE_QUERY_FLOW_CONFIGURATION_DETAILED_DESIGN.md) | v1.22 / Approved |
| `L2_01_01` | [Knowledge 检索、本地模型与阶段 A 语料生命周期](design/L2_01_01_SINGLE_AGENT_KNOWLEDGE_RETRIEVAL_LOCAL_MODEL_DETAILED_DESIGN.md) | v2.8 / Approved |
| `L2_01_02` | [Knowledge 证据、出域、摘要与效果验证](design/L2_01_02_SINGLE_AGENT_KNOWLEDGE_EVIDENCE_EGRESS_SUMMARY_EFFECTIVENESS_DETAILED_DESIGN.md) | v1.20 / Approved |
...
- 当前实施计划：[P3_00 单体 Agent 查询能力实施与收口计划](plans/P3_00_SINGLE_AGENT_CODE_IMPLEMENTATION_PLAN.md)，v2.49 / Reviewed；工作包、Gate、当前验证和提交状态只在该计划维护。
- 当前验收计划：[UAT_00 单体 Agent 结构化查询用户验收计划](plans/UAT_00_SINGLE_AGENT_ACCEPTANCE_TEST_PLAN.md)，v1.24 / Reviewed；Knowledge 在线与阶段 A 语料专项验收由 [UAT_01](plans/UAT_01_SINGLE_AGENT_KNOWLEDGE_ACCEPTANCE_TEST_PLAN.md) v1.27 独立治理。后续演进见 [ROADMAP_01](plans/ROADMAP_01_SINGLE_AGENT_KNOWLEDGE_CORPUS_RETRIEVAL_GRAPH_EVOLUTION_PLAN.md) v0.8。
...
Employee/Transaction 需求、设计与35/35 UAT 已完成且不得回退。Knowledge 默认关闭的生产接线、功能 UAT 37/37、域目录 v2、Summary V5及non-live与效果口径 v2 已实施；阶段 A 正文及附件完整性已完成审计、版本化处理、candidate 索引、14/14专项 UAT 和受控发布。最新有效P5效果等级仍为 `partially_effective`；阶段 B 已实施有界检索与最小必要域指令，当前V5部分真实场景已通过，但跨域必要证据覆盖及完整专项仍未通过；图谱未实施。具体 Gate、candidate、测试总数和 evidence 只在 P3、UAT_01 与 evidence 中维护。
