...
- [`L2_01_00` 单体 Agent Knowledge 查询流程与配置详细设计 L2](design/L2_01_00_SINGLE_AGENT_KNOWLEDGE_QUERY_FLOW_CONFIGURATION_DETAILED_DESIGN.md)（v0.3，Approved；v0.2 五轮独立评审及 v0.3 summary task 组合根兼容性复评通过；`KQ-GATE-002` Open，未实施、未生效）
...
- [`L2_01_02` 单体 Agent Knowledge 证据、出域、摘要与效果验证详细设计 L2](design/L2_01_02_SINGLE_AGENT_KNOWLEDGE_EVIDENCE_EGRESS_SUMMARY_EFFECTIVENESS_DETAILED_DESIGN.md)（v0.2，Approved；三轮作者内审及五轮独立评审通过；证据实施、真实出域与效果门禁 Open，未实施、未生效）
...
| `L2_01_00` | 单体 Agent Knowledge 查询流程与配置 L2 | `L1_01` | 已创建（v0.3 Approved，五轮评审及 summary task 兼容性复评通过；实施门禁 Open） |
| `L2_01_01` | 单体 Agent Knowledge 检索与本地模型接入 L2 | `L1_01` | 已创建（v0.2 Approved，五轮独立评审通过；实施/集成门禁 Open） |
| `L2_01_02` | 单体 Agent Knowledge 证据、出域、摘要与效果验证 L2 | `L1_01` | 已创建（v0.2 Approved，三轮作者内审及五轮独立评审通过；实施/出域/效果门禁 Open） |
...
- 第二批详细设计中，`L2_00_00` v0.2、`L2_00_02` v0.4、`L2_01_00` v0.3 均为 Approved，其中 `L2_01_00` 的 summary task 组合根兼容性复评已通过；`L2_02_00` v0.4 的精确十进制共享契约已完成两轮独立复评，`REV-BQCOM-023`～`025` 全部关闭，下游与 Core 定向兼容检查通过并恢复 Approved。连同 `L2_00_01`，这些 L2 均未实施、未生效，各自实施/集成门禁保持 Open。
...
- 第四批详细设计 `L2_01_02` v0.2 已完成三轮作者内审及五轮独立评审，`REV-KEV-001`～`023` 全部关闭并 Approved；同时完成 `L2_01_00` summary task 组合根和 `L2_01_01` 候选消费兼容检查。证据代码、真实策略目录、DeepSeek 出域和 P5 效果验证仍未实施，相关门禁全部 Open。
...
- `L2_00_00` v0.2、`L2_00_01/02` v0.4、`L2_01_00` v0.3 与 `L2_02_00` v0.4 当前均为 Approved；严格校验或设计评审通过不代表实施门禁关闭、实现完成、真实 Provider/业务链路可用或真实数据允许出域。
- `L2_01_01/02` v0.2、`L2_02_01/02` v0.3 当前均为 Approved；Knowledge 候选消费、Employee GET/no-body 和 Transaction 精确金额请求的相应兼容性检查均已完成。任何评审结论都不构成代码实施授权、Provider 契约确认、真实链路可用或真实数据出域证据。
