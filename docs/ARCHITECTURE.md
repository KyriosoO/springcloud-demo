...
- [单体 Agent 核心与运行架构 L1](design/SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md)（v0.1，草稿、未评审、未实施、未生效；`CR-GATE-001` 未关闭）

## 架构文档状态

- L0 总体架构：v0.3 五轮独立评审及 v0.4 针对性复核均无剩余 S0/S1，项目维护者已完成确认，`SA-GATE-001` 已关闭；v0.4 是当前三份 L1 的编写基线。
- L1 领域或模块架构：核心与运行 L1 v0.1 已形成草稿，明确 Spring 接入治理、LangGraph 唯一编排、确定性核心、能力契约/注册及 DeepSeek 模型端口边界；Knowledge L1 和业务查询 L1 待创建。
- L2 详细设计：尚未建立；核心与运行 L1 的 `CR-GATE-001` 未关闭，当前不得将其草稿作为已批准的 L2 或实现基线。
...
- 核心与运行 L1 v0.1 只对其草稿范围提供下位设计输入，不改变 L0 状态，也不代表内部协议、模型 PoC 或实现已完成。
