...
- `REQ_00` v2.4；L0 v2.8、L1 Core/Knowledge/Business v3.5/v1.21/v2.8，10份L2当前版本见下表。实施计划P3 v2.59；验收计划UAT_00 v1.24、UAT_01 v1.35；路线图ROADMAP_01 v0.9。阶段B新增语义仍须按评审和实施事实分开管理，不继承历史通过结论。本索引不复制工作包、Gate、candidate、哈希或动态测试总数。
...
- 当前实施计划：[P3_00 单体 Agent 查询能力实施与收口计划](plans/P3_00_SINGLE_AGENT_CODE_IMPLEMENTATION_PLAN.md)，v2.59 / Reviewed；工作包、Gate、当前验证和提交状态只在该计划维护。
- 当前验收计划：[UAT_00 单体 Agent 结构化查询用户验收计划](plans/UAT_00_SINGLE_AGENT_ACCEPTANCE_TEST_PLAN.md)，v1.24 / Reviewed；Knowledge 在线与阶段 A 语料专项验收由 [UAT_01](plans/UAT_01_SINGLE_AGENT_KNOWLEDGE_ACCEPTANCE_TEST_PLAN.md) v1.35 独立治理。后续演进见 [ROADMAP_01](plans/ROADMAP_01_SINGLE_AGENT_KNOWLEDGE_CORPUS_RETRIEVAL_GRAPH_EVOLUTION_PLAN.md) v0.9。
