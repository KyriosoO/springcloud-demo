...
- [`L2_00_01` 单体 Agent 核心执行与能力注册详细设计 L2](design/L2_00_01_SINGLE_AGENT_CORE_EXECUTION_CAPABILITY_REGISTRATION_DETAILED_DESIGN.md)（v0.3，Approved、已评审通过、未实施、未生效；`CR-GATE-002` 保持 Open）
...
- `L2_00_01` 核心执行与能力注册详细设计：v0.3 已完成五轮独立评审—修订—复核并进入 Approved，关闭 `REV-L2-001`～`REV-L2-009`，固化空注册表零模型调用、一次性动作与取消终态、公共结果映射、模型窄决定、code/source-only 失败契约、能力注册冻结和实现/测试落点；`CR-GATE-002` 保持 Open，未授权代码实施。
- 其余 L2 详细设计尚未建立；三份 L1 的 L1→L2 分解门禁均已关闭，可继续编写 `L2_00_00`、`L2_00_02`、`L2_01_00`～`L2_01_02`、`L2_02_00`～`L2_02_02`。切片实施、外部契约变更、真实模型/数据集成和效果证据均未完成。
...
- `L2_00_01` v0.3 是模型无关 Python 核心执行与能力注册切片的 Approved 详细设计基线，可用于申请关闭实施门禁；Approved 不等于当前实施依据或代码授权，`CR-GATE-002` 未关闭时不得创建目标代码、测试、配置或依赖。Spring→Python 传输/入口上下文和真实 DeepSeek 契约仍分别由 `L2_00_00`、`L2_00_02` 承接。
