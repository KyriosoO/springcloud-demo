...
| 当前基线 | Knowledge 链路、新版 filters/config/三动作 Adapter/生产组合根、Transaction operator-specific 文本策略及六场景 controlled 真实联调已通过；正式完整 UAT 尚未完成 |
...
开放事项：Employee Controller 读取守卫及两个 ES POST endpoint-scoped 共享 JWT role converter 已实施；真实 Servlet 过滤链角色矩阵、detail/fallback 兼容和 Employee 全模块回归已通过。新版 filters 合同、统一配置、两个 Employee Adapter、扩展 Transaction Adapter、Date/Decimal 合同、operator-specific 文本策略、生产组合根、1438 项 non-live 和六场景真实 controlled 均已具备当前代码证据；完整正式 UAT 尚未完成。首次失败结果保持不可变，只用于追溯，不替代新目标成功证明。
