...
| `IMPL-KFLOW-004` | 已有 `knowledge/rewrite_v3.py`（精确合同）与 `knowledge/semantic_planner.py`（同一Rewrite Stage协议适配）；`knowledge/rewrite_v4.py`保留澄清规则，新增`knowledge/rewrite_v5.py`细化域指令；bootstrap唯一绑定V5；历史文件不改 |
