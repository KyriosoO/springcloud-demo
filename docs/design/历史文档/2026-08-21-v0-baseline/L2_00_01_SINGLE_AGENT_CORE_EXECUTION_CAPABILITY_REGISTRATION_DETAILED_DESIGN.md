...
| 文档层级 | L2 详细设计 |
| 文档状态 | Approved |
| 评审状态 | v0.3 五轮、v0.4 针对性复评、v0.5 实现代码对照评审、v0.6 三轮作者内审/五轮独立设计评审及 v0.8 相邻接缝聚焦评审均保持有效；v0.10 已同步相邻 `WP-MODEL-RUNTIME-01` 实施证据；v0.11 仅同步问题出域非 live 安全门禁，不改变本文 Core/Resolver 契约 |
| 当前版本 | v0.11 |
| 日期 | 2026-08-12 |
| 适用范围 | Python `agent-runtime` 内的 LangGraph 请求状态、Provider-neutral 本地 Resolver 与混合动作解析、`agent-core` 确定性执行、`agent-capability-api`、进程内能力注册运行时、组合根及模型无关测试替身 |
| 上位文档 | [`L1_00`《单体 Agent 核心与运行架构 L1》](L1_00_SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md) v0.6（已评审/已通过，`CR-GATE-001/003` 已关闭；本轮只同步相邻问题输入门禁） |
| 来源文档 | [`REQ_00`《单体 Agent 查询能力建设需求说明》](../REQ_00_SINGLE_AGENT_QUERY_REQUIREMENTS.md) v1.3；[`L0_00`《单体 Agent 查询能力 L0 总体架构设计》](L0_00_SINGLE_AGENT_ARCHITECTURE.md) v0.9 |
| 关联文档/契约 | [`L1_01` Knowledge L1](L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) v0.4；[`L1_02` 业务查询 L1](L1_02_SINGLE_AGENT_BUSINESS_QUERY_ADAPTER_ARCHITECTURE.md) v0.4；[`L2_00_00` Spring 接入与运行协同](L2_00_00_SINGLE_AGENT_SPRING_ACCESS_RUNTIME_COORDINATION_DETAILED_DESIGN.md) v0.7 Approved；[`L2_00_02` DeepSeek 模型接入与受控生成](L2_00_02_SINGLE_AGENT_DEEPSEEK_MODEL_ACCESS_CONTROLLED_GENERATION_DETAILED_DESIGN.md) v0.17（`GATE-020/SA-GATE-002` 与问题输入 `CR-GATE-003` 已按各自范围关闭；默认仍为stub）；[`L2_01_00` Knowledge 查询流程与配置](L2_01_00_SINGLE_AGENT_KNOWLEDGE_QUERY_FLOW_CONFIGURATION_DETAILED_DESIGN.md) v0.7 Approved；[`L2_02_00` 业务查询公共约束、配置与出域](L2_02_00_SINGLE_AGENT_BUSINESS_QUERY_COMMON_CONSTRAINTS_CONFIGURATION_EGRESS_DETAILED_DESIGN.md) v0.8 Approved |
| 实现基线 | v0.5 `agent-runtime` Core 历史实现保持；v0.6 `LocalActionResolver`、ID-only `CapabilitySelectionNode`、混合裁决、固定失败映射和组合根绑定已实现。直接测试 36 passed、全量行为回归 376 passed/4 skipped、compileall 与完整 `src tests` strict mypy 229 files 均通过；`VAL-CORE-006/007` 已通过 |
| 技术基线 | Python `>=3.12,<3.13`；`langgraph==1.2.9`；使用 `StateGraph`、`TypedDict` 状态和 `context_schema` 运行上下文，不配置 checkpointer 或 store |
| 是否可作为实现依据 | 按范围可用 |
| 实施依据说明 | 作为已实现 v0.5 Core 与 v0.6 混合解析代码的维护依据；Core、业务 Resolver 与 ID-only 中立契约未因 v4 Provider 编码或 Runtime模型装配而修改。相邻`L2_00_02` v0.16已完成受控wiring并关闭`GATE-020/SA-GATE-002`；模型仍通过既有`CapabilitySelectionNode/AnswerGenerationNode`注入 |
| 当前允许实施范围 | 维护已实现 Core/Resolver 与相邻默认stub模型装配；后继真实出域仍按独立工作包与门禁推进 |
| 当前禁止动作 | 修改 Core/Resolver、最终`ActionCandidate`或领域参数契约，执行额外真实模型调用，启用真实业务/知识数据出域，或把Runtime切片完成解释为系统/生产生效 |
| 修改权限 | 用户于 2026-08-12 授权同步问题出域非 live 安全证据；明确禁止修改本文公共 Core/Resolver 契约、真实模型/数据调用和 Git commit/push |
| 维护责任人 | 项目维护者（个人开发者，姓名未在需求中指定） |

> v0.8 保持 v0.6 的最终 `ActionCandidate`、`CapabilitySelectionDecision`、`CapabilityExecutionCore.execute`、能力注册表和图执行结果契约不变；相邻 `L2_00_02` 可把模型侧编码从 Tool Calls 改为 JSON ID envelope，Core 仍只消费 canonical capability ID。本文不定义 Employee/Transaction 具体语法或 DeepSeek Provider DTO；二者分别归业务 L2 与 `L2_00_02`。
...
| 17 | 2026-08-01 | 1、3.6、7、15～16、18、20～21 章 | `WP-CORE-01` 实施完成后的原子证据同步 | 记录 Core 实现基线、61 项测试及严格类型/依赖验证；同步代码对照设计评审修复，关闭 `CR-GATE-002`，版本升为 v0.5；不扩大 HTTP、领域、模型或集成范围 |
| 18 | 2026-08-07 | 1～4、7～11、15～21 章 | 增加本地 Resolver、ID-only 模型选择、混合裁决、启动一致性、失败和测试契约 | 修复 v2 模型同时选择动作并生成业务参数的不稳定边界；保持最终候选、Core 执行和既有业务 Provider 契约兼容 |
| 19 | 2026-08-07 | 1～3、15～16、19～21 章 | `WP-ACTION-RESOLUTION-01` 实施与验证证据同步 | 记录 `IMPL-CORE-010～013` 已实现、`VAL-CORE-006` 36 passed、全量行为回归 376 passed/4 skipped、compileall/生产及相关 strict mypy 通过和两轮代码对照设计评审；完整 tests strict mypy 的 28 项工作包外既存错误保持可见，故不宣称 `VAL-CORE-007` 全部通过或工作包 Done |
| 20 | 2026-08-07 | 1、16、18～21 章 | `VAL-CORE-007` 类型门禁修复与工作包关闭 | 仅修复 11 个测试文件中的 28 项既存类型错误；定向 strict mypy、50 项定向测试、完整 376 passed/4 skipped、229 files strict mypy 与 compileall 通过。未改生产代码或公共契约；`WP-ACTION-RESOLUTION-01` 置 Done |
| 21 | 2026-08-07 | 1～2、5、8.2、8.6、16、18～21 | v3 PoC 失败后的 v4 相邻契约同步 | 明确 Core 只消费 Provider-neutral ID 决定，不感知 Tool Calls 或 JSON envelope；执行 Schema 继续不进入模型，最终候选、Resolver、失败语义和既有代码均不变；v4 增量受 `GATE-036` 聚焦复评约束 |
| 22 | 2026-08-07 | 1～2、5～6、8.2/8.8、15、18～21 | v4 Provider-neutral ID 边界独立聚焦评审修复 | 删除现行 Tool Schema 投影残留，明确 Core 只提供模型安全描述候选且 Provider wire/目录/历史证据归 `L2_00_02`；清理立项期现状冲突并记录 `GATE-036` 只满足评审子条件 |
| 23 | 2026-08-07 | 1、18～21 | v4 非 live 实施证据相邻同步 | 核实 `L2_00_02` v0.12 的 catalog/no-tools/exact ID 实现仍只返回 `CapabilitySelectionDecision`，未修改 `ActionCandidate`、Core、Resolver、能力参数 Schema 或 handler；同步 `VAL-MODEL-008` 与 `GATE-036` Closed，不重开 Core 评审 |
| 24 | 2026-08-10 | 1～2、5、16、19～21 | v4 candidate-01失败及corrected非live相邻状态同步 | 核实 `L2_00_02` v0.14 只版本化PoC fixture/provenance/manifest/tests；candidate-01失败不可改判，candidate-02未消费；Provider-neutral ID、最终`ActionCandidate`、Core、Resolver、能力参数Schema和handler均未修改，不重开Core评审 |
| 25 | 2026-08-10 | 1～2、5、16、19～21 | candidate-02 live通过相邻状态同步 | 核实`L2_00_02` v0.15仅新增append-only result/consumed证据并关闭`GATE-038`；30次只验证ID决定，未进入混合节点、Core、Resolver、validator或handler，本文公共契约和正式评审结论不变 |
| 26 | 2026-08-12 | 1～2、5、16、19～21 | `WP-MODEL-RUNTIME-01` 相邻实施状态同步 | 核实模型组合根仅通过既有Provider-neutral selector/answer节点与外层Context binding装配；未修改`ActionCandidate`、`CapabilitySelectionDecision`、Resolver、validator、handler或Core执行语义；`GATE-020/SA-GATE-002`按模型Runtime切片关闭 |
| 27 | 2026-08-12 | 1～2、18～21 | 问题出域非 live 安全门禁同步 | 核实输入 Guard、Knowledge/Business fixture、selector/answer/rewrite/summary 零 transport 测试均在 Core 外部完成；`CR-GATE-003` 按问题输入安全前置关闭，`ActionCandidate`、Resolver、validator、handler 与 Core 执行语义不变 |

## 3. 背景、目标与范围

### 3.1 背景与问题

L1_00 已确认 LangGraph 是唯一 Agent 编排权威，`agent-core` 只承担确定性执行约束，并通过稳定能力 API 和启动期冻结的进程内注册表连接 Knowledge、Employee、Transaction。设计立项时尚无目标 Python 实现，若直接进入能力开发，三个能力容易分别定义动作描述、执行上下文、结果状态、注册方式或模型载荷，最终造成核心业务分支、状态分裂和跨能力契约漂移；当前实现事实以第 1、15、16、19～21 章为准。
...
| `REQ-CORE-010` | 需要执行参数的业务动作由本地确定性 Resolver 形成候选，模型只选择能力 ID | 本地唯一候选时模型调用为零；本地 invalid/多候选失败关闭；零匹配才调用模型；模型选择非空参数动作时不得猜参或执行 | L0 `SA-C-022`；L1_00 `CR-AD-009` |
...
- Provider-neutral `LocalActionResolver`、`CapabilitySelectionNode` 和混合动作解析节点的公共契约、裁决顺序与启动校验。
- 并发、无持久化、无重放、日志脱敏、回滚和测试追踪。

### 3.4 范围外

- Spring `agent-service`、外部 HTTP API、JWT 验签和 Spring→Python 传输 DTO；归 `L2_00_00`。
- DeepSeek 请求/响应、问题最小化、能力 ID 选择 Provider、回答生成、模型重试和真实模型 PoC；归 `L2_00_02`。
- Knowledge 的问题改写、逻辑域、检索、BGE、证据、摘要和出域策略；归 L1_01 及其 L2。
- Employee/Transaction 的精确动作、有限语法 Resolver 实现、字段、Adapter 客户端、权限和出域策略；归 L1_02 及其 L2。
...
| 测试 | 适用 | 已存在 Python 单元、契约、架构和模型无关集成测试 |
...
7. v0.5 已按当前实现证据把本切片落点更新为“已存在”；仍不存在的相邻切片继续保持建议状态。
...
| `CON-CORE-014` | L0 `SA-C-022`、L1_00 `CR-AD-009` | 外部模型只返回能力 ID；业务参数由本地 Resolver 产生；最终候选仍由 Core 与注册项 validator 重校验 | `DR-CORE-015/016/017/018` | 无 |
...
| `REQ-CORE-010`、`CON-CORE-014` | 动作解析 | `DR-CORE-015`、`DR-CORE-016`、`DR-CORE-017`、`DR-CORE-018` | 混合节点、Resolver、组合根 | 模型 ID 与本地参数职责分离；最终候选不变 | `IMPL-CORE-010`、`IMPL-CORE-011`、`IMPL-CORE-012`、`IMPL-CORE-013` | `TEST-CORE-011`、`TEST-CORE-012`、`TEST-CORE-013`、`TEST-CORE-014` | `VAL-CORE-006`、`VAL-CORE-007` |
...
| L0_00 v0.8 | parent | 不弱化单体 Agent、LangGraph 权威和失败关闭 | 定义总体架构 | `SA-C-*`、`SA-AD-*` | 架构权威 | 只读 |
| L1_00 v0.5 | parent | 细化 L2_00_01 唯一范围 | 定义核心运行模块边界和门禁 | `CR-AD-*`、统一状态 | 直接上位权威 | 只读 |
| L1_01 v0.3 | peer | 提供公共能力契约，供 Knowledge 未来实现 | 拥有 Knowledge 流程、领域结果和出域策略 | `knowledge.query` 处理器 | Knowledge 状态/配置 | 只读 |
| L1_02 v0.3 | peer | 提供公共能力契约，供业务 Adapter 实现 | 拥有业务动作、领域结果、权限和出域策略 | 业务动作处理器 | 业务动作/配置 | 只读 |
| `L2_00_00` v0.5 Approved | peer | 定义 Python 内部执行上下文的消费语义 | 定义 Spring→Python 传输、JWT 验证、截止时间换算和外部映射 | `ExecutionContext` 构造边界 | 契约资产和限定双进程接入已验证 | 只读 |
| `L2_00_02` v0.16（Runtime受控装配已验证） | peer | 提供模型节点读取/写入的核心状态字段和安全调用前提 | 定义 Provider 编码、ID 决定、输入闸门、回答生成和client生命周期 | `CapabilitySelectionInput/Decision`、安全载荷 | 模型调用状态 | 只读；`GATE-038/020/SA-GATE-002`按各自范围Closed，默认仍为stub |
| `L2_01_00` v0.6 Approved | peer/consumer | 提供公共能力执行上下文和结果契约 | 定义 Knowledge 单动作流程、配置、阶段端口及领域模型 task definition 装配 | `CapabilityExecutionContext.original_question`、`CapabilityResult` | Knowledge 请求级状态 | 只读；summary task 不进入 Core registry/JSON |
| `L2_02_00` v0.7 Approved | peer/consumer | 提供公共能力契约和 JWT wrapper | 定义业务查询公共约束、配置、出域及 business wire 私有精确十进制；不得把其类型写回 Core JSON | `OpaqueUserToken`、`CapabilityResult`、safe payload；`ExactDecimal` 不跨入本文契约 | 业务查询公共状态 | 只读；Core JSON 边界定向检查通过 |
| 当前仓库代码 | implementation_baseline | 提供已实现 Core、Access、Knowledge/Business、模型 stub/隔离 transport 与本地 Resolver 的核实依据 | 各工作包按自身门禁继续演进 | 已冻结公共契约和测试证据 | 项目维护者 | 本轮只读 |
...
| 事实编号 | 当前事实 | 证据 | 设计含义 |
|---|---|---|---|
| `FACT-CORE-001` | 2026-07-25 立项时工作区不存在目标 Agent Python 工程 | 当日路径核实；仅作历史立项证据 | 解释“建议新增”审计标签来源，不代表当前状态 |
| `FACT-CORE-002` | 当前 `agent-runtime` 已包含 Core、Access、Knowledge/Business、模型本地/隔离 transport、两个业务 Resolver、v4 catalog/selector、candidate-02证据及默认stub下的受控Runtime装配 | 2026-08-12 源码、测试、工作包与Runtime代码对照设计复核 | 既有Core/Resolver未修改；DeepSeek transport、ID-only selector、answer generator与`ModelContext`通过既有Provider-neutral接缝装配，目标环境未启用 |
...
|---|---|---|---|---|---|
| `CapabilityDescriptor` | 已存在 | 描述稳定能力 ID、模型可见最小语义和参数 Schema | 处理器、URL、领域配置、供应商字段 | 公共能力描述变化 | 代码绑定不可变描述 |
| `CapabilityArgumentValidator[TInput]` | 已存在 | 将候选 JSON 校验并解析为能力代码定义的不可变强类型输入 | 动作选择、下游调用 | 领域输入约束变化 | 原始参数→`TInput` 或失败 |
| `CapabilityHandler[TInput]` | 已存在 | 异步执行一个已提交查询动作 | 图推进、第二动作、核心注册 | 能力业务语义变化 | `TInput`+上下文→公共结果 |
| `CapabilityRegistrationCandidate[TInput]` | 已存在 | 将描述、启用状态、同类型校验器和处理器代码绑定 | 动态实例化、配置脚本 | 启动装配变化 | 单个注册候选 |
| `CapabilityRegistryBuilder` | 已存在 | 收集候选、执行全量校验、生成冻结快照 | 请求期查找、处理器调用 | 注册规则变化 | 候选集合→冻结注册表 |
| `FrozenCapabilityRegistry` | 已存在 | 提供有序描述快照和按 ID 只读查找 | 运行期写入、热更新 | 可执行集合变化仅发生在重启 | ID→已注册能力 |
| `RequestExecutionScope` | 已存在 | 保存受控执行上下文和一次性动作 latch | 图阶段状态、领域状态 | 单请求创建/释放 | 上下文+claim/finish |
| `CapabilityExecutionCore` | 已存在 | 按固定顺序校验、claim、调用并约束结果 | 推理、领域策略、语义重试 | 公共执行规则变化 | 候选+scope→公共结果 |
| `AgentInputState`/`AgentRequestState`/`AgentOutputState` | 已存在 | 定义 LangGraph 输入、内部和输出字段 | 身份上下文、持久化、领域流水线状态 | 图公共状态变化 | 请求状态 |
| `GraphRunContext` | 已存在 | 通过 LangGraph `context_schema` 向唯一能力执行节点提供请求执行 scope | 图业务字段、模型输入、持久状态 | 运行依赖变化 | 调用期上下文 |
| `ActionSelectionNode`/`AnswerGenerationNode` | 已存在 | 定义核心图面对模型 L2 的窄异步 Protocol，只返回候选/答案/有限失败决定 | graph state update、最终 status、capability ID、用户结果、Provider DTO、Prompt、模型重试和领域出域策略 | 图与模型 L2 边界变化 | 窄输入→窄决定 |
| `select_action_node`/`generate_answer_node` | 已存在 | 从共享 state 确定性投影窄输入、调用注入 Protocol，并由 wrapper 独占 graph update 与最终状态映射 | 供应商调用、核心执行、领域结果解释 | 模型节点输入投影或确定性映射变化 | state→窄输入→窄决定→state update |
| `execute_capability_node` | 已存在 | 从图状态调用核心并写入唯一能力结果 | 选择动作、回答生成 | 图与核心协作变化 | state→state update |
| `route_after_capability`/`finalize_without_model` | 已存在 | 按公共 result/egress 确定路由并生成固定非模型终态 | 领域出域判定、模型生成和 HTTP 映射 | 公共结果分支变化 | state→路由/终态 update |
| `CoreRuntimeSettings` | 已存在 | 提供核心资源边界和启动校验 | 域内配置、服务地址、模型配置 | 本地容量边界变化 | 强类型不可变设置 |
| `RuntimeCompositionRoot` | 已存在 | 显式装配设置、注册候选、注册表、核心和图 | 请求期调用、动态发现 | 具体实现集合变化 | 依赖→运行时对象图 |
| `AgentRuntimeInvoker` | 已存在 | 校验 Python 入口问题和上下文，并通过 `context=` 调用已编译图 | Spring 协议、模型输入最小化、图内编排 | 运行入口契约变化 | question+scope→`AgentSemanticOutcome` |
...
| `DR-CORE-015` | `LocalActionResolver` 是同步、纯、Provider-neutral 的动作归属域接缝；只返回 `no_match/candidate/invalid`，candidate 仅含本 Resolver 的 `JsonObject arguments`，不得携带其他 capability ID | 能力 API、域提供方 | 动作解析 | 域语法不泄漏到核心，Resolver 不能跨动作改写目标 |
| `DR-CORE-016` | 混合裁决按 canonical capability ID 顺序调用本地 Resolver：正常返回时收集全部结果；首个协议违例/异常立即以内部失败终止且不再调用后续 Resolver。聚合后按“任一 `ambiguous_intent` 或 candidate≥2 → ambiguous；否则任一 invalid → invalid；否则 candidate=1 → 本地候选；否则调用模型”的唯一优先级裁决 | LangGraph 混合节点 | 每次动作选择 | 唯一候选、确定性错误优先级、歧义失败关闭、业务问题不回退模型猜参 |
| `DR-CORE-017` | 模型决定只含 canonical capability ID；空参数 Schema 动作由混合节点绑定 `{}`；非空 Schema 动作必须已有本地 Resolver，若模型选中则返回 `invalid_argument/core.local_arguments_required`，不得执行；最终候选继续由 Core/注册项 validator 校验 | 混合节点、组合根 | 模型决定/启动 | 模型无法生成业务参数，既有 Core 执行边界保持不变 |
| `DR-CORE-018` | 组合根校验本地 Resolver ID 全局唯一、对应启用 descriptor，且每个非空执行 `argument_schema` 的启用能力恰有一个 Resolver；Resolver 不得由请求、模型、环境类名或运行期注册决定 | 组合根 | 启动 | 配置缺失或错配阻止就绪，运行期集合冻结 |
...
禁止 `$ref`、`oneOf`、`anyOf`、`allOf`、可执行扩展、默认表达式和供应商字段。Schema 只用于本地候选/执行参数的供应商无关边界、空参数动作识别和最终校验追踪，不直接或摘要投影给模型；`L2_00_02` v4 只投影 descriptor 的 canonical ID、display name、description、aliases，并只返回 Provider-neutral capability ID 决定。真正的运行参数由同一注册候选绑定的 `CapabilityArgumentValidator` 再次确定性校验，执行 Schema 与校验器的一致性由各能力契约测试负责。
...
`ActionCandidate` 是混合动作解析节点形成的供应商无关最终输出；可以源自本地 Resolver 参数或模型能力 ID + 空对象绑定。它不包含 URL、HTTP 方法、类名、方法名、物理索引、SQL、ES DSL、角色、令牌或下游服务选择。v0.6 不修改该类型及 `CapabilityExecutionCore.execute` 的签名。

### 8.6.1 本地 Resolver 与模型能力选择契约

| 类型 | 字段/方法 | 固定语义 |
|---|---|---|
| `LocalActionInvalidReason` | `missing_required/duplicate_argument/conflicting_argument/unsupported_clause/malformed_value/ambiguous_intent` | 有限枚举；不得包含用户问题、字段值或自由文本 |
| `LocalActionResolution` | `kind: no_match/candidate/invalid`；candidate 分支仅 `arguments: JsonObject`；invalid 分支仅 `reason` | 三分支互斥；candidate 不携带 capability ID；no_match 不携带参数/原因 |
| `LocalActionResolver` | `capability_id: str`；`resolve(question: str) -> LocalActionResolution` | 同步纯函数；不得 I/O、调用模型/下游、读取 JWT/角色、写共享状态或记录 question/arguments |
| `CapabilitySelectionDecision` | `kind: candidate/unsupported/failure`；candidate 分支仅 `capability_id`；failure 分支仅有限 `ModelNodeFailure` | 模型层不得返回 `arguments`、state update、最终 outcome 或供应商对象 |
| `ActionSelectionDecision` | `kind: candidate/unsupported/invalid_argument/failure`；candidate 分支仅最终 `ActionCandidate`；invalid 分支仅有限 `invalid_code` | 混合节点对图 wrapper 的唯一决定；`invalid_code` 仅为 `core.local_action_invalid/core.local_action_ambiguous/core.local_arguments_required` |

执行 `argument_schema` 的“空参数”只认精确 canonical 结构 `{"type":"object","properties":{},"required":[],"additionalProperties":false}`；不得以缺字段、等价宽 Schema 或 validator 恰好接受空对象替代。模型端口只接收 `L2_00_02` 定义的安全 capability catalog 并返回 ID-only 决定，既不接收执行 Schema，也不改变 descriptor 的执行 Schema。

本地裁决的有限映射必须唯一：正常返回的 Resolver 结果全部收集后，只要存在 `invalid(ambiguous_intent)` 或 candidate 数量≥2，即使同时存在其他 invalid，也优先使用 `core.local_action_ambiguous`；否则只要存在任一其他合法 `invalid(reason)`，使用 `core.local_action_invalid`；仅在不存在 invalid 且 candidate 恰为 1 时构造本地候选；不存在 invalid/candidate 才调用模型。模型选择非空执行 Schema 动作使用 `core.local_arguments_required`。Resolver 抛出异常、返回不满足三分支不变量的对象、candidate 参数违反公共 JSON 边界或其 `capability_id` 与冻结绑定不一致，均不是用户参数错误或模型失败：`HybridActionSelectionNode` 按 canonical 顺序在首个此类违例处停止，丢弃原异常/载荷并抛出无正文的 typed `InvalidActionResolution`，由 `select_action_node` 固定映射为 `internal_failure/core.invalid_action_resolution`。该异常不得成为 `ModelNodeFailure`，也不得触发后续 Resolver、模型、validator、handler 或下游调用。
...
- DeepSeek v4 只可由 `L2_00_02` 从 `CapabilityDescriptor` 的模型安全展示元数据形成有界目录，并返回 Provider-neutral canonical ID；不得投影执行 Schema、生成执行参数，供应商 JSON 字段也不得回写公共描述。
...
| 本地解析识别后非法 | Resolver 返回除 `ambiguous_intent` 外的有限 invalid reason | `core.local_action_invalid` | `invalid_argument` | 0 | 本图不重试 | 仅记录 reason 枚举 |
| 本地解析歧义 | Resolver 返回 `ambiguous_intent` 或两个及以上 candidate | `core.local_action_ambiguous` | `invalid_argument` | 0 | 本图不重试 | 仅记录命中数，不记录问题/参数 |
| 本地解析协议违例 | Resolver 抛异常、返回非法分支/参数或绑定不一致 | `core.invalid_action_resolution` | `internal_failure` | 0 | 本图不重试 | 仅记录稳定规则码和安全 capability ID，不记录异常类型、正文或参数 |
| 非空参数动作缺少本地候选 | 本地零 candidate 后模型选中非空执行 Schema 动作 | `core.local_arguments_required` | `invalid_argument` | 0 | 本图不重试 | 仅记录安全 capability ID |
...
| `CapabilitySelectionInput` | `question`、`descriptors` | 混合动作解析节点确定性投影 | 只交模型能力选择端口；不含 Resolver、Runtime、scope、执行参数、domain result 或整个 state |
| `CapabilitySelectionDecision` | `kind`、可选 `capability_id/failure` | `CapabilitySelectionNode` | 只能表达 canonical ID、无支持动作或有限模型失败；不能包含 arguments、graph update、最终 outcome或供应商对象 |
| `ActionSelectionDecision` | `kind`、可选 `candidate/invalid_code/failure` | 混合 `ActionSelectionNode` | 只能表达最终候选、无支持动作、有限参数失败或模型失败；Resolver 协议违例不伪装为决定，而以 typed `InvalidActionResolution` 交 wrapper 映射；不能包含 graph update、最终 outcome 或供应商对象 |
...
2. `select_action_node` 和 `generate_answer_node` 是核心图 wrapper；前者先对冻结 descriptor tuple 执行空集合确定性短路，非空时调用组合根构造的混合 `ActionSelectionNode`。混合节点严格执行 8.6.1/`DR-CORE-016/017`，只有零本地匹配才构造 `CapabilitySelectionInput` 并调用注入的模型 `CapabilitySelectionNode`。wrapper 仍是 `action_candidate/final_outcome` 唯一写入者；具体模型端口归 `L2_00_02`。
...
以下 v0.5 路径已在 `WP-CORE-01` 中创建，v0.6 路径已在 `WP-ACTION-RESOLUTION-01` 中创建或修改；存在状态不授权相邻 HTTP、领域或模型切片。

| 实现编号 | 状态 | 类型 | 路径 | 符号/配置项 | 责任 | 必要性 | 设计规则 |
|---|---|---|---|---|---|---|---|
| `IMPL-CORE-001` | 已存在 | Python 构建 | `agent-runtime/pyproject.toml` | Python `>=3.12,<3.13`、`langgraph==1.2.9`、test extra | 建立最小 Python 工程和锁定依赖 | 当前 Core 工程基线 | `DR-CORE-008/013` |
| `IMPL-CORE-002` | 已存在 | Python 契约 | `agent-runtime/src/agent_runtime/capability_api/contracts.py` | descriptor、含 `original_question` 的 context、candidate、result、validator/handler/provider Protocol、JSON 冻结与不变量 | 唯一公共能力语义 | 防止三类能力各自定义契约 | `DR-CORE-003/004/005/007/011/012/013/014` |
| `IMPL-CORE-003` | 已存在 | Python 核心 | `agent-runtime/src/agent_runtime/core/registry.py` | `CapabilityRegistryBuilder`、`FrozenCapabilityRegistry`、注册错误 | 启动校验、冻结、快照和只读查找 | 有限发现与一致执行集合 | `DR-CORE-003/006/007/010/011/013` |
| `IMPL-CORE-004` | 已存在 | Python 核心 | `agent-runtime/src/agent_runtime/core/execution.py` | `RequestExecutionScope`、`ActionExecutionLatch`、`CapabilityExecutionCore` | 固定校验、单动作 claim、deadline、调用和结果约束 | 核心确定性边界 | `DR-CORE-001/002/004/005/009/011/012` |
| `IMPL-CORE-005` | 已存在 | LangGraph Schema | `agent-runtime/src/agent_runtime/graph/state.py` | input/internal/output/update TypedDict、模型节点窄输入、`GraphRunContext`、`AgentSemanticOutcome` | 请求级唯一 state、节点最小输入和运行上下文 Schema | 防内部身份及领域结果进入错误 state/output/模型节点 | `DR-CORE-001/004/005/008/012` |
| `IMPL-CORE-006` | 已存在 | LangGraph 节点 | `agent-runtime/src/agent_runtime/graph/nodes.py` | action/answer Protocol 和投影 wrapper、`execute_capability_node`、公共路由、固定终态 | 图与核心及相邻模型 L2 的最小协作 | 防图直接调用处理器/注册表，防模型节点接收整个 state | `DR-CORE-001/002/005/008/009/012` |
| `IMPL-CORE-007` | 已存在、v0.6 已修改 | Python 装配 | `agent-runtime/src/agent_runtime/bootstrap.py` | `RuntimeCompositionRoot`、显式 provider/Resolver 列表、无 checkpointer/store 图编译 | 唯一具体装配点 | 新能力不侵入核心 | `DR-CORE-006/007/008/010/013/018` |
| `IMPL-CORE-010` | 已存在 | Python 公共契约 | `agent-runtime/src/agent_runtime/capability_api/action_resolution.py` | `LocalActionInvalidReason`、`LocalActionResolutionKind`、`LocalActionResolution`、`LocalActionResolver` | 域 Resolver 的 Provider-neutral 窄接缝 | 不包含具体业务语法 | `DR-CORE-015` |
| `IMPL-CORE-011` | 已存在 | Python 图契约 | `agent-runtime/src/agent_runtime/graph/action_resolution.py` | `CapabilitySelectionInput/Decision`、`CapabilitySelectionNode`、`InvalidActionResolution`、`HybridActionSelectionNode` | 本地/模型裁决、协议违例隔离与最终候选构造 | 不执行 capability | `DR-CORE-016/017` |
| `IMPL-CORE-012` | 已修改 | Python 图节点 | `agent-runtime/src/agent_runtime/graph/nodes.py` | 扩展 `ActionSelectionDecisionKind` 与固定 invalid 映射 | wrapper 单写 state | 不解析域语法 | `DR-CORE-001/016/017` |
| `IMPL-CORE-013` | 已修改 | Python 装配 | `agent-runtime/src/agent_runtime/bootstrap.py` | Resolver 唯一性、descriptor 对齐、非空 Schema 覆盖校验与混合节点装配 | 启动失败关闭 | 不动态发现 Resolver | `DR-CORE-018` |
| `IMPL-CORE-008` | 已存在 | Python 配置 | `agent-runtime/src/agent_runtime/settings.py` | `CoreRuntimeSettings` | 核心限制强类型绑定、范围校验和冻结 | 有界执行与失败关闭 | `DR-CORE-006/009/011/013` |
| `IMPL-CORE-009` | 已存在 | Python 运行入口 | `agent-runtime/src/agent_runtime/runtime.py` | `AgentRuntimeInvoker` | 防御性校验 question/context 及原始问题同源，以 `context=` 调用已编译图并返回语义结果 | 防敏感 scope 进入 state，阻止问题漂移或越界问题到达模型 | `DR-CORE-004/008/011/012/014` |
...
以下 Python 路径和签名已按本文创建；v0.5 仅确认边界关键实现与测试存在，不把普通私有辅助函数提升为新契约。
...
| `agent-runtime/src/agent_runtime/capability_api/action_resolution.py` | `LocalActionInvalidReason`、`LocalActionResolutionKind`、`LocalActionResolution`、`LocalActionResolver` | 8.6.1 的本地解析窄契约 | keyword-only 冻结 dataclass/有限 `StrEnum`/`Protocol`；不含 capability 目标切换、JWT 或 I/O | 业务域 Resolver、混合节点、组合根 |
| `agent-runtime/src/agent_runtime/graph/action_resolution.py` | `CapabilitySelectionInput`、`CapabilitySelectionDecisionKind`、`CapabilitySelectionDecision`、`CapabilitySelectionNode`、`InvalidActionResolution`、`HybridActionSelectionNode` | 11.1～11.2 的 ID-only 模型选择、确定性裁决和 Core 协议违例 | 冻结 dataclass/有限 `StrEnum`/窄 `Protocol`；typed 异常无用户正文/参数；模型决定无 arguments；混合节点无 I/O，除零匹配时调用选择端口 | 核心图 wrapper、L2_00_02 模型实现、本地 stub |
| `agent-runtime/src/agent_runtime/graph/nodes.py` | `ActionSelectionInput`、`AnswerGenerationInput`；`ModelNodeFailureKind`、`ModelNodeFailure`、`ActionSelectionDecisionKind`、`ActionSelectionDecision`、`AnswerGenerationDecisionKind`、`AnswerGenerationDecision` | 11.1～11.2 的图节点最小输入、窄决定和固定失败映射 | keyword-only 冻结 dataclass/有限 `StrEnum`；不是共享 graph state；决定对象不能携带最终 outcome、领域结果或供应商对象 | 核心图 wrapper、混合节点和回答节点 |
...
| `InvalidActionResolution` | typed exception；公开属性仅 `code: Literal["core.invalid_action_resolution"]`；不得保存 cause、message、question、arguments、Resolver repr 或模型对象；抛出时使用 `from None` |
| `CapabilitySelectionDecision` | `kind: CapabilitySelectionDecisionKind`，枚举固定为 `candidate/unsupported/failure`；candidate 分支仅 `capability_id: str`；failure 分支仅 `failure: ModelNodeFailure` |
| `ActionSelectionDecision` | `kind: ActionSelectionDecisionKind`，枚举固定为 `candidate/unsupported/invalid_argument/failure`；candidate 分支仅 `candidate: ActionCandidate`；invalid 分支仅 `invalid_code: ActionSelectionInvalidCode`；failure 分支仅 `failure: ModelNodeFailure` |
...
> 下表“建议新增”前缀保留 v0.4 Approved 时的设计标签，用于审计原批准签名；v0.5 当前存在性以 15.1 `IMPL-CORE-001～009` 的“已存在”和 16.3 验证证据为准。
...
| 建议新增：`agent-runtime/src/agent_runtime/capability_api/action_resolution.py` `LocalActionResolver` | `@property def capability_id(self) -> str`；`def resolve(self, question: str) -> LocalActionResolution` | question 已由 Runtime 校验；同步、确定性；返回值按 8.6.1 校验 | 只返回本动作 arguments/no_match/有限 invalid；异常由混合节点转 `internal_failure/core.invalid_action_resolution` | 无 I/O、日志、JWT/角色读取或共享状态 | `HybridActionSelectionNode`；`TEST-CORE-011/012` |
| 建议新增：`agent-runtime/src/agent_runtime/graph/action_resolution.py` `CapabilitySelectionNode.__call__` | `async def __call__(self, input: CapabilitySelectionInput) -> CapabilitySelectionDecision` | 只接收有界 question 和冻结 descriptors；不得接收 Resolver/Runtime/arguments/整个 state | 只返回 canonical capability ID、unsupported 或有限 failure；DeepSeek 映射归 L2_00_02 | 可调用模型；不写 graph state、不执行能力 | `HybridActionSelectionNode`；`TEST-CORE-011/013` |
| 建议新增：同上 `HybridActionSelectionNode.__call__` | `async def __call__(self, input: ActionSelectionInput) -> ActionSelectionDecision` | 构造时接收冻结 descriptors、按 ID 排序的 resolver tuple 和 `CapabilitySelectionNode`；按 `DR-CORE-016/017` 裁决 | 返回最终 candidate/unsupported/invalid/failure；Resolver 协议违例或异常抛无正文 `InvalidActionResolution`，由 wrapper 映射；非法模型决定仍按模型失败契约处理 | 仅在零本地匹配时调用模型选择端口；不执行能力、不记录正文 | `select_action_node`；`TEST-CORE-011～014` |
...
| 建议修改：`agent-runtime/src/agent_runtime/bootstrap.py` `RuntimeCompositionRoot.build` | `@staticmethod def build(*, settings: CoreRuntimeSettings, providers: Sequence[CapabilityRegistrationProvider], capability_selector: CapabilitySelectionNode, answer_generator: AnswerGenerationNode, local_action_resolvers: Sequence[LocalActionResolver] = ()) -> AgentRuntimeInvoker` | 仅启动期；先冻结 registry，再校验 resolver ID 唯一/启用 descriptor 对齐/每个非空执行 Schema 恰一 Resolver，最后构造混合节点 | 返回完整运行入口；错配抛稳定启动错误并阻止就绪；不转换为请求结果 | 创建对象图和冻结快照；不进入请求调用链，不动态发现 Resolver | Python 进程启动入口；`TEST-CORE-002/005/007/009/013/014`、`VAL-CORE-003/006/007` |
...
1. 表中 `RuntimeCompositionRoot.build`、`CapabilitySelectionNode`、混合 `ActionSelectionNode` 和 `AnswerGenerationNode` 只固定核心图面对相邻 L2 的窄接口；DeepSeek Provider DTO、Prompt 和外部错误仍由 `L2_00_02` 拥有。
...
1. 创建 `agent-runtime/pyproject.toml` 和空包结构。
...
该顺序记录已关闭 `CR-GATE-002` 时采用的 Core 实施次序，不是当前待执行清单，也不构成 v4 模型代码实施授权。
...
| `TEST-CORE-011` | 建议新增 | `DR-CORE-015/016` | Unit | `agent-runtime/tests/unit/graph/test_hybrid_action_resolution.py` | 注入 no_match/candidate/各 invalid reason/非法对象/公共 JSON 越界/抛异常 Resolver 与模型 spy，覆盖 0/1/2 candidate、普通 invalid+多 candidate、普通 invalid+ambiguous、canonical 顺序及首个协议违例后的剩余 Resolver 零调用 | 验证混合裁决、精确错误优先级与零调用 | 一 candidate→最终候选且模型 0；普通 invalid→`local_action_invalid`；ambiguous/multi（即使并存普通 invalid）→`local_action_ambiguous`；协议违例→`internal_failure/core.invalid_action_resolution` 且后续 Resolver 0；零匹配→模型恰 1；异常无正文泄露 | 回退模型猜参、把 Core 缺陷伪装成模型/用户失败、顺序或错误码不稳定、多个候选被任选 |
| `TEST-CORE-012` | 建议新增 | `DR-CORE-015` | Contract/Architecture | `agent-runtime/tests/contract/test_local_action_resolver_contract.py` | 构造合法/非法三分支和伪 Resolver；AST/spy 检查 I/O、日志、JWT/角色访问 | 验证 Resolver 窄契约与纯函数边界 | candidate 无 capability ID；invalid reason 有限；禁止依赖/副作用不存在 | Resolver 跨动作选 ID、访问外部依赖或记录正文 |
| `TEST-CORE-013` | 建议新增 | `DR-CORE-017` | Unit/Contract | `agent-runtime/tests/contract/model/test_capability_selection.py` | 模型 stub 返回合法/alias/未知/非空参数动作 ID、夹带 arguments/非法决定 | 验证 ID-only 选择和空参数绑定 | 合法空 Schema→`{}`；alias/未知/夹带参数拒绝；非空 Schema→`core.local_arguments_required`且 handler 0 | 模型参数进入候选或非空动作被执行 |
| `TEST-CORE-014` | 建议新增 | `DR-CORE-018` | Unit/Architecture | `agent-runtime/tests/unit/test_action_resolution_composition.py` | 组合根注入重复、未知、disabled、缺失/多 Resolver 及合法集合 | 验证启动一致性和显式装配 | 任一错配阻止就绪；空 Schema 可无 Resolver；非空 Schema 恰一；无扫描/动态类名 | 运行期才发现缺口或 Resolver 动态加载 |
...
| 本地业务动作 | 一个 Resolver 返回 candidate，其他返回 no_match | 调用混合节点 | 按 canonical ID 调用；构造对应最终候选；模型选择端口 0 次；候选仍进入 Core/validator |
| 本地歧义/非法 | 两个 Resolver 返回 candidate，任一 Resolver 返回 invalid，或两者并存 | 调用混合节点 | 按 8.6.1 固定优先级返回 `local_action_ambiguous` 或 `local_action_invalid`；模型、validator、handler 均 0 次；日志无 question/arguments |
| 模型选择空参数动作 | 所有 Resolver no_match，模型返回 `knowledge.query` | 调用混合节点 | 仅为精确空执行 Schema 绑定 `{}`；最终候选进入 Core |
| 模型选择非空参数动作 | 所有 Resolver no_match，模型返回 Employee/Transaction ID | 调用混合节点 | 返回 `invalid_argument/core.local_arguments_required`；不生成参数、不执行 handler |

### 16.3 验证定义

| 验证编号 | 工作目录/前置 | 命令或人工步骤 | 验证范围与充分性 | 预期结果 | 更广回归 | 当前执行状态 |
|---|---|---|---|---|---|---|
| `VAL-CORE-001` | `D:\codex`；目标文档和 validator 可读 | `python C:\Users\zhoud\.agents\skills\detailed-design-document\scripts\validate_detailed_design.py --file D:\codex\docs\design\L2_00_01_SINGLE_AGENT_CORE_EXECUTION_CAPABILITY_REGISTRATION_DETAILED_DESIGN.md --root D:\codex --strict` | 校验本文结构、逐行追踪、状态、引用和质量信号；只能证明确定性文档规则，不替代语义/独立评审 | 0 errors、0 warnings | 不需要其他文档 validator；关联文档保持只读 | 已执行：v0.5 0 errors、0 warnings（2026-08-01） |
| `VAL-CORE-002` | `D:\codex\agent-runtime`；源码、test extra 和本地 stub 已创建 | `python -m pytest tests/unit -q` | 覆盖公共契约、注册、执行、并发、限制和日志；足以定位单模块规则，但不证明包依赖和图协作 | 全部通过 | 共享能力 API 同时执行 `VAL-CORE-003/004` | 已执行：44 passed（隔离 Python 3.12 环境，2026-08-01） |
| `VAL-CORE-003` | `D:\codex\agent-runtime`；contract/architecture/integration tests 和模型无关 stubs 已创建 | `python -m pytest tests/contract tests/architecture tests/integration -q` | 覆盖双处理器形态、边界签名、扩展、依赖、state/context 隔离和模型无关图；不证明真实模型或业务集成 | 全部通过 | 真实模型/数据另受 `SA-GATE-002/006`、`CR-GATE-003` | 已执行：17 passed（2026-08-01） |
| `VAL-CORE-004` | `D:\codex\agent-runtime`；源码、测试和锁定开发依赖已创建 | 依次执行 `python -m compileall -q src tests`、`python -m mypy --strict src tests`、`python -m pip check` | 证明语法、本文签名/泛型绑定和依赖一致；不证明运行行为 | 三条命令均无错误 | 与 `VAL-CORE-002/003` 联合才构成核心实现验证 | 已执行：compileall 0、mypy 24 files 无问题、隔离环境 pip check 无破损（2026-08-01） |
| `VAL-CORE-005` | `D:\codex`；本文、实现、测试、依赖报告和门禁材料可用 | 按 `code-review-against-docs` review-and-fix 两轮核对依赖图、签名、注册快照、取消竞态、OpenAPI 隔离和非范围 | 证明实现服从本文且实施授权未被本地测试扩大 | Core 不理解 Knowledge、不扩大模型出域、不破坏单动作/状态边界、不接 HTTP/领域/外部调用 | 后续语义变化需重新评审；相邻切片仍受单独门禁 | 已执行：两轮发现已修复，复核无未关闭 S0/S1/S2（2026-08-01） |
| `VAL-CORE-006` | `D:\codex\agent-runtime`；`IMPL-CORE-010～013` 实施后 | `python -m pytest tests/unit/graph/test_hybrid_action_resolution.py tests/contract/test_local_action_resolver_contract.py tests/contract/model/test_capability_selection.py tests/unit/test_action_resolution_composition.py -q` | 直接证明 Resolver 契约、混合裁决、ID-only 决定、启动一致性和模型零调用；不证明具体域语法或真实模型 | 全部通过 | 与 `VAL-CORE-002～004/007` 联合 | 已执行：36 passed（2026-08-07） |
| `VAL-CORE-007` | `D:\codex\agent-runtime`；v0.6 实施完成 | `python -m pytest tests/unit tests/contract tests/architecture tests/integration -q`，随后 `python -m mypy --strict src tests` | 证明 v0.6 未破坏既有 Core、Access、Knowledge/Business fake 契约及类型边界；不证明真实 DeepSeek | 全部通过 | DeepSeek v4 Provider 编码另由 `L2_00_02` 验证；本行不因其变化失效 | 已执行并通过：pytest 376 passed/4 skipped；完整 `src tests` strict mypy 229 files 无问题；compileall 通过（2026-08-07） |
...
|---|---|---|---|---|---|---|
| `RISK-CORE-001` | 契约漂移 | descriptor 执行 Schema、Resolver 输出与 argument validator 不一致 | Resolver 产生 validator 拒绝的字段，或 Schema 缺少实际限制 | 误拒绝或参数扩权 | 每个非空动作提供 Resolver→公共 JSON→validator 接受/拒绝样例；模型只见安全 descriptor 目录且执行 Schema 零可达 | 不阻塞本文；阻塞对应 Resolver 实施 |
| `RISK-CORE-002` | 敏感数据 | JWT 或结果对象被 LangGraph/日志自动序列化 | scope 误入 state、模型节点读取 Runtime 或对象 repr 被记录 | 身份/数据泄露 | `context_schema` 隔离、Runtime 访问架构测试、token wrapper、日志捕获断言 | Core 基线已验证；后续改动必须回归，不阻塞 v4 文档评审 |
| `RISK-CORE-003` | 并发 | 图误配置并发边或重复节点调用 | 同一 scope 并发提交 | 重复下游查询 | 原子 latch 和并发测试 | Core 基线已验证；后续图改动重新评审 |
| `RISK-CORE-004` | 结果污染 | handler 返回可变、超界或自定义对象 | 处理器复用业务/ORM模型 | 序列化失败、泄露或迟后修改 | JSON 白名单、深冻结、字节/深度限制 | Core 基线已验证；新增能力继续执行契约测试 |
...
| `RISK-CORE-008` | 外部契约 | Spring→Python 传输和截止时间换算后续发生漂移 | 重新部署或修改双进程入口 | context 构造不一致 | 本文只固定 Python 消费语义；以 L2_00_00 v0.5 和既有 E2E 契约回归为准 | 既有限定联调已验证；后续协议变更需重新评审 |
| `RISK-CORE-009` | 模型协作 | 动作候选和最终回答节点由 L2_00_02 定义 | 接入真实 DeepSeek | Schema 映射或结果路由不一致 | 使用本地替身验证核心；wiring 入口受 P3 `GATE-020`，完成/启用受 `SA-GATE-002`，敏感问题受 `CR-GATE-003` | 不阻塞模型无关核心设计；阻塞真实模型 |
| `RISK-CORE-010` | 原始问题漂移 | 图输入与处理器上下文由不同值构造，或能力信任模型回填问题 | 后续 Runtime/Knowledge 装配错误 | 改写失去原问题权威、检索与回答无法追踪 | 单源构造、精确相等闸门和零调用负向测试已在 Core 验证；后续装配继续回归 | Core 切片已验证；相邻装配仍受各自门禁 |
| `RISK-CORE-011` | 解析覆盖 | 本地 Resolver 规则重叠或无法识别合法业务表述 | 多候选或 no_match 后模型选择非空动作 | 请求失败关闭 | 域 L2 固化有限语法；冲突/歧义测试；不得以模型猜参或静默忽略字段降级 | 对应业务 Resolver 完成声明的必要前置；不重开 Provider 门禁 |
...
| `CR-GATE-002` | `slice_implementation` | `agent-runtime` 模型无关的核心执行、能力 API、注册、图 wrapper 与本地替身测试切片 | 开始该目标生产代码和测试实施 | v0.4 针对性复评通过且本文可实施；项目维护者明确授权；`VAL-CORE-002/003/004` 和实现代码对照设计评审通过 | 用户 2026-08-01 授权、本文复评/追踪、`agent-runtime` 测试与评审证据 | 项目维护者 | P3 核心实现前 | 已核对明确授权、限定范围、61 项测试、严格类型/依赖验证和两轮代码评审 | Closed | 若重新打开，禁止扩大或替换 Core；不影响只读核实 | 已完成的本地 Core 与 stub 基线 |
| `SA-GATE-002` | `slice_implementation_completion` | DeepSeek 模型切片 | 声明真实模型 Runtime 切片完成或在目标环境显式启用 DeepSeek | `IMPL-CORE-010～013` 与两个域 Resolver 已验证；candidate-02 30/30与answer 6/6成立；P3 `GATE-020`关闭并完成默认stub、fake-only受控wiring，预算/secret/组合根/生命周期通过 | L2_00_02/模型提供方/业务 L2 | 项目维护者/DeepSeek | 模型切片完成声明或目标环境 deepseek 前 | Provider契约测试、Resolver零调用与wiring回归 | Closed（2026-08-12；仅Runtime实现切片） | 不改变Core/Resolver；目标环境启用及真实数据出域仍需独立授权和`CR-GATE-003/SA-GATE-006` | 本地Resolver、默认stub、显式provider装配代码 |
| `CR-GATE-003` | `integration` | 用户问题模型输入 | 外发可能敏感的原始/改写问题 | 输入分类、最小化、Knowledge/Business fixtures、拒绝/unknown 零调用测试完成 | L2_00_02、2026-08-12 非 live 安全回归 | 项目维护者/模型提供方 | 首次敏感问题联调前 | 模型 spy/负向测试与全量非 live 回归 | Closed（2026-08-12；仅问题输入安全前置） | 允许通过 Guard 的最小非敏感问题进入后继受控模型工作包；具体敏感/unknown 问题仍零调用，本地 Resolver 路径继续不消费模型 | 本地 Resolver/选择替身 |
...
`CR-GATE-002` 的历史关闭只覆盖 v0.5 模型无关 Core；v0.6 的 `IMPL-CORE-010～013` 已由 `WP-ACTION-RESOLUTION-01` 单独授权、实现并通过完整 `VAL-CORE-007`，工作包已 Done。`CR-GATE-003` 已按问题输入安全前置关闭，但具体敏感/unknown 问题仍失败关闭；本地业务 Resolver 路径继续不消费模型。

### 18.3 需要后续授权的动作

- Core 后续语义或契约修改须重新取得范围授权并复跑 `VAL-CORE-002～005`；本次关闭不持续授权未来修改。
...
可作为后续实施申请的输入；这是 v0.4 当时结论。v0.5 的授权、实现和门禁状态以 16.3、18.2 与 21 章为准。
...
定向检查未发现新的 S0/S1/S2。`L2_02_00` v0.4 没有扩大本文 8.4 的 Core `JsonObject`，也没有改变能力 API、执行语义或单动作边界；这是 v0.4 实施前检查结论，v0.5 已在不改变该结论的前提下关闭 Core `CR-GATE-002`。
...
定向检查未发现新的 S0/S1/S2。`L2_01_00` v0.3 只在顶层组合根补齐领域 model task definitions 的显式装配，没有扩大本文能力 API、Core JSON、状态、注册快照或执行语义；这是 v0.4 实施前检查结论，v0.5 已在不改变该结论的前提下关闭 Core `CR-GATE-002`。

### 19.6 v0.6 混合动作解析评审批次

| 阶段 | 审计 ID | 本文重点 | 结论 |
|---|---|---|---|
| 三轮作者内审 | `AR-HYBRID-01～03` | Resolver/Core/模型责任分离；公共类型、边界签名和失败矩阵；版本、门禁与追踪闭合 | 修复后无遗留 Blocker/Major，严格校验通过 |
| 五轮独立评审 | `FR-HYBRID-01～05` | canonical 顺序、协议违例 fail-fast、歧义/非法优先级、空执行 Schema、本地候选二次校验、组合根与跨层兼容 | 新增发现均已关闭，无未关闭 S0/S1/S2；最终 `ActionCandidate`、`CapabilityExecutionCore.execute` 和 Access/OpenAPI 不变 |

逐轮设计发现摘要见 `P3_00` 13.18；`WP-ACTION-RESOLUTION-01` 两轮代码对照设计评审及验证限制见 `P3_00` 13.19。实现不关闭 `SA-GATE-002`。

### 19.7 `WP-ACTION-RESOLUTION-01` 代码对照设计评审

第 1 轮冻结 3 个 medium：动作决定运行时类型校验不足、Resolver 原异常可能留在异常上下文、六类 invalid 与畸形候选负向覆盖不完整；均在授权代码/测试范围内修复。第 2 轮重新核对 `DR-CORE-015～018`、selection-only 接缝、canonical 顺序、失败映射、组合根一致性和禁止范围，无未关闭 blocker/high/medium。评审只证明本工作包代码符合本文；当时保留的完整 tests strict mypy 验证限制已由 19.8 的独立类型门禁修复关闭。

### 19.8 `VAL-CORE-007` 类型门禁修复证据

仅修改 11 个测试文件，处理 AST annotation optional、递归 `JsonObject` 运行时收窄、`Mapping` 环境替身、`RuntimeInvoker` Protocol 参数、Knowledge 泛型显式绑定、helper 返回类型及有意非法输入的显式 cast。未增加宽泛 ignore、未降低断言，生产代码和公共契约 diff 为空。定向 strict mypy 无问题、定向测试 50 passed；完整行为回归 376 passed/4 skipped、完整 strict mypy 229 files 无问题、compileall 与定向 diff check 通过。

### 19.9 v0.8 Provider-neutral ID 边界独立聚焦评审

| 轮次 | 冻结发现 | 修复与复核 | 结论 |
|---:|---|---|---|
| 1 | `REV-V4-001`（S1）：8.8 的现行 Tool Schema 投影与 v4 no-tools/ID-only 冲突；`REV-V4-006`（S2）：立项期版本、事实与实施顺序混入当前基线 | 删除 Tool Schema 投影，改为只消费模型安全目录与 canonical ID；更新直接依赖、当前事实、风险和历史实施顺序 | 待第 2 轮复核 |
| 2 | 重新检查 `CapabilitySelectionDecision`、空执行 Schema、本地参数所有权、失败映射、当前状态及与 `L2_00_02` v0.11 的接缝 | 未发现新的 S0/S1/S2；`REV-V4-001/006` Closed | 聚焦评审通过；不修改 Core/Resolver/测试，不关闭仍缺代码授权的 `GATE-036` |

本轮只确认 Provider-neutral ID 消费边界；DeepSeek wire、catalog 内容校验、历史 evidence provenance 和 PoC 归 `L2_00_02`，其修复不得反向扩大本文公共契约。
...
- [x] Core 落点已区分 v0.5 历史实现与 v0.6 `IMPL-CORE-010～013` 当前实现；业务域 Resolver 已按各自 L2 完成，v3 模型实现仅作为失败历史，v4 非 live 实现只存在于相邻模型切片且未扩散到 Core。
...
- [x] 项目维护者明确关闭 `CR-GATE-002` 并授权 Core 代码实施；限定验证与代码对照设计评审已通过。
- [x] v0.6 混合动作解析完成 `AR-HYBRID-01～03`、`FR-HYBRID-01～05` 及两轮代码对照设计评审，代码发现全部关闭。
- [x] `VAL-CORE-007` 的完整 `python -m mypy --strict src tests` 通过；229 files 无问题，`WP-ACTION-RESOLUTION-01` 已 Done。
- [x] v0.8 与 `L2_00_02` v0.11 的 Provider-neutral ID 边界已完成独立聚焦复评，无未关闭 S0/S1/S2；`L2_00_02` v0.16 已完成candidate-02与Runtime受控装配验证并关闭`GATE-038/020/SA-GATE-002`各自切片，且未修改本文公共契约。

## 21. 当前结论

- 本文版本：v0.11。
- 文档状态：Approved。
- 评审状态：历史评审结论保持有效；v0.8 Provider-neutral ID 边界独立聚焦评审通过，无未关闭 S0/S1/S2。
- 实施状态：`WP-CORE-01`、`WP-ACTION-RESOLUTION-01`、业务域本地 Resolver、相邻 `WP-MODEL-ACTION-V4-LOCAL-01`、`WP-MODEL-ACTION-POC-04`与`WP-MODEL-RUNTIME-01`已Done；v0.6 Provider-neutral混合解析、selection-only接缝及完整`VAL-CORE-007`已验证。DeepSeek v3与v4 candidate-01失败历史保持，candidate-02已30/30通过。
- 生效状态：未生效。
- 是否可作为实现依据：已评审Core范围可以；v0.11仅同步问题输入安全门禁证据，Runtime wiring仍只消费既有Provider-neutral节点并由模型/接入层持有生命周期，不改变本文Core契约。
- `CR-GATE-001/002/003`、`SA-GATE-002` 已按各自范围关闭；`SA-GATE-006` 保持 Open。
- 生效状态仍为未生效：虽已有 Access、领域能力、业务域 Resolver、三条受控真实只读链、本地混合解析及模型Runtime实现证据，但尚无完整系统默认装配、目标部署或生产生效证据。
- 本轮未修改 Core/Resolver/领域参数契约；受控模型装配通过既有节点注入并使用fake transport验证，未读取key、未产生真实outbound、未授权Git commit/push。
