...
| 文档层级 | L2 详细设计 |
| 文档状态 | Approved |
| 评审状态 | 历史 v0.2 五轮独立评审、v0.3 summary task 复评和 v0.5 Retrieval 状态同步保持有效；v0.6 selection-only、v0.7 问题出域安全证据与 v0.8 生产 summary v2 单注册聚焦设计评审保持有效；v0.9 已完成本地实现和代码复核；v0.10同步V2 stability preparation；v0.11仅同步下游Knowledge出域闭环；v0.13 representative v2输入一致性联合聚焦设计评审通过；v0.14完成v2前置、终态最小实现和代码对照设计复核，`REV-KFLOW-023/GATE-046`已关闭 |
| 当前版本 | v0.14 |
| 日期 | 2026-08-13 |
| 适用范围 | Python `agent-runtime` 内唯一 `knowledge.query` 能力的参数契约、请求级流程状态、问题改写、首批逻辑知识域、域选择、检索计划、跨域失败优先级、配置与组合根 |
| 上位文档 | [`REQ_00`](../REQ_00_SINGLE_AGENT_QUERY_REQUIREMENTS.md) v1.3；[`L0_00`](L0_00_SINGLE_AGENT_ARCHITECTURE.md) v0.11；[`L1_01`](L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) v0.6（已评审/已通过，`KQ-GATE-001/CR-GATE-003` 已按各自范围关闭） |
| 直接依赖 | [`L2_00_01`](L2_00_01_SINGLE_AGENT_CORE_EXECUTION_CAPABILITY_REGISTRATION_DETAILED_DESIGN.md) v0.11（Approved）的能力 API、`original_question`、空执行 Schema 绑定与公共结果；[`L2_00_02`](L2_00_02_SINGLE_AGENT_DEEPSEEK_MODEL_ACCESS_CONTROLLED_GENERATION_DETAILED_DESIGN.md) v0.17（Approved）的 selection-only 能力选择、输入闸门、模型上下文与受控结构化任务 |
| 下位/后续契约 | [`L2_01_01`](L2_01_01_SINGLE_AGENT_KNOWLEDGE_RETRIEVAL_LOCAL_MODEL_DETAILED_DESIGN.md) v0.6 Approved；[`L2_01_02`](L2_01_02_SINGLE_AGENT_KNOWLEDGE_EVIDENCE_EGRESS_SUMMARY_EFFECTIVENESS_DETAILED_DESIGN.md) v0.28 Approved |
| 实现基线 | `agent-runtime/src/agent_runtime/knowledge` 已实现Knowledge Capability、配置、目录、问题语义、改写/域选择/计划、Provider与阶段接缝；真实Retrieval已验证。`WP-K-SUMMARY-V2-LOCAL-01`已将生产组合根切换为rewrite v1 + summary v2单注册并通过fake/nonlive与代码对照设计复核；V1/历史/validator不变。下游`WP-K-EGRESS-01`已完成30/30 live及post-consumption闭环。representative v2前置已冻结并通过`VAL-KEV-014`；Capability现会在`question_egress_denied=true`且零域时返回既有策略拒绝，未拒绝零域仍为`no_result`，`GATE-046/VAL-KFLOW-006`已按非live范围关闭 |
| 是否可作为实现依据 | 按范围可用 |
| 当前允许实施范围 | 维护已验证的flow/config、denied/zero-domain终态与生产rewrite v1 + summary v2单注册；准备或执行新的live P5 candidate仍须单独设计、冻结和授权 |
| 当前禁止动作 | 放宽P5 packer、安全Guard或validator；修改公共 Flow/Stage/Core/HTTP Schema、领域契约、representative v1、历史candidate/manifest/evidence；读取`LLM_API_KEY`、真实模型/检索调用、创建新live candidate、Git commit/push |
| 修改权限 | 用户于2026-08-13授权创建representative v2并聚焦修订`L2_01_00/L2_01_02/P3_00`；设计复核通过后恢复`GATE-046`最小非live实现及原子同步；不授权生产Question Guard、公共契约、v1/历史资产、live、Git commit/push |
| 维护责任人 | 项目维护者（个人开发者，姓名未在需求中指定） |

> 本文只拥有 Knowledge 单动作内部的流程与配置，不拥有物理索引、ES/BGE 协议、统一候选字段、证据出域字段或摘要事实校验。v0.2 的 Approved 来自五轮独立评审，v0.3 的 Approved 来自 summary task 组合根兼容性针对性复评；两者均不构成实现授权或真实链路可用证据。
...
| 12 | 2026-08-01 | 1、5、13～14、18～20 | 第四批 L2 跨文档兼容修复 | 将预留 Evidence 接缝具体化为 rewrite/summary 两个纯 task definitions 在 model registry 冻结前统一注册；同步 `L2_01_02` 状态，进入 v0.3 针对性复评，实施/集成门禁不变 |
| 13 | 2026-08-01 | 1、5、13～14、18～20 | 第四批 L2 终审原子同步 | summary task 组合根、disabled 路径、registry 冻结顺序和契约测试针对性复评通过，关闭 `REV-KFLOW-022`；同步 `L2_01_02` v0.2 Approved，实施/集成门禁保持 Open |
| 14 | 2026-08-01 | 1～2、13～16、19～20 | `WP-KFLOW-01` 实施状态原子同步 | 记录 Knowledge flow/config、Capability/Provider、阶段接缝和 fake stages 已按授权实现并完成测试及代码对照设计评审；关闭本切片 `KQ-GATE-002`，真实检索、模型和证据出域门禁保持 Open |
| 15 | 2026-08-03 | 1、18～20 章及 `L2_01_01/P3_00` | `WP-KRET-REAL-01` 门禁状态原子同步 | 同步 `L2_01_01` v0.6 的真实 JWT/ES/BGE 两域四路与负向矩阵证据，将本文镜像的 `SA-GATE-003` 改为 Closed；默认 Runtime、模型与证据出域状态不变 |
| 16 | 2026-08-07 | 1～2、8、18～20 章 | Core/Model selection-only 定向兼容同步 | 明确模型只选择 `knowledge.query` ID；因执行 Schema 精确为空，混合节点确定性绑定 `{}`，既有 Capability/原问题/改写/检索/证据流程和实现证据均不变 |
| 17 | 2026-08-12 | 1～2、14、16、19～20 章及 `L2_00_02/L2_01_02/P3_00` | Knowledge 问题出域非 live 安全证据同步 | 新增公开问题最小化后仅到 fake rewrite gateway、敏感/unknown 问题 rewrite 零调用并将 denied 状态传入 Evidence、fresh Guard summary 零调用证据；`CR-GATE-003/GATE-021`按问题输入前置关闭，真实证据/模型出域仍受 `GATE-022/SA-GATE-006` |
| 18 | 2026-08-12 | 1～2、5～6、13～16、19～20 章及 `L2_01_02/P3_00` | summary v2 生产组合根聚焦修订 | 保持 flow、Stage、Core/HTTP 与 v1 历史不变；v2 位于独立 `summary_task_v2.py`，enabled 路径只创建 rewrite v1 + summary v2 并一次冻结 registry，v1 文件及其历史仅作不可变基线；新增本地切换门禁和组合根反证测试 |
| 19 | 2026-08-12 | 1～2、12～16、19～20章及`L2_01_02/P3_00` | summary v2生产组合根实施验证同步 | 独立v2模块与rewrite v1 + summary v2单注册已实现；直接20项、Knowledge 124/5 skipped、全量640/9 skipped、strict mypy 264 files和compileall通过，代码复核“符合”；V1/历史/validator/公共契约不变，关闭`KQ-GATE-003/GATE-042` |
| 20 | 2026-08-12 | 1～2、5、16、19～20章及`L2_01_02/P3_00` | 下游 `GATE-043` preparation 引用同步 | 同步 L2_01_02 v0.15 的V2 runner/manifest冻结事实；run/hash已形成但真实outbound未授权，Flow/Stage/Core/HTTP契约和生产单注册不变 |
| 21 | 2026-08-13 | 1～2、5、16、19～20章及`L0_00/L1_00/L1_01/L2_01_02/P3_00/ARCHITECTURE` | 下游 `WP-K-EGRESS-01` 完成状态同步 | 同步30/30真实summary、post-consumption严格哈希/绑定/终态回归及Knowledge范围`SA-GATE-006/GATE-032`关闭；Flow/Stage/Core/HTTP契约、生产summary v2单注册、V1/历史/validator均不变，live P5仍Open |
| 22 | 2026-08-13 | 1～2、4、6、8～14、16、19～20章及`L2_01_02/P3_00` | `GATE-046`终态优先级聚焦设计 | 明确仅在`question_egress_denied=true`与零域同时成立时，策略拒绝优先返回既有`model_egress_denied/knowledge.rewrite_input_denied`；未拒绝零域继续`no_result`。固定Capability唯一映射、检索/Evidence/model调用均0、primary/ablation一致、公共Schema与历史资产不变 |
| 23 | 2026-08-13 | 1～2、16、18～20章及`L2_01_02/P3_00` | `GATE-046`独立聚焦设计复核阻断同步 | 复核发现冻结`draft-security-invalid-id`被当前生产Question Guard判为allowed且命中税务域，无法在仅修改Capability的范围内满足四负例两变体策略拒绝/零调用；登记`REV-KFLOW-023`，保持`GATE-046` Open并停止代码实施 |
| 24 | 2026-08-13 | 1～2、14、16、18～20章及`L2_01_02/P3_00` | `GATE-046`版本化评估输入一致性修复设计 | 保持representative v1、candidate-01/02/03、全部历史hash和生产Question Guard不变；GATE-046改用representative v2中四个可被当前Guard确定性拒绝且域选择为空的非敏感合成安全负例。Capability方案、公共终态和普通零域语义不变 |
| 25 | 2026-08-13 | 1～2、14、16、18～20章及`L2_01_02/P3_00` | `GATE-046`非live实现、验证与代码对照设计复核 | representative v2、authorization、provenance及hash已冻结，四负例生产Guard denied且域选择为空，v1/历史保持精确hash；仅在Capability零域分支增加denied flag优先映射。四负例×两变体经真实Capability和严格packer均为既有策略拒绝，模型/检索/Evidence/retry/resume为0，普通零域仍`no_result`；Knowledge 191 passed/6 skipped、全量693 passed/10 skipped、strict mypy 279 files及compileall通过，关闭`VAL-KFLOW-006/GATE-046` |
...
| `CON-KFLOW-011` | 用户2026-08-13授权；L2_01_02 `RISK-KEV-010`；`GATE-045`有限失败证据 | 显式问题出域拒绝不得被后继零域/无结果覆盖；未拒绝的普通零域语义保持不变 | `DR-KFLOW-015` | `GATE-046` |
...
| `CON-KFLOW-011` | denied/zero-domain终态 | `DR-KFLOW-015` | Capability | 既有`model_egress_denied`或`no_result`组合 | `IMPL-KFLOW-007` | `TEST-KFLOW-011` | `VAL-KFLOW-006` |
...
| L2_00_02 v0.17 Approved | direct dependency | 提供 Provider-neutral task registry、问题闸门、安全模型上下文和受控结构化任务；Knowledge 自有 rewrite/summary Prompt | Provider、问题输入策略、供应商错误及公共传输预算 | `QuestionEgressGuard`、`ModelCallContextAccessor`、`BoundedStructuredModelGateway` | 模型调用状态 | v2 继续消费既有 `(task_id,task_version)` 接缝，无需修改模型公共层 |
| L2_01_01 v0.6 Approved | downstream provider | 定义检索计划消费和 coverage 控制语义 | ES/BGE、候选、融合、重排、读取授权 | `KnowledgeRetrievalStage[TBatch]` | 排序候选 batch | 当前冻结 Profile/快照的真实 Retrieval 已验证；默认仍 disabled |
| `L2_01_02` v0.17 Approved | downstream provider | 定义证据 stage 调用时机与输入控制语义，并按其 task version 提供生产 summary definition | 证据、三层出域、summary v1/v2、validator 和最终领域结果 | `KnowledgeEvidenceStage[TBatch]`、`KnowledgeSummaryTaskV2.definition()` | 证据/模型安全载荷 | v1历史保持不可变；v2本地实现、生产单注册、30/30 live及post-consumption闭环已验证；Knowledge范围完成门禁Closed，live P5 Open |
...
| `FACT-KFLOW-003` | 已存在（文档） | L2_00_02 v0.17 Approved 定义问题闸门、安全模型上下文、受控结构化任务和 `deepseek-v4-pro` Provider 边界，并明确领域拥有 Prompt | 本文不重复供应商协议，只定义 Knowledge task/version与生产注册选择 |
...
| `FACT-KFLOW-006` | 已存在（运行证据） | `GATE-045`在首个安全负例primary上出现rewrite transport=0、`question_egress_denied=true`、零域后Capability=`no_result`，P5 packer按既有安全终态要求失败；candidate-03证据不可复用或改写 | 证明问题位于Capability终态优先级，不应放宽packer、Guard、dataset或公共Schema |

### 6.2 当前问题与设计根因

历史根因不是“缺少一个 Knowledge 服务”，而是需要一个同时满足核心单动作契约和 Knowledge 内部多阶段语义的进程内能力实现。当前新增根因更窄：`handle`在域选择为空后无条件构造`no_result`，没有先消费同一`RewriteResult`中已经确定的`question_egress_denied`，使策略终态被低优先级无结果覆盖。
...
| 零域终态前置策略判定 | 必须 | 既有`RewriteResult`与公共`model_egress_denied`组合 | 修复策略拒绝被`no_result`覆盖 | 不修改packer接受规则、不增加新字段/状态/错误码、不让evaluation拥有生产终态语义 |
...
| `DR-KFLOW-015` | 域选择为空时先检查权威`RewriteResult.question_egress_denied`：为`true`时返回既有`model_egress_denied + knowledge.rewrite_input_denied + policy/denied`组合，policy version取该`RewriteResult.question_policy_version`，且retrieval、Evidence、rewrite/summary model追加调用均为0；仅当flag为`false`时才返回`no_result/no_matching_domain + not_applicable`。该规则不得由P5 packer、配置或dataset重定义 | Capability | rewrite success后的零域终止 | 策略拒绝不被无结果覆盖，普通零域兼容不变 |
...
在 Core v0.6/Model v0.8 的混合裁决中，模型侧只看到 `knowledge.query` 的能力 ID/描述及精确空 tool Schema，并且只返回 canonical ID；混合节点确认该 descriptor 的执行 Schema 也是精确空对象后，确定性构造 `ActionCandidate("knowledge.query", {})`。Knowledge 不需要 `LocalActionResolver`，也不允许模型回填原问题、逻辑域或检索参数；随后仍由注册项 validator 与 Core 单动作约束复核。该兼容同步不改变本 L2 任一阶段、配置或外部调用。
...
6. 域选择为空时按`DR-KFLOW-015`先检查`question_egress_denied`：为`true`返回既有策略拒绝，为`false`返回受控`no_result`；两者的检索/Evidence调用均为零。
...
8. 未被问题出域策略拒绝的合法执行后无候选/无匹配域/后续证据不足：`no_result`；`question_egress_denied=true`不得进入该优先级。
...
| 全局问题闸门拒绝、允许本地回退且域选择为空 | `model_egress_denied` | `knowledge.rewrite_input_denied` | policy | denied，使用`RewriteResult.question_policy_version`；不得返回`no_result` |
...
| 未被问题出域策略拒绝且无匹配启用域 | `no_result` | 无 | 无 | not_applicable |
...
4. 能力启用时先分别由 `KnowledgeRewriteTaskV1.definition()` 与
   独立 `summary_task_v2.py` 中的 `KnowledgeSummaryTaskV2.definition()` 创建两个纯、冻结、无客户端的
   `ModelTaskDefinition`；顶层 `agent_runtime.bootstrap` 把二者与 L2_00_02 的
   action/answer task 组成代码显式 tuple，再一次性构建/冻结 model task registry 和
   gateway。二者的领域 Prompt/DTO 分别由本文与 L2_01_02 定义；model 公共模块不得反向
   导入 Knowledge，配置也不能追加 task。生产 registry 不得同时注册
   `KnowledgeSummaryTaskV1` 与 `KnowledgeSummaryTaskV2`，也不得根据环境、请求或 Provider
   动态选择版本；v1 只允许由历史隔离 harness/test 显式实例化。
...
disabled 路径在第 3 步终止，不创建任一 Knowledge task definition、rewriter、stage 或客户端；
...
| `IMPL-KFLOW-007` | 已实现、建议修改 | Python capability | `agent-runtime/src/agent_runtime/knowledge/capability.py` | `KnowledgeQueryCapability.handle`零域终止分支 | 在现有零域`no_result`前消费`RewriteResult.question_egress_denied`并构造既有策略拒绝组合；其他流程不变 | 闭合`GATE-045`暴露的终态优先级，保持Capability唯一公共映射 | `DR-KFLOW-001/002/007/008/010/011/012/013/014/015` |
...
| `IMPL-KFLOW-009` | 已实现、建议修改 | Python composition | `agent-runtime/src/agent_runtime/bootstrap.py` | 从 L2_01_02 建议新增的独立 `knowledge.evidence.summary_task_v2` 模块导入 v2 definition，将生产 summary v1 definition 原子替换为 v2；`summary_task.py` 零修改 | 组合根接入与单版本注册 | 新能力不侵入 core，历史v1文件不被修改、删除或双注册 | `DR-KFLOW-009/012`、`L2_01_02 DR-KEV-016/017` |
...
| `TEST-KFLOW-008` | 建议修改 | `DR-KFLOW-001/009/012`、`L2_01_02 DR-KEV-016/017` | Contract | `agent-runtime/tests/contract/knowledge/test_provider_registration.py` | disabled、1 域、2 域 provider；rewrite/summary task-definition、model-registry/core-registry fakes；历史v1隔离fixture | 精确一个 capability candidate；disabled 不建任一 Knowledge task/client/stage；enabled 在 gateway 前精确冻结 `(knowledge_rewrite,"1")` 与 `(knowledge_summary,"2")`，生产无 `(knowledge_summary,"1")`；v2定义来自独立模块；v1仍可被历史隔离测试直接创建，且 `summary_task.py` SHA-256 保持 `dba0175a7e2810ea1a1c5601499cd9da74de6c3cf60b4026cc7136233b864645`；model公共模块无Knowledge import | 循环装配、动态扫描、v1文件漂移、v1/v2双注册、自动回退、漏/重复task、多个动作或缺依赖仍就绪 |
...
| `TEST-KFLOW-011` | 建议新增 | `DR-KFLOW-014/015` | Integration/Evaluation non-live | `agent-runtime/tests/integration/knowledge/test_question_egress.py`；建议新增`agent-runtime/tests/evaluation/knowledge/test_live_p5_denied_zero_domain.py` | 从L2_01_02定义的representative v2读取四个`security_negative`，分别执行primary生产rewriter与rewrite_ablation rewriter，二者均通过真实`KnowledgeQueryCapability`；primary使用fake model transport；另测未拒绝普通零域；v1精确hash保持不变 | 每个安全负例先由生产Question Guard判定denied且域选择为空，两变体均为`model_egress_denied/knowledge.rewrite_input_denied`、policy一致、retrieval/Evidence/transport=0、retry/resume=0；普通零域仍`no_result`；`live_executor.py`零修改且既有严格packer可接受终态 | 测试专用输入绕过生产分类、只测fake stage而绕过Capability、放宽packer、策略拒绝变`no_result`、普通零域被误拒绝、v1漂移或发生任何模型/检索调用 |
...
| 未被策略拒绝的普通非税问题、零域 | primary=1、ablation=0 | 0 | 0 | `no_result`；证明普通零域兼容不变 |
| 问题出域拒绝、本地回退、零域 | 0 | 0 | 0 | primary/ablation均为`model_egress_denied`；不得被零域覆盖 |
...
| `VAL-KFLOW-005` | 本文、L2_00_01 v0.11、L2_00_02 v0.17、L2_01_01 v0.6、L2_01_02 v0.17及代码复核证据 | 核对原问题同源、问题闸门、Stage类型、registry冻结顺序、v1历史不可变、v2生产单注册、disabled和责任防重叠 | 证明跨L2语义未漂移，不替代真实集成 | 聚焦设计及代码对照设计复核无未关闭重大问题 | 2026-08-13结论“符合”；本地实现、30/30 live、post-consumption非live闭环均通过，Flow/Stage/Core/HTTP契约无变化 |
| `VAL-KFLOW-006` | `D:\codex\agent-runtime`；representative v2输入分类和设计复核须先解除`REV-KFLOW-023` | 运行`TEST-KFLOW-011`、representative v1/v2一致性、Knowledge相关与全量非live pytest、`python -m mypy --strict src tests`、compileall、candidate-01/02/03 manifest/evidence历史hash、L2/P3严格validator和代码对照设计复核 | 证明策略拒绝优先、普通零域兼容、零外部调用、v1/历史不可变及无公共契约扩张 | 全通过且`live_executor.py`、representative v1/gold、历史资产diff=0；随后才可关闭`GATE-046` | 2026-08-13已通过：定向9 passed；Knowledge 191 passed/6 skipped；全量693 passed/10 skipped；strict mypy 279 files、compileall、L2/P3严格validator与代码对照设计复核通过；外部调用=0 |
...
|---|---|---|---|---|---|---|
| `RISK-KFLOW-001` | 直接依赖 | L2_00_01 v0.11 与 L2_00_02 v0.17 的当前契约仍要求 Knowledge 精确空执行 Schema 自动绑定 `{}`，模型只选择 capability ID | 后续把 `knowledge.query` 增加参数或错误投影执行 Schema | Flow 与 Core/Model 不兼容，或原问题/域控制权进入模型 | 保持空 Schema contract test、组合根覆盖校验和代码对照设计评审；加参必须新契约版本 | 当前 flow 切片证据仍有效；summary task version变化不扩大该边界 |
...
| `RISK-KFLOW-005` | 下游契约 | Retrieval/Evidence batch 已由后续 L2 定义并完成本地 fake/stub 消费，真实 Provider 仍未联调 | 真实端到端接入 | 类型、快照或 failure 漂移 | 保持泛型 stage 接缝和跨 L2 契约测试；真实集成门禁关闭前禁用 live Provider | 不阻塞本地切片；真实端到端保持未获准 |
| `RISK-KFLOW-006` | 预算 | 8/20/15 秒尚无真实链路耗时证据 | P4 联调 | 误超时或余量浪费 | 保持硬上限，采集指标后在上位允许范围内收紧 | 不阻塞设计；阻塞性能结论 |
| `RISK-KFLOW-007` | 信息泄露 | no_result/coverage 可能暴露禁用域或不可读文档存在性 | 权限过滤或配置变化 | 推断知识存在 | 用户结果统一 `no_candidate`；只输出由问题确定的选中域与安全枚举；整域拒绝无领域载荷；本地负向测试已通过 | 本地切片已验证；真实授权场景仍受集成门禁控制 |
| `RISK-KFLOW-008` | 组合根版本 | 生产同时注册 summary v1/v2，或用配置/请求动态选择版本 | v2切换或回滚实现不完整 | 同一 task ID 多版本歧义、历史证据污染、不同请求得到不可审计语义 | 生产显式 tuple 只含 rewrite v1+summary v2；disabled两者均不创建；v1仅历史隔离实例化；回滚整个组合根切换而非双注册 | 由 `KQ-GATE-003/P3_00 GATE-042` 控制实施；不影响本文设计完整性 |
| `RISK-KFLOW-009` | 安全终态优先级 | `question_egress_denied=true`与零域并存时，低优先级`no_result`覆盖策略拒绝 | representative v2安全负例允许本地fallback且选择零域 | 安全边界虽未泄漏，但P5终态失真并中断 | `DR-KFLOW-015`由Capability唯一执行；L2_01_02先以生产Guard/域选择冻结四个denied+零域输入，再由真实Capability+fake transport覆盖两变体和普通零域 | `GATE-046`已关闭；后续输入/Guard/终态漂移须由同类回归阻断新live candidate |
...
| `KQ-GATE-002` | slice_implementation | 本文唯一负责的 Knowledge flow/config 切片 | 创建目标 Capability、配置、目录、改写/选择/计划代码与测试 | 历史实施授权、测试和代码对照设计评审已通过；selection-only 兼容不改变该切片 | 本文历史评审、P3 与本地实施证据 | 项目维护者 | P3 flow 实施前 | 核对空 Schema、非 live 范围和既有回归 | Closed | 本地 flow 可维护；Core 混合节点、真实模型和证据出域由各自新工作包/门禁控制 | 内存 fake stage |
| `KQ-GATE-003` | slice_implementation | summary v2生产组合根切换 | 新增独立v2模块，修改组合根和直接相关测试；不调用真实模型 | exact task/instruction、registry/disabled、V1/历史hash、fake重复ref、Knowledge/全量非live、类型与代码对照设计复核全部通过 | `L2_01_02`、`P3_00 GATE-042`、本地验证证据 | 项目维护者 | v2生产注册前 | exact registry/disabled/history/fake tests与代码对照设计复核 | Closed（2026-08-12） | 允许维护v2生产单注册及准备新live manifest；禁止V1/历史/validator修改、双注册、自动回退或outbound | 回滚组合根到v1+stub |
| `CR-GATE-003` | integration | 原问题进入 DeepSeek 改写 | 外发可能敏感的用户问题 | L2_00_02 输入分类、最小化、Knowledge rewrite→Evidence denied 传播与零调用负向测试通过 | 模型 L2/Provider 契约、`TEST/VAL-MODEL-017/013` | 项目维护者/模型方 | 首次真实改写联调前 | model spy/负向测试 | Closed（2026-08-12；仅问题输入安全前置） | 允许通过 Guard 的非敏感最小问题进入另行授权的后继工作包；敏感/unknown 继续本地回退且模型零调用 | 原问题本地回退 |
| `SA-GATE-003` | integration | 真实 retrieval stage | 接入真实 ES/BGE | L2_01_01 契约、读取授权、模型兼容和负向测试通过 | L2_01_01/提供方 | 项目维护者/提供方 | P4 | 契约/集成测试 | Closed | 当前冻结 Profile/快照的真实 Retrieval 前置满足；默认 Runtime 与证据出域不随之启用 | 内存统一候选 batch |
| `SA-GATE-006` | integration | 真实 evidence stage 模型输入 | 外发真实知识证据 | L2_01_02 三层策略、文档元数据、零调用、30/30受控live及post-consumption闭环通过 | L2_01_02/策略权威 | 项目维护者/模型方 | P4 | 出域矩阵/model spy/append-only evidence | Closed（2026-08-13；仅Knowledge当前冻结切片） | 禁止复用已消费授权、常规无界外发或扩展到业务域/新快照 | 合成安全证据；新快照须新门禁 |
| `GATE-046` | slice_implementation | denied/zero-domain终态修复 | 修改`KnowledgeQueryCapability.handle`及直接相关非live测试 | L2_01_02 representative v2输入分类/冻结和本文联合聚焦设计复核符合；`VAL-KFLOW-006`全部通过；公共Schema、packer、representative v1/gold和历史资产不变 | 本文`DR-KFLOW-015/TEST-KFLOW-011`、L2_01_02 `DR-KEV-022/023`、P3_00 | 项目维护者 | 新P5 candidate准备前 | 设计复核、pytest/mypy/hash与代码对照设计复核 | Closed（2026-08-13） | 允许维护已验证终态；新live candidate、密钥、outbound及历史授权复用仍须独立授权 | 真实Capability+fake transport |

### 16.3 需要后续授权的动作

- 为已关闭的本切片增加超出 `WP-KFLOW-01` 的能力或公共契约。
- 修改 `es-query-service`、BGE、DeepSeek、OpenAPI 或其他外部契约。
- 使用真实用户问题、真实知识正文或付费模型调用做联调。
- 实施或启用 `KnowledgeSummaryTaskV2`、调整生产 registry、删除/修改 v1，或创建/执行新稳定性 run；均须后续独立授权。
...
| 4 | 2026-08-12 | 0 | 1 | 1 | 2 | 无 | v2聚焦第1轮：修复生产双版本歧义，固定rewrite v1+summary v2唯一tuple，v1仅历史隔离使用 |
| 5 | 2026-08-12 | 0 | 1 | 1 | 2 | 无 | v2聚焦第2轮：补齐disabled、回滚、组合根反证和独立实施门禁，保持Flow/Stage/Core/HTTP不变 |
| 6 | 2026-08-12 | 0 | 1 | 1 | 2 | 无 | v2聚焦第3轮：发现同文件追加v2会改变v1文件哈希，改为独立`summary_task_v2.py`并固定`summary_task.py`精确SHA-256反证；同步验证定义和下一授权边界，严格校验后进入聚焦评审 |
| 7 | 2026-08-13 | 0 | 1 | 1 | 2 | 无 | `GATE-046`聚焦自检：将宽泛“零域直接no_result”收紧为显式flag二分；补齐既有policy版本/错误码复用、普通零域兼容、真实Capability两变体、零调用和历史资产反证，未修改公共Schema或packer |
...
全量检查；未发现新的 S0/S1/S2，`REV-KFLOW-001`～`021` 全部关闭。本文 v0.2 Approved，
设计已具备 flow/config 切片的设计完整性。该轮终审时 L2_01_01 仍为 Draft、L2_01_02 尚未创建；
这一历史前提已由后续批次替代，但 `KQ-GATE-002` 未关闭始终意味着不能据此实施目标代码、
启用真实链路或声明效果达标。

### 18.6 v0.3 summary task 组合根兼容性针对性复评

| 发现 ID | 严重度 | 冻结证据与影响 | 修复 | 状态 |
|---|---|---|---|---|
| `REV-KFLOW-022` | S1 | v0.2 组合根只显式创建 `KnowledgeRewriteTaskV1`；L2_01_02 新增 summary task 后若在 registry 冻结后补注册，将导致启动遗漏或 model→Knowledge 反向依赖 | enabled 路径在 registry 冻结前同时创建 rewrite/summary 两个纯 definition，与 action/answer definitions 一次性冻结；disabled 路径两者均不创建，并补齐 `TEST-KFLOW-008` | Closed |

针对性复评重新核对 Knowledge task 所有权、顶层组合根顺序、disabled 路径、唯一 capability 数量、公共 model 模块依赖方向及契约测试；未发现新的 S0/S1/S2。本文 v0.3 恢复 Approved；该结论不关闭 `KQ-GATE-002` 或任何真实模型、检索、证据出域门禁。

### 18.7 v0.8 summary v2 生产单注册独立聚焦设计评审

本次 targeted-check 只复核 `KnowledgeSummaryTaskV2` 的生产组合根消费，不重做 flow/config 全文评审。检查了 L2_00_02 的 `(task_id,task_version)` Provider-neutral registry、L2_01_02 的 v2 独立模块与 v1 文件哈希、enabled/disabled 装配、生产单版本约束、回滚和 P3 门禁顺序。

结论为“符合”，未发现未关闭 S0/S1/S2：生产 enabled 路径精确注册 rewrite v1 + summary v2，disabled 路径不创建 Knowledge task/client/stage；v1 只允许历史隔离实例化且 `summary_task.py` 字节保持不变；禁止双注册、动态版本选择和自动回退。`GATE-042` 仍须另行代码授权和非 live 证据，故本结论不证明 v2 已实施、生效或可进行真实模型调用。

### 18.8 v0.12 denied/zero-domain终态优先级独立聚焦设计评审

本次 targeted-check 仅复核`DR-KFLOW-015`及其与L0/L1公共状态、`L2_01_02 DR-KEV-022`、现有`RewriteResult`、Question Guard和P5严格packer的关系，不重做全文批准。Capability终态映射本身无公共契约冲突，但实施前事实核对发现如下阻断：

| 发现ID | 严重度 | 证据 | 影响 | 状态 |
|---|---|---|---|---|
| `REV-KFLOW-023` | S1 | 当前`QuestionEgressGuard`对冻结case `draft-security-invalid-id`返回`allowed`：合成marker不匹配18位身份证模式，而问题中的“税务”命中`public_knowledge`；随后域选择命中税务域 | 该case的primary会产生rewrite model调用，ablation会进入retrieval，无法证明四个安全负例两变体均为策略拒绝且零model/retrieval/Evidence调用；只调整Capability零域分支不能改变flag=false且域非空的事实 | Closed（v0.13以representative v2修复输入，不改v1或Guard） |

评审结论为“Blocked”。必须先由单独授权选择并设计以下之一：版本化修正评估输入及provenance/hash，或扩展生产Question Guard及其上位模型安全设计；禁止把fake rewriter伪装成生产primary、在Capability识别测试case、修改冻结v1 dataset/history或放宽packer。`GATE-046`保持Open，当前不允许实施代码。

### 18.9 v0.13 representative v2输入一致性联合聚焦设计评审

本次 targeted-check 只复核L2_01_02 `DR-KEV-023`是否为`DR-KFLOW-015`提供真实可达的denied+零域输入，并核对生产Question Guard/域选择器、Capability唯一终态责任、v1/历史不可变性和P3门禁顺序；不重做Flow全文评审。

| 聚焦项 | 设计证据 | 结论 |
|---|---|---|
| 责任边界 | 测试数据只提供输入；Guard仍负责分类，Capability只消费`question_egress_denied`和域选择结果，不识别case ID/marker | 符合 |
| 终态可达性 | 四个固定v2问题经当前生产Guard实测均为`denied/sensitive_input`，经当前生产域选择器均为空；普通零域flag=false的设计分支不变 | 符合 |
| 不变性 | representative v1四项SHA-256已复算并与冻结值一致；生产Guard、公共Flow/Stage/Core/HTTP、packer和历史candidate不在修改范围 | 符合 |
| 顺序与无环 | v2输入闭环先于Capability实施；P3实测39节点、69条直接依赖、46门禁且DAG无环；`GATE-046`关闭后仍不能直接执行live | 符合 |

评审结论为“符合”，未发现未关闭S0/S1/S2；`REV-KFLOW-023`由版本化输入方案关闭，不表示v2资产或Capability已实施。只有`VAL-KEV-014`完成后才可修改`IMPL-KFLOW-007`，且`GATE-046`关闭不授权live。
...
- [x] v0.2 五轮独立正式评审已关闭全部 S0/S1/S2。
- [x] v0.3 summary task 组合根兼容性针对性复评通过。
- [x] 本切片 `KQ-GATE-002` 已在另行实施授权、测试和代码对照设计评审后关闭。
- [x] selection-only 模型选择只返回 `knowledge.query` ID，混合节点仅因精确空执行 Schema 绑定 `{}`；无需 Knowledge Resolver，原问题与内部流程不变。
- [x] summary v2 位于独立 `summary_task_v2.py`，生产组合根目标固定为 rewrite v1 + summary v2；`summary_task.py` 及v1保留但不进入生产registry，公共 Flow/Stage/Core/HTTP 契约不变。
- [x] `KQ-GATE-003/P3_00 GATE-042`已关闭；当前生产组合根为rewrite v1 + summary v2单注册，V1文件及历史保持不变。
- [x] `DR-KFLOW-015`明确`question_egress_denied=true + zero-domain`优先返回既有策略拒绝组合；未拒绝普通零域仍为`no_result`，无新字段、状态或错误码。
- [x] v0.12发现的`REV-KFLOW-023`已选择L2_01_02 representative v2版本化输入方案；生产Question Guard与representative v1不改。
- [x] v0.13联合聚焦设计评审通过，生产分类事实和P3 DAG无环已核实；未修改生产Guard、v1或历史资产。
- [x] representative v2资产及独立authorization/provenance/hash已冻结，`VAL-KEV-014`通过；四个安全负例生产Guard均denied且域选择为空，v1和历史资产保持精确hash。
- [x] `GATE-046/VAL-KFLOW-006`已完成：Capability最小终态修复、真实两变体、严格packer、普通零域、零调用、全量非live、类型及代码对照设计复核均通过。

## 20. 当前结论

- 本文版本：v0.14。
- 文档状态：Approved。
- 评审状态：历史 v0.2 五轮独立评审与 v0.3 summary task 兼容性复评保持有效；v0.6 selection-only与v0.8 summary v2聚焦评审结论保持有效。v0.13 representative v2联合聚焦设计评审通过；v0.14代码对照设计复核符合，`REV-KFLOW-023/GATE-046`已关闭。
- 实施状态：既有flow/config、真实Retrieval接缝、问题出域安全证据与summary v2单注册保持有效。`DR-KFLOW-015/TEST-KFLOW-011/VAL-KFLOW-006`已实施验证，策略拒绝不会再被零域`no_result`覆盖。
- 生效状态：未生效。
- 是否可作为实现依据：按既有范围可用；新live P5 candidate仍须另行设计、冻结、授权和验证。
- 确定性文档校验：strict模式0 errors/0 warnings；不替代独立聚焦设计评审。
- 当前只证明策略拒绝与普通零域的唯一终态优先级及非live实现符合性；不证明live P5效果、业务结果出域、默认启用或生产生效。
