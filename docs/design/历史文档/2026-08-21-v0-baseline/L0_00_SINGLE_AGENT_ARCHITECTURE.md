...
| 文档状态 | 已评审 |
| 评审状态 | 有条件通过 |
| 实施状态 | P3 当前71个工作包中61个Done、10个Deferred，Ready/In Progress/Blocked均为0；Core、Access、模型Runtime、Knowledge真实检索与当前冻结切片出域、Employee/Transaction受控真实只读联调、Knowledge live P5及三能力真实Provider + 默认stub系统E2E均已有证据。P5已形成`ineffective`的有效效果结论；Employee/Transaction真实结果进入外部模型为当前交付范围外的可选实验；目标环境默认启用和生产部署仍未完成 |
| 生效状态 | 未生效 |
| 当前版本 | v0.13 |
...
| 治理的 L1 文档 | [`L1_00`《单体 Agent 核心与运行架构 L1》](L1_00_SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md)（v0.8，已评审/已通过）；[`L1_01`《单体 Agent 知识查询能力架构 L1》](L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md)（v0.7，已评审/已通过，Knowledge出域与P5效果结论已完成）；[`L1_02`《单体 Agent 业务查询适配架构 L1》](L1_02_SINGLE_AGENT_BUSINESS_QUERY_ADAPTER_ARCHITECTURE.md)（v0.5，已评审/已通过，业务真实模型出域转为可选Deferred） |
...
> 本文是 P1 总体架构产出。v0.6 的架构决策保持：DeepSeek只负责无敏感执行参数的能力ID选择，Employee/Transaction参数由本地Resolver解析，最终`ActionCandidate`与Core不变。v0.12收敛门禁治理：执行许可、验收门禁和工作包状态分离；业务真实模型出域不再阻塞P3/P4主链，未关闭的领域出域门禁继续失败关闭实际外发。
...
| 15 | 2026-08-01 | 文档治理、下位状态与实施证据范围 | 原子同步 P3 的 Core、Access、Model local、Knowledge 与 Business 共 12 个本地 fake/stub 工作包及下位文档状态 | 不改变 v0.5 架构决策；真实 ES/BGE/DeepSeek、业务服务/JWT、Provider 改造、数据出域、P5、部署和生产生效门禁保持 Open |
| 16 | 2026-08-03 | 文档治理、下位状态与实施证据范围 | 原子同步 `L2_02_02` v0.6、`P3_00` v0.8 及 Transaction Provider 候选/50,2 契约已完成本地验证 | 不改变 v0.5 架构决策；Transaction 真实 JWT/业务数据/出域、其他 Provider、P5、部署和生产生效门禁保持 Open |
| 17 | 2026-08-03 | 4.2、6.3、10.3、13、17.1、18.2 | 原子同步 `L2_00_03` v0.2 正式评审及共享 Authority Converter/三个 Provider opt-in 当前候选事实 | 不改变 v0.5 安全所有权或业务最终授权边界；共享实现关闭、真实 JWT、业务数据、出域和生产生效门禁保持 Open |
| 18 | 2026-08-06 | 文档治理、Transaction 契约、阶段门禁、待决事项、追踪与实现证据 | 基于 `wp-txn-real-01-20260806T134518Z.json` 及独立代码对照设计复核，原子同步 `transaction.search` 真实 JWT、精确金额、可见性、禁止接口和日志证据，关闭 `SA-GATE-005` | 不改变 v0.5 架构决策；只允许受控目标配置启用 Transaction 只读动作，`CR-GATE-003/SA-GATE-006`、默认启用、模型出域和生产生效保持 Open |
| 19 | 2026-08-06 | 文档治理与 `SA-GATE-002` | 原子同步 `action-selection-v2` 固定 10 case×3 次 PoC：30/30 完成、23/30 结构有效、23/30 预期匹配，且三个 Transaction case 分别为 0/3、2/3、0/3 | 不改变 v0.5 架构决策；PoC 未达关闭阈值，`SA-GATE-002` 保持 Open，Runtime wiring、追加付费调用和真实数据出域继续禁止 |
| 20 | 2026-08-07 | 5～9、12～17 章 | 将动作解析拆为模型能力 ID 选择与业务本地确定性参数解析，保持最终 `ActionCandidate`、Core、Provider、权限和精确金额契约不变 | v2 PoC 证明“模型同时选择动作并生成业务参数”不稳定；以最小跨层修改消除敏感问题/参数进入 DeepSeek 的必要性，并为 selection-only v3 PoC 建立新门禁 |
| 21 | 2026-08-07 | 文档治理、下位状态与实施证据范围 | 原子同步 Provider-neutral 混合动作解析实现、36 项直接测试、376 项行为回归和两轮代码对照设计评审；完整 tests strict mypy 仍有 28 项工作包外既存错误 | 不改变 v0.6 架构决策或生效状态；`WP-ACTION-RESOLUTION-01` 保持 In Progress，业务 Resolver、v3 PoC、Runtime wiring 与出域后继不解锁 |
| 22 | 2026-08-07 | 文档治理、下位状态与实施证据范围 | 原子同步完整 `VAL-CORE-007`：376 passed/4 skipped、229 files strict mypy 与 compileall 通过，`WP-ACTION-RESOLUTION-01` 置 Done | 不改变 v0.6 架构决策或生效状态；仅将业务 Resolver 重算为 Ready，v3 PoC、Runtime wiring 与出域后继仍受原门禁控制 |
| 23 | 2026-08-07 | 文档治理、模型状态、门禁与下位版本 | 原子同步 v3 30/30已完成但17/30结构、3/30预期的失败证据，以及L2/P3的v4 JSON ID后继方案 | 不改变v0.6架构决策；v3转历史Deferred，v4实施/PoC分别受`GATE-036/037`控制，`GATE-020/SA-GATE-002`继续Open |
| 24 | 2026-08-07 | 文档治理、下位评审与门禁状态 | 原子同步 `L2_00_01` v0.8、`L2_00_02` v0.11 独立聚焦评审通过及 P3 v0.25 无环复核 | 不改变 v0.7/L0 架构决策；`GATE-036` 仅满足评审子条件，代码、付费 PoC、wiring 与完成门禁继续 Open |
| 25 | 2026-08-07 | 文档治理、下位实施与门禁状态 | 原子同步 `L2_00_01` v0.9、`L2_00_02` v0.12 与 P3 v0.26 的 v4 非 live 实施、验证、未消费候选 manifest 和代码对照设计复核 | 不改变 v0.7/L0 架构决策；`GATE-036` Closed，`GATE-037/020/SA-GATE-002` 继续 Open，默认 Runtime 与生效状态不变 |
| 26 | 2026-08-10 | 文档治理、下位失败证据与后继门禁状态 | 原子同步 `L2_00_02` v0.14 与 P3 v0.28：保留 candidate-01 不可变失败证据，记录 corrected candidate-02 非 live 验证、未消费 manifest 及代码对照设计复核 | 不改变 v0.7/L0 架构决策；`GATE-037` 已消费关闭但不表示 PoC 通过，`GATE-038/020/SA-GATE-002` 继续 Open，默认 Runtime 与生效状态不变 |
| 27 | 2026-08-10 | 文档治理、下位live证据与后继门禁状态 | 原子同步`L2_00_02` v0.15与P3 v0.29：candidate-02恰好30次全部满足结构/语义/逐case/零参数/零业务执行门槛 | 不改变v0.7/L0架构决策；`GATE-038` Closed，`GATE-020/SA-GATE-002`继续Open，默认Runtime与生效状态不变 |
| 28 | 2026-08-12 | 文档治理、下位Runtime实施与门禁状态 | 原子同步`L1_00` v0.5、三份核心L2与P3 v0.30：受控模型组合根、Context binding、单client生命周期和fake失败回归通过 | 不改变L0职责边界；`GATE-020/SA-GATE-002`仅按Runtime实现切片关闭，真实数据出域、目标环境启用和生效状态不变 |
| 29 | 2026-08-12 | 文档治理、下位问题输入安全证据与门禁状态 | 原子同步三份 L1、相关 L2 与 P3 的问题分类、最小化、未知/拒绝零调用证据，关闭 `CR-GATE-003/GATE-021/023/025` | 不改变 L0 架构决策；只解除问题输入安全前置，真实知识证据/业务结果出域继续受 `SA-GATE-006` 与领域结果门禁控制 |
| 30 | 2026-08-12 | 文档治理、Knowledge下位实施与门禁状态 | 原子同步`L1_01` v0.5、`L2_01_02` v0.7与P3 v0.32：真实文档级策略目录/快照一致性和非live安全实现已验证，一次性summary联调失败且授权耗尽 | 不改变L0架构决策；`GATE-022/032`与`SA-GATE-006/007`保持Open，默认启用、业务结果出域和生产生效不变 |
| 31 | 2026-08-13 | 文档治理、Knowledge下位实施与门禁状态 | 原子同步`L1_00` v0.7、`L1_01` v0.6、`L2_01_00` v0.11、`L2_01_02` v0.17与P3 v0.43：summary v2 30/30、post-consumption严格闭环及代码复核通过 | 不改变L0架构决策；`SA-GATE-006`仅按Knowledge当前冻结切片关闭，`GATE-032`关闭。Employee/Transaction、live P5、默认启用和生产生效继续由独立门禁控制 |
| 32 | 2026-08-20 | 文档治理、核心决策、阶段路径、门禁与下位状态 | 基于跨层门禁审计收敛执行许可、验收门禁和工作包状态；将业务真实模型出域稳定性实验移出当前P3/P4完成条件，解除系统E2E对其依赖，并同步Knowledge P5已形成`ineffective`有效结论 | 恢复个人学习验证项目“先打通完整链路”的优先级；不关闭Employee/Transaction真实出域安全门禁，不修改需求、生产契约、代码或历史证据 |
| 33 | 2026-08-20 | 文档治理、下位实施证据与第一阶段验收状态 | 原子同步多域batch快照成员校验及真实Knowledge/Employee/Transaction Provider + 默认stub的Spring→Runtime系统E2E完成证据；P3转为61 Done/10 Deferred，其余状态0 | 不改变L0架构决策、scoped业务结果出域门禁或未生效状态；系统E2E不代表目标部署、生产启用或效果达标 |
| 34 | 2026-08-20 | 当前事实、Authority门禁、P5状态与设计权威边界 | 修正P5过期事实，以组合式真实JWT证据关闭当前受控配置的`AUTH-GATE-002`，并明确历史run/hash/candidate仅为审计证据 | 消除跨层状态冲突并阻止测试执行治理继续膨胀；不改变业务最终授权、目标部署门禁、历史证据或生产代码 |
...
| 本文 | parent | 本次按用户授权同步Knowledge出域闭环证据 | v0.6架构评审保持有效；v0.11未改变L0边界，`SA-GATE-001`结论不变 |
| [`L1_00` 核心与运行 L1](L1_00_SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md) | governed_child | 原子同步系统E2E完成状态 | v0.8已评审/已通过；Core/Resolver、Runtime、问题输入安全及真实三Provider + 默认stub系统E2E已验证 |
| [`L1_01` Knowledge L1](L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) | governed_child | 原子同步多域与系统E2E状态 | v0.7已评审/已通过；Flow/Retrieval/Evidence、真实检索、多域batch契约、26-case数据集、当前冻结切片出域及live P5均已验证，效果结论`ineffective` |
| [`L1_02` 业务查询适配 L1](L1_02_SINGLE_AGENT_BUSINESS_QUERY_ADAPTER_ARCHITECTURE.md) | governed_child | 原子同步系统E2E状态 | v0.5已评审/已通过；两域受控真实联调、本地Resolver及默认stub系统E2E已验证；真实结果外发实验Deferred，默认/生产未生效 |
...
| 事实 ID | 已核实事实 | 证据 | 架构含义 |
|---|---|---|---|
| SA-FACT-001 | 立项时工作区不存在新的 Agent 目标实现；后续已按本文边界建立受限实现切片 | 根目录模块、`serviceCenter/pom.xml` 与 P3 实施证据 | Agent 仍按新模块集合建设，不迁移旧 Agent 实现；当前实施状态以文首治理信息和 18.2 为准 |
| SA-FACT-002 | `auth-service` 签发包含 `sub`、`iat`、`exp`、`token_type=user` 和 `role` 的 JWT | `auth-service/.../JwtService.java` | 身份和角色声明来源可复用 |
| SA-FACT-003 | `common-security` 已形成严格的 `role`→`ROLE_ADMIN/ROLE_VIEWER` Servlet/Reactive Converter 和具名 Bean；Knowledge、Employee、Transaction 已完成限定范围的真实签发 JWT 消费验证 | `L2_00_03`、共享安全代码、本地测试及三个 Provider 的受控联调证据 | 共享映射与限定集成已验证；默认部署和生产生效仍须由各自门禁独立证明 |
...
| SA-FACT-006 | Employee detail、Transaction search 及 Knowledge typed endpoint 已形成 endpoint-scoped 角色安全链/Guard，并完成各自动作范围内的真实 JWT、成功响应可见性与拒绝矩阵受控验证 | 三个 Provider 安全配置、Guard、合成 JWT 测试及 `SA-GATE-003/004/005` 关闭证据 | 已验证结论只适用于冻结 Profile/动作与受控目标配置；默认启用、模型出域和生产生效不得由此推导 |
...
| SA-FACT-010 | 立项时未发现问题改写、知识域注册与路由、多路召回融合、语义重排或证据化答案摘要实现；后续已形成完整实现、当前冻结检索/证据出域闭环与测试，并完成有效live P5，结论为`ineffective` | 工作区代码、P3实施证据及Knowledge L2 | 四项能力仍由Agent内的Knowledge切片承接；`ineffective`是当前冻结数据集/快照的有效效果结论，不得外推为默认在线链、后续快照或生产效果 |
...
5. Employee/Transaction 现有业务接口已分别核实并冻结为 `employee.detail` 与 `transaction.search`；未来新增动作仍必须在真实集成前重新核实公开契约和授权范围。
...
- 由 Agent 解析一个可用查询能力：外部模型只可返回已注册能力 ID；需要执行参数的 Employee/Transaction 动作由本地确定性 Resolver 生成结构化参数。
...
下表保留 P1/P2 立项时的“当前状态”和原始差距，用于说明目标架构的形成依据；后续实施事实不回写覆盖历史基线，当前状态以文首治理信息、第 13 章门禁和 18.2 为准。
...
| 角色授权 | JWT claim、统一 Authority Converter 和三个目标入口已存在；共享映射、组合式真实 JWT 角色矩阵及各域受控可见性已验证 | 业务系统按本域角色最终授权 | 保持共享映射与分域最终授权；目标部署/生产生效仍须独立核验 | P3-P4 |
...
| 模型 → Agent 核心 | 无默认受信内容 | 能力 ID 选择 | 映射表、注册状态和动作白名单；模型不得生成 Employee/Transaction 执行参数 |
| 模型 → 知识查询能力 | 无默认受信内容 | 改写问题、域选择、重排结果和答案草稿 | 原问题语义约束、知识域注册表、候选集合边界和证据一致性 |
| Agent → 业务 Adapter | 已验证的执行上下文 | 本地 Resolver 形成的候选参数 | Core 公共 JSON 校验、注册项 validator 与 Adapter 动作级边界校验 |
...
| SA-C-022 | 外部模型只选择已注册能力 ID，不生成 Employee/Transaction 执行参数；业务参数由动作归属域的本地确定性 Resolver 解析，歧义、冲突或不完整输入失败关闭，最终候选仍由 Core 与注册项 validator 重校验 | 动作解析、模型与业务查询边界 | 避免敏感业务问题和执行参数成为模型生成职责，同时保持 Core/Adapter 稳定契约 | 模型零调用、Resolver 冲突、公共 JSON、域 validator 和单动作测试 | 无 |
...
| DeepSeek 模型接入端口及实现 | 通过 `deepseek-v4-pro` 提供能力 ID 选择和回答生成所需的外部模型调用 | 不生成 Employee/Transaction 执行参数，不直接执行动作，不持有业务权限，不接收未通过出域策略的数据 | 无业务状态 | 项目维护者 | 核心与运行 L1 |
...
2. LangGraph 运行时通过混合动作解析节点形成最终候选：本地 Resolver 优先处理需要执行参数的业务动作；没有本地匹配时，模型端口只选择能力 ID；任何候选必须经 `agent-core`、注册表和 Adapter 的确定性校验后才能执行。
...
| LangGraph 运行时 | DeepSeek 模型接入端口 | 无执行参数的能力 ID 选择和回答生成 | 编排节点 → 端口 | 模型实现生成业务执行参数、直接执行工具或接收未通过出域策略的数据 |
| LangGraph 运行时 | 本地动作 Resolver | Employee/Transaction 有限语法识别和结构化参数解析 | 混合动作解析节点 → 域 Resolver | Resolver 访问网络、解析 JWT/角色、记录原问题/参数或绕过 Core validator |
...
| Employee 查询 | 用户问题 | LangGraph 调用 Employee 本地 Resolver 解析动作参数，`agent-core` 与 Adapter 重校验 | Employee Adapter 调用 `employee-service` | `employee-service` | 本地解析歧义、401/403 或超时均失败关闭；动作解析阶段外部模型零调用 | Agent 与 Employee 各记录本边界事件，不记录问题正文或标识值 |
| Transaction 查询 | 用户问题 | LangGraph 调用 Transaction 本地 Resolver 解析动作参数，`agent-core` 与 Adapter 重校验 | Transaction Adapter 调用 `mq-procedure-service` | `mq-procedure-service` | 本地解析歧义、金额边界、401/403 均失败关闭；动作解析阶段外部模型零调用 | Agent 与 Transaction 各记录本边界事件，不记录问题正文或参数值 |
| 不支持或跨业务域问题 | 用户问题 | LangGraph 运行时 | 不调用下游 | 无 | 明确不支持或请求缩小范围；知识动作内部多知识域不在此限制内 | Agent 记录 `unsupported` |
| 参数不合法 | 本地 Resolver 候选或模型能力 ID | 混合动作解析节点、注册表及 Adapter | 不调用下游 | 无 | 返回受控参数错误，可在当前请求内提示用户澄清；业务问题不得回退模型猜参 | Agent 只记录 `invalid_argument` 和非敏感原因码 |
...
sequenceDiagram
    ...
    participant X as 混合动作解析
    participant M as 模型能力选择端口
    ...
    G->>X: 已启用动作描述 + 用户问题
    alt Employee/Transaction 本地确定性匹配
        X->>X: 域 Resolver 解析参数；模型零调用
    else 无本地匹配
        X->>M: 最小化问题 + 已启用能力 ID 描述
        M-->>X: 单个能力 ID（无执行参数）
        X->>X: 仅空参数动作绑定 {}
    end
    X-->>G: 单个 ActionCandidate 或固定失败
    G->>C: 候选动作 + 请求级执行上下文
    C->>R: 查询动作并校验启用状态
    R-->>C: 动作处理器或拒绝
    C->>D: 受控请求 + 原始用户 JWT
...
| `role` → `GrantedAuthority` 映射 | `common-security` | Knowledge、Employee、Transaction | 统一实现，不允许各服务自行字符串解析；受控真实集成与目标部署/生产生效分门禁 | `L2_00_03` v0.4 已关闭 `AUTH-GATE-001/002`；`AUTH-GATE-003` 继续控制目标部署与生效声明 |
| Employee 查询契约 | `employee-service` | Employee Adapter | 优先复用现有只读能力；强类型配置只能收紧，不能扩展公共契约或授权 | 候选接口存在，最终动作映射待业务查询 L1/L2 核实 |
| Transaction 查询契约 | `mq-procedure-service` | Transaction Adapter | 优先复用现有只读能力；强类型配置只能收紧，不能扩展公共契约或授权 | 已确认复用 `POST /txn/search` 并仅注册 `transaction.search`；受控真实联调已验证，默认启用与模型出域仍关闭 |
...
| 模型结构化与生成契约 | DeepSeek `deepseek-v4-pro` 外部模型端口 | LangGraph 运行时和 Knowledge Capability | 供应商差异限制在端口实现；仅接收通过出域判定的数据 | corrected candidate-02与受控Runtime装配已验证，`GATE-038/020/SA-GATE-002`按各自切片关闭；默认仍stub，真实数据出域未授权 |
...
| SA-AD-016 | 采用“本地业务参数 Resolver 优先、模型仅选能力 ID”的混合动作解析；保持最终 `ActionCandidate` 和 Core 执行契约不变 | 模型同时生成动作和业务参数；全部动作均使用本地规则；新增独立解析服务 | v2 PoC 证明模型生成业务参数不稳定；混合方案限制修改范围、避免敏感业务问题进入模型，并保留 Knowledge 等空参数动作的语义选择能力 | LangGraph 动作选择节点、模型端口、业务动作定义与组合根 | 有限语法需显式扩展；未识别或歧义输入会失败关闭 | 无 |
| SA-AD-017 | 将真实外部调用的一次执行许可、领域出域验收和工作包进度分离；P3/P4系统E2E以真实Provider加默认stub模型证明三条主链，Employee/Transaction真实模型出域作为可选实验 | 继续以一次性candidate/wrapper和30次稳定性阈值阻塞系统E2E；直接取消数据出域控制 | 原始需求只要求三条查询链、权限/异常和Knowledge效果结论；分离后既保留真实数据失败关闭，又避免测试治理设施反向阻塞核心目标 | 全局门禁治理、业务出域、P3计划与系统E2E | 可选实验不再提供当前交付完成信号，必须通过独立领域出域门禁恢复 | 无 |
| SA-AD-018 | 当前架构权威只保留稳定行为、边界、通用门禁语义和当前状态；精确run、manifest、hash、candidate、JAR与HEAD仅作为不可变审计证据，不构成可复用授权或后继设计入口 | 将每次实验失败继续提升为新架构决策、工作包和领域专用wrapper；删除历史证据 | 防止测试执行治理形成第二套架构，同时保留付费预算、无自动重试、敏感数据不持久化、失败关闭和结果证据等必要安全约束 | L1/L2当前结论、P3计划权威及未来live实验 | 未来恢复实验须先诊断；仅存在新的未决决策或安全边界时才新建有界门禁，并优先复用通用受控harness | 无 |

## 13. L1 下位文档治理计划

| L1 文档 | 权威范围 | 必须承接的约束 | 关联 L1 | 下位 L2 | 顺序或前置 | 状态 |
|---|---|---|---|---|---|---|
| [`L1_00`《单体 Agent 核心与运行架构 L1》](L1_00_SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md) | Spring `agent-service`、Python `agent-runtime`、LangGraph 唯一编排、`agent-core`、`agent-capability-api`、能力注册运行时、混合动作解析、DeepSeek 模型端口、入口认证和请求级执行 | SA-C-001、002、005、007 至 012、014、018、019、022 | `L1_01` 知识查询能力、`L1_02` 业务查询适配 | `L2_00_00` Spring 接入与运行协同、`L2_00_01` 核心执行与能力注册、`L2_00_02` DeepSeek 模型接入与受控生成、`L2_00_03` 用户角色 Authority Converter | v0.8保持正式评审结论；混合解析/域Resolver、受控Runtime、问题输入安全及真实三Provider + 默认stub系统E2E已验证 | v0.8已评审/已通过；业务真实结果外发实验Deferred，目标环境与生产未生效 |
| [`L1_01`《单体 Agent 知识查询能力架构 L1》](L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) | `agent-knowledge-capability`、`agent-knowledge-adapter`、Knowledge 能力描述与配置、逻辑知识域、检索基础设施消费与集成边界、BGE Embedding/Rerank 消费端口、证据上下文及三层出域策略 | SA-C-001、002、005 至 012、014 至 019、021（003、004、013、020 不适用） | `L1_00` 核心与运行、`L1_02` 业务查询适配 | `L2_01_00` Knowledge 查询流程与配置、`L2_01_01` Knowledge 检索与本地模型接入、`L2_01_02` Knowledge 证据/出域/摘要与效果验证 | v0.7架构评审保持通过；真实检索、多域batch/Evidence、26-case数据集、出域、live P5及系统E2E已验证；P5结论`ineffective` | v0.7已评审/已通过；当前交付完成，默认启用/生效未完成 |
| [`L1_02`《单体 Agent 业务查询适配架构 L1》](L1_02_SINGLE_AGENT_BUSINESS_QUERY_ADAPTER_ARCHITECTURE.md) | Employee/Transaction 能力描述、域内本地参数 Resolver、强类型动作配置、两个业务 Adapter、业务字段出域和权限联调 | SA-C-001 至 014、019、020、022（其中 015 至 018、021 不适用） | `L1_00` 核心与运行、`L1_01` 知识查询能力 | `L2_02_00` 业务查询公共约束/配置/出域、`L2_02_01` Employee Adapter/授权联调、`L2_02_02` Transaction Adapter/授权联调 | v0.4 正式评审结论保持；既有 Adapter/Provider、两个本地 Resolver 与业务问题输入零调用证据有效，`GATE-023/025`关闭 | v0.4 已评审/已通过；真实业务结果出域未完成、未生效 |
...
| P4 集成验证 | 单模块测试通过 | 三条查询链路、真实业务域权限/异常路径和双进程系统闭环通过 | Knowledge使用已验证真实检索；Employee/Transaction使用已验证真实Provider；答案生成默认使用stub，真实业务结果外发另受领域出域门禁 | 对应Provider integration门禁；业务真实模型出域不是本阶段前置 | 禁用失败动作或恢复stub，不回退其他能力 | 无 |
| P5 效果验证 | 知识库链路可用 | 形成真实问题初步效果结论，不要求结论必须有效 | 执行代表性问题集并记录人工rubric | SA-GATE-007 | 保留链路和原始结论，不改判历史 | 无 |
...
| SA-GATE-002 | slice_implementation_completion | Agent 核心模型接入 | 声明真实模型 Runtime 实现切片完成 | 混合解析和域Resolver、candidate-02、answer历史PoC、默认stub下的受控wiring及预算/失败/secret/组合根/生命周期测试通过；项目维护者确认 | 项目维护者/模型提供方 | 模型切片完成声明前 | Closed（2026-08-12；仅实现切片）；允许维护显式provider装配，禁止据此启用目标环境、追加付费调用或外发真实数据 | 核心与模型L2 |
| SA-GATE-003 | integration | Knowledge 查询切片 | 接入当前 ES 税务知识索引 | **已满足并关闭（2026-08-03）**：当前冻结 Profile/正式只读别名下真实 JWT、ES、BGE-M3、Rerank 两域四路及拒绝矩阵已验证；管理接口不可达、日志零泄漏和快照证据齐备 | 项目维护者、`es-query-service` 维护方 | Knowledge P4 联调前 | 已关闭；只允许当前冻结快照的受控真实 Retrieval，默认 Runtime、Evidence、模型出域和生产生效仍受独立门禁控制 | Knowledge 检索适配与检索基础设施契约 L2 |
| SA-GATE-004 | integration | Employee Adapter | 接入 Employee 真实数据 | **已满足并关闭（2026-08-06）**：`employee.detail` 到既有只读详情接口的映射、`admin/viewer` Authority、业务 guard、字段/status、允许拒绝矩阵及 Gateway/Servlet 日志安全证据均已验证 | `auth-service`、`common-security`、`employee-service` | Employee P4 联调前 | 已关闭；允许受控目标配置启用；默认 action、正式 Gateway 路由、模型出域和生产生效不在关闭范围 | Employee Adapter 与权限 L2 |
| SA-GATE-005 | integration | Transaction Adapter | 接入 Transaction 真实数据 | **已满足并关闭（2026-08-06）**：确认 `transaction.search`→`POST /txn/search`；真实 `admin/viewer` 允许及 unknown/missing/malformed/service-token 拒绝；Authority、业务 guard、响应可见性、JSON number→BigDecimal 精确金额、禁止接口、调用计数和日志零泄漏均由 `wp-txn-real-01-20260806T134518Z.json` 及独立代码对照设计复核证明 | `auth-service`、`common-security`、`mq-procedure-service` | Transaction P4 联调前 | 已关闭；允许受控目标配置启用 Transaction 只读动作；默认启用、模型出域与生产生效仍不在关闭范围 | Transaction Adapter 与权限 L2 |
| `SA-GATE-006.KNOWLEDGE` | integration | 当前冻结Knowledge切片真实证据进入DeepSeek | 外发当前目录/Profile/索引快照的知识证据 | 三层只收紧策略、零调用负向、允许路径、引用/grounding及持续校验证据通过 | 项目维护者/知识策略权威/模型提供方 | 当前切片首次真实外发前 | Closed（2026-08-13）；仅覆盖冻结切片，版本或快照变化后重新失败关闭 | Knowledge与模型L2 |
| `SA-GATE-006.EMPLOYEE` | integration | Employee真实结果进入DeepSeek | 外发真实Employee结果 | 字段默认拒绝、交集、有限转换、facts/grounding、零调用负向及至少一次受控允许路径形成安全证据；稳定性重复试验不作为关闭必要条件 | 项目维护者/Employee/模型提供方 | 首次真实Employee结果外发前 | Open；不阻塞本地/真实Provider查询或stub系统E2E，严格禁止真实Employee结果外发 | Business/Employee与模型L2 |
| `SA-GATE-006.TRANSACTION` | integration | Transaction真实结果进入DeepSeek | 外发真实Transaction结果 | 字段默认拒绝、Decimal、facts/grounding、零调用负向、无聚合越界及至少一次受控允许路径形成安全证据；稳定性重复试验不作为关闭必要条件 | 项目维护者/Transaction/模型提供方 | 首次真实Transaction结果外发前 | Open；不阻塞本地/真实Provider查询或stub系统E2E，严格禁止真实Transaction结果外发 | Business/Transaction与模型L2 |
| SA-GATE-007 | closure | P5 知识库效果验证 | 声明第一阶段效果验证完成 | 代表性问题集、成对运行、人工rubric和明确效果结论完成；结论可以是effective或ineffective | 项目维护者 | P5 结束前 | Closed（2026-08-13）；candidate-04形成有效`ineffective`结论，禁止改判或以其证明生产效果 | Knowledge L2 测试设计和效果验证记录 |
...
|---|---|---|---|---|---|
| 模型不能稳定选择合法能力 ID | v3与v4 candidate-01失败；candidate-02只证明冻结10 case可行 | Provider/model/catalog漂移时可能再次误选；不影响本地业务参数解析 | 中 | 模型端口隔离、标准JSON ID exact校验、append-only candidate-02证据；Provider/model/catalog变化须新run/门禁，不复用`GATE-038` | 项目维护者 |
| 本地有限语法不能覆盖用户表述 | Employee/Transaction 问题未匹配、冲突或包含未声明条件 | 请求以 `invalid_argument/unsupported` 失败关闭 | 中 | 代码绑定语法、歧义零调用、基于真实失败样本受控扩展；不得回退模型猜参 | 项目维护者 |
| 当前业务接口过宽 | Adapter 直接暴露通用查询 DTO | 聚合、敏感字段或超大查询被模型触发 | 中 | Adapter 定义有限动作输入，不透传模型任意 DTO | 项目维护者 |
| RBAC 后续配置或代码漂移 | 业务系统绕过共享 Authority、移除动作守卫或发布错误角色配置 | 越权查询 | 中 | 保持统一角色映射、本域角色矩阵、401/403 回归和受控启用；部署/生产生效仍按独立门禁 | 项目维护者 |
...
|---|---|---|---|---|
| Spring `agent-service` 与 LangGraph 运行时的内部请求、取消、健康检查和失败契约（已解决） | `L2_00_00` v0.5 已定版且 Access 契约/Runtime/Spring/E2E 限定实现已验证 | 核心与运行 L1/L2 | `CR-GATE-002`（Closed） | 不再阻塞核心运行 L2 或混合动作解析；后续部署/生产生效仍按独立门禁判断 |
| Knowledge 默认 Runtime 装配与后续快照治理 | 当前冻结Retrieval、模型Runtime、问题输入安全、文档级目录/快照一致性、30/30真实summary及post-consumption闭环已验证 | Knowledge L1/L2、模型接入L2、`es-query-service` | Knowledge范围SA-GATE-006与P3 `GATE-032`已Closed | 当前冻结切片不再阻塞；默认启用仍未授权，目录/Profile/索引/task变化须新门禁和新证据 |
| Employee 新增本地参数 Resolver 的实现与验证 | Employee Adapter 新切片 | 项目维护者/Agent 实施方 | P3 `WP-BUSINESS-LOCAL-RESOLVER-01` | 不重开 `SA-GATE-004`；只阻塞新 Resolver 与其默认装配 |
| Transaction 现有只读接口到最终有限动作的映射（已解决） | 已确认 `transaction.search`→`POST /txn/search`，受控真实联调已闭环 | 项目维护者/Transaction 域 | SA-GATE-005（Closed，2026-08-06） | 不再阻塞受控目标配置；默认启用、模型出域与生产生效仍受独立边界控制 |
| 知识文档级出域元数据的字段、版本与 ES 快照同步机制（已解决） | 首期项目维护者权威、目录/provenance/manifest与当前冻结快照已全量一致 | Knowledge L1/L2、检索基础设施 L2 | P3 `EXT-003`（已满足目录前置） | 当前冻结切片已完成允许路径闭环；后续快照变化须重新验证，不自动继承关闭状态 |
| 知识库 live P5 执行、阶段指标结果和证据答案判定结论（已解决） | 首批26-case representative数据集、gold、授权、快照与哈希已冻结；candidate-04已形成有效`ineffective`结论 | 项目维护者 | SA-GATE-007（Closed，2026-08-13） | 保留原始证据和结论，不补跑、不改判，也不将其外推为生产效果 |

## 16. 约束追踪与验证

| 需求或约束 | L0 落点 | 适用 L1 | 下位验证证据 | 当前状态 |
|---|---|---|---|---|
| FR-01 单体 Agent | SA-C-001、002、019；模块地图 | 核心与运行 | 单逻辑 Agent 清单、唯一 LangGraph 编排和双进程调用测试 | Core、Access、Provider-neutral混合解析及默认stub下的真实Provider Runtime装配验证已通过；完整系统默认装配与生产生效待关闭 |
| FR-02 知识库查询 | Knowledge Capability/Adapter；SA-C-015 至 019、021；SA-GATE-003/006/007 | 知识查询能力 | 分阶段单元/契约/故障测试、目录/快照一致性、出域零调用、受控 ES 集成和效果记录 | 当前冻结 Retrieval、真实策略目录消费、真实证据出域及live P5均已完成；有效效果结论为`ineffective` |
| FR-03 Employee 查询 | SA-C-003、004、006、013、020；Employee Adapter | 业务查询适配 | Adapter、配置收紧、字段脱敏、权限和 Employee 集成测试 | `employee.detail` 受控真实联调与本地 Resolver 已验证；默认启用和模型出域未完成 |
| FR-04 Transaction 查询 | SA-C-003、004、006、013、020；Transaction Adapter | 业务查询适配 | Adapter、配置收紧、字段脱敏、权限和 Transaction 集成测试 | `transaction.search` 受控真实联调已验证；默认启用和模型出域未完成 |
| FR-05 Adapter 模块 | SA-AD-004、011；模块地图 | 知识查询能力、业务查询适配 | 模块依赖和契约测试 | Knowledge、Employee、Transaction Adapter 及业务本地 Resolver 的限定实现已验证 |
| FR-06 有限查询动作 | SA-C-002、005、006、013、016 | 三份 L1 | 未注册/禁用/越界参数、非法知识域/索引/DSL 拒绝测试 | 既有动作目录、边界测试、Provider-neutral 混合裁决及 Employee/Transaction 有限语法已验证；v4 模型 ID 选择待验证 |
| CFG-01 至 CFG-04 | SA-C-006、012、016、020、021；动作、知识域、检索和出域配置机制 | 知识查询能力、业务查询适配 | 强类型绑定、启动失败、只收紧和兼容性测试 | 既有强类型配置与 Resolver 配置限定实现已验证；默认/生产启用仍待完成 |
| SEC-01/02 用户 JWT | SA-C-007；安全机制 | 三份 L1 | 三类能力的 401、业务 JWT 透传和无服务令牌回退测试 | 三类能力的受控真实 JWT/拒绝矩阵已验证；默认/生产生效未证明 |
| SEC-03/04 业务域授权与角色映射 | SA-C-004；SA-GATE-004/005 | 核心运行共享安全、业务查询适配 | `admin`、`viewer` 允许及其他角色拒绝测试；Authority 映射；真实数据可见性 | 共享映射及 Employee/Transaction 受控真实授权闭环已验证；默认/生产生效未证明 |
| SEC-05 服务身份 | SA-C-007 | 三份 L1 | 三类能力缺少用户 JWT 均失败关闭的测试 | 受控联调中的 missing/malformed/service-token 拒绝矩阵已验证；新增动作仍须继承同一边界 |
| EXT-01/02 新增能力不侵入 | SA-C-010；SA-AD-002 | 三份 L1 | 新增模拟能力扩展测试 | 既有 Core/Adapter/Resolver 扩展接缝和注册校验已验证；v4 Provider 编码不得改变该边界 |
| EXT-03 Multi-Agent 预留 | SA-C-014；能力 API 和模型端口 | 核心与运行 | 架构依赖检查 | 已设计，不实施 Multi-Agent |
| 异常处理要求 | SA-C-009、018、019、021；关键流程 | 三份 L1 | 错误分类、双重编排拒绝、出域零调用和故障注入测试 | 既有路径、混合解析、v4严格解码与candidate-02真实PoC已形成限定验证；模型wiring仍待验证 |
| 最小日志要求 | SA-C-011；可观测机制 | 核心与运行 | 日志字段和脱敏测试 | 既有实现、真实联调及candidate-02有限evidence已形成零泄漏证据；模型wiring仍须回归 |
| 测试要求 | 质量目标、门禁和各追踪项 | 三份 L1 | P2 测试设计及 P3/P4 结果 | 已分配 |
| 第一阶段验收标准 | P1-P5 阶段路径 | 三份 L1 | P4 集成与 P5 分阶段效果证据 | P4 的三条受控只读链、Provider-neutral 混合解析、业务 Resolver、受控模型Runtime、真实三Provider + 默认stub系统E2E及完整类型门禁已验证；Knowledge live P5已形成有效但`ineffective`结论。业务真实结果外发实验Deferred，目标部署与生产生效未完成 |
...
v0.4针对性复核已覆盖运行拓扑、检索边界、业务权限和数据出域；v0.5关闭Knowledge消费/提供方物理映射重复权威；v0.6评审覆盖混合动作解析、模型/本地责任分离和既有门禁兼容。v0.12根据跨层门禁审计新增`SA-AD-017`并同步当前实施状态：Knowledge P5已以`ineffective`有效结论关闭，Employee/Transaction真实模型出域继续按域失败关闭；三能力真实Provider + 默认stub系统E2E现已完成。v0.13新增`SA-AD-018`，关闭当前受控配置的`AUTH-GATE-002`并把精确运行资产降为审计证据，不改变未生效和非生产就绪结论。
...
| 2026-08-07 | v0.6 混合动作解析独立评审（五轮评审—修订—复核） | 有条件通过，既有阶段结论不变 | 模型只选 capability ID，业务参数由域内本地 Resolver 形成；最终候选/Core/Provider/授权边界不变；新增发现均已关闭，无未关闭 S0/S1/S2 | 新实现与 v3 PoC 仍按 P3、`SA-GATE-002/GATE-020/035` 控制；不重开既有真实检索或业务联调门禁 |

> 第一行正式评审记录对应 v0.3，第二、三行分别记录 v0.4 与 v0.5 针对性复核，第四行记录 v0.6 混合动作解析评审。本文保持“有条件通过”，条件仅约束仍开放的模型、出域、效果、默认启用和生产生效门禁，不再阻塞已批准下位设计。
> v0.6 的顺序化审计批次使用 `AR-HYBRID-01～03` 与 `FR-HYBRID-01～05`；逐轮冻结发现、修复和终审结果统一记录于 `P3_00` 13.18，本文只保留 L0 结论，避免把下位字段/函数细节复制为第二权威。
> v0.12为门禁治理修订；历史证据和正式评审结论保持不变。当前只将业务真实模型出域移出P3/P4主链，并把`SA-GATE-006`正式拆为三个作用域实例；Employee/Transaction实例仍Open，未授权任何真实外发。
...
- 2026-08-03 增量核实 `common-security` 共享 Authority Converter、三个 Provider endpoint-scoped opt-in/Guard 和本地合成 JWT 测试；后续 Knowledge/Employee/Transaction 受控真实联调另形成限定 evidence，但均不等于默认或生产生效。
- 2026-08-06 基于 `wp-txn-real-01-20260806T134518Z.json`（SHA-256 `1109da47183a822c9ad82fbcc2ef3619163a8089d8f0f420d045e0d30d80f7d1`）及独立代码对照设计复核，确认 Transaction 真实角色矩阵、精确金额链、正式 Gateway 路由、调用计数和日志零泄漏；该证据未调用 DeepSeek、未启用默认 action，也不证明模型出域或生产生效。
- 2026-08-07 `action-selection-v3-20260807-candidate-01`按一次性授权完成30次，结构17/30、预期3/30、真实业务调用0；append-only失败证据与消费标记已核对，不能关闭`GATE-020/SA-GATE-002`，不得补跑或改判。
- 2026-08-10 `action-selection-v4-20260807-candidate-01`按一次性授权完成30次但单case未达标，故仍为不可改写失败证据；corrected candidate-02随后按独立`GATE-038`完成恰好30次并全部通过，`GATE-038`已消费关闭。
- 2026-08-12 `WP-MODEL-RUNTIME-01`在不读取key、不新增live调用的边界内完成默认stub/显式deepseek组合根、Context binding与client生命周期验证；`GATE-020/SA-GATE-002`仅按Runtime实现切片Closed，未授权真实数据出域或目标环境启用。
- 2026-08-13 `knowledge-egress-v2-20260812-candidate-01`按冻结manifest和`P3_00:GATE-043`完成恰好30次真实summary，30/30终态与有效、三案例各10/10且全部安全计数为0；后续post-consumption测试在不改变manifest或append-only证据的前提下完成精确哈希/绑定/终态校验，定向21 passed、Knowledge180 passed/6 skipped、全量非live647 passed/10 skipped及strict mypy268 files通过。该证据仅关闭Knowledge当前冻结切片，不授权业务出域、默认启用或生产生效。

除明确标注的 2026-08-03 增量项外，这些代码是 2026-07-24 的现状证据；实现事实均不具有反向修改本文目标架构的权威。
