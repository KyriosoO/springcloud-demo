...
| REQ-03-005、CON-03-004 | DR-03-007、DR-03-009、DR-03-022 | Java Lifecycle | deadline、调用预算与逐连接 UTC | IMPL-03-003、IMPL-03-007、IMPL-03-010、IMPL-03-011 | TEST-03-008、TEST-03-009、TEST-03-018 | VAL-03-002、VAL-03-003 |
...
| REQ-03-008、CON-03-004 | DR-03-009、DR-03-010、DR-03-021 | Java + dependency owner | 幂等、重试、调用预算与策略证据 | IMPL-03-003、IMPL-03-010、IMPL-03-011 | TEST-03-005、TEST-03-009、TEST-03-017 | VAL-03-002、VAL-03-003 |
...
重试退避消耗原 deadline，不暂停时钟；每次调用先预约预算。`maxAttempts` 和 `callBudgetLimit` 按 DR-03-021 在 execution 创建时连同策略证据固化，后续策略变化只能收紧剩余量，不能增加已持久上限。未提供明确策略时使用安全基线 `maxAttempts=1`、`callBudgetLimit=1`；此时已预约调用后的自动重放不可达，恢复只能失败关闭。已评审 SAFE_REPLAY 能力可在系统硬上限内固化大于 1 的 maxAttempts，使新 attempt 恢复分支可达。
...
| IMPL-03-002 | 建议新增 | Java domain | `agent-service/src/main/java/com/dylan/baseline/agent/execution/domain/` | execution/attempt 状态与纯决策 | DR-03-002、003、005、008、017、018、021 | TEST-03-002、003、013、017 / VAL-03-002 |
| IMPL-03-003 | 建议新增 | Java application | `.../execution/application/ExecutionLifecycleService.java` | 编排生命周期；建议类名，不冻结私有签名 | DR-03-001、004～010、017～021 | TEST-03-001～014、017 / VAL-03-002 |
...
| IMPL-03-010 | 建议新增 | Java port | `.../execution/application/ExecutionAttemptRunner.java` | 事务外执行一个 attempt；后续能力实现该边界 | DR-03-005～012、018、019、021 | TEST-03-007～011、013、014、017 / VAL-03-002 |
