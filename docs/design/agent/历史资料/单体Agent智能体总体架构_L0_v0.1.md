...
| 文档状态 | 已评审 |
| 评审状态 | 已通过；历史评审结论继续有效。2026-07-17 完成本次三文档交叉评审—修复循环，`REV-XDOC-001～002`、`REV-EVID-001` 全部关闭，`AD-23～AD-24`、`DEC-17` 及两份 L1 的同层边界已联合复核通过；文档仍未实施、未生效，各 L2 与系统质量门禁继续有效 |
| 实施状态 | 未实施 |
| 生效状态 | 未生效 |
| 当前版本 | v0.1 |
...
| 治理的 L1 文档 | `docs/design/agent/单体Agent应用与能力架构_L1_v0.1.md`（已创建，已评审/已通过）；`docs/design/agent/检索与索引基础设施架构_L1_v0.1.md`（已创建，已评审/已通过） |
...
| 18 | 2026-07-17 | 第 4～5、13、15～17 节及评价证据入口 | 将当前 DOCUMENT 评价基线同步为 `3.0.0-synthetic`，拆分“固定包已形成”和“目标系统实跑未完成”门禁，并补充冻结包命令勘误入口 | 修复 `REV-XDOC-001`、`REV-EVID-001`，避免继续以 v2 候选池缺口阻断 L2 或把 Oracle 自测误作目标系统实跑 |
| 19 | 2026-07-17 | 文档治理信息、第 13、15～17 节 | 回写三文档交叉复评通过结论并同步两份 L1 状态 | 关闭 `REV-XDOC-002`；明确 L1 可进入受各自前置条件约束的 L2 详细设计，不把文档通过扩大为实现或系统质量通过 |
...
- DOCUMENT 首期质量阈值保持为：`Recall@50 ≥ 0.90`、`NDCG@10 ≥ 0.80`、事实支持率 `≥ 0.90`、关键要点覆盖率 `≥ 0.85`、引用精确率 `≥ 0.98`、事实声明引用覆盖率 `≥ 0.90`、应拒答样本召回率 `≥ 0.95`、错误拒答率 `≤ 0.05`、越权引用或泄漏为0。`EB-L1-003-DOCUMENT-EVAL` 当前权威固定包为 `3.0.0-synthetic`：114个语料块、30个案例（20个可回答、10个应拒答），全部归入单一 `document-default` 语料库和 `internal` 安全等级，不含文档级 ACL；20个 Recall@50 计分案例的最小有效候选池为56、最少干扰项为55，7个访问拒绝案例要求零候选。固定包、v3 Schema/验证器和 SHA-256 已形成并通过静态校验，满足 DOCUMENT/权限安全 L2 的固定评价工件前置条件；`validation_report.json` 仍标记 `target_system_quality_run_executed=false`，因此不代表系统已经跑分或具备生产业务代表性。命令用途以 `docs/design/agent/evidence/eb-l1-003/README.md` 的冻结包勘误为准。
- 生产 SLO、容量、网络隔离和高可用拓扑尚未给出，不回退 L0 已通过结论，但阻塞生产架构批准。
...
具体延迟、吞吐数值仍须由两份 L1 基于本机基准补齐；DOCUMENT `3.0.0-synthetic` 固定模拟评价包、指标精确定义、有效 Recall@50 候选池和可重算报告契约已形成，但尚未在实际目标制品上跑分。固定包形成解除评价工件前置，不等于质量达标；系统质量放行前仍须生成通过 v3 验证器的目标系统实际报告，生产批准前还须补充经授权脱敏的业务代表性扩展版本。
...
|---|---|---|---|---|---|---|
| docs/design/agent/单体Agent应用与能力架构_L1_v0.1.md | Java Agent、Python Runtime、能力体系、Adapter、执行状态与 Java/Python 契约 | AD-01～AD-14、AD-16～AD-24；DEC-01～DEC-17 | 检索与索引 L1 | 契约治理、执行生命周期、QUERY/AGGREGATE、DOCUMENT 编排、Adapter、安全与可观测 L2 | AD-23～AD-24/DEC-17 的查询侧权限组合与未来 ACL 演进已联合复评；对应 L2 仍须满足各自前置门禁 | 已创建；已评审/已通过；未实施/未生效 |
| docs/design/agent/检索与索引基础设施架构_L1_v0.1.md | 原子检索、索引/向量生命周期、受控文档源、ES Alias/重建、Embedding 和 ES 韧性 | AD-01、AD-06～AD-09、AD-13、AD-15～AD-20、AD-23～AD-24；DEC-04、DEC-05、DEC-07～DEC-08、DEC-10～DEC-13、DEC-17 | Agent 应用与能力 L1 | 查询/管理契约、文档源录入清单、索引生命周期、权限过滤、Alias/回滚、性能与运维 L2 | 文档源、首期语料库权限及评价工件架构基线已确认；进入各 L2 正式评审仍须满足对应前置条件 | 已创建；已评审/已通过；未实施/未生效 |
...
|---|---|---|---|---|---|---|
| 0. 架构定基线 | 已确认需求与本 L0 架构内容 | L0 经正式评审 | 评审边界、所有权、约束和待决项 | 独立评审 S0/S1（或作者自审等价 P0/P1）关闭；评审结论明确 | 仅文档，无运行时影响 | 无 |
...
以下事项均不回退本 L0 已通过结论；其截止条件决定对应 L2、实现验收或上线门禁。
...
| DOCUMENT 固定评测集工件与目标系统实跑 | 检索和生成验收、结果复现 | 产品/检索/AI/安全负责人 | 固定工件已完成：`EB-L1-003-DOCUMENT-EVAL` `3.0.0-synthetic`（2026-07-17）；目标系统实跑待系统质量放行前完成 | 否（固定包已解除 DOCUMENT/权限安全 L2 正式评审前置）；是（实际报告未形成，阻断系统质量放行）。包校验和 Oracle 自测不得替代目标制品实跑；业务代表性扩展仍属生产门禁，命令解释见 `docs/design/agent/evidence/eb-l1-003/README.md` |
| 文档来源、稳定 ID、业务版本、资源访问属性和撤权事件契约 | 索引生命周期、授权组合与引用稳定性 | 文档域/安全/检索负责人 | 已完成架构语义冻结并通过联合复评（2026-07-17）：受控原始存储+版本化录入清单、独立服务身份脚本、单一语料库/统一策略、撤权围栏成功点；字段级清单/接口由 L2 承接 | 否；`EB-RETR-L1-001` 的架构前置证据已形成，检索 L1 已评审/已通过 |
...
| AD-24 | 两份 L1 | 单一语料库、统一数据安全/访问策略、RBAC+DOCUMENT+语料库权限组合、召回前硬过滤、未来权限码 ACL 兼容迁移和越权测试 | 两份 L1 已同步并通过联合复评；字段级契约与运行证据待对应 L2 |
| DEC-01～DEC-17 | 按第 13 节映射 | L1 决策映射表、L2 交付及测试证据 | DEC-15 已由 DEC-16 替代；DEC-17 已同步两份 L1 并通过联合复评，运行证据待 L2 |
...
| 2026-07-17 | 三文档交叉评审—第 1 轮修复后复评 | 需最小治理修订 | `REV-XDOC-001`、`REV-EVID-001` 已关闭，L0→两份 L1 的 AD-01～AD-24 追踪完整；`REV-XDOC-002` 仍有4处旧“草稿/未评审”措辞 | 已按同一发现 ID 完成最小状态措辞修订，不影响架构语义 |
| 2026-07-17 | 三文档交叉评审—第 2 轮复评 | 通过（`L0→L1` ready） | 原冻结发现全部关闭，治理状态、同层边界和 v3 评价门禁一致，无新增S0/S1/S2 | 两份 L1 已评审/已通过；未实施/未生效、目标制品实跑、业务代表性、L2 外部契约及生产门禁继续有效 |
