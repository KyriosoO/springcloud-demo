...
| 替代关系 | v1.0 已评审通过并成为当前架构设计基线；v0.1 作为冻结历史保留，不再继续修订 |

## 2. 版本迁移说明

| 项目 | 说明 |
|---|---|
| 来源文档 | [单体Agent智能体总体架构_L0_v0.1.md](./历史资料/单体Agent智能体总体架构_L0_v0.1.md) |
...
| 2 | 2026-07-17 | 第 9、14 节 | 恢复跨 Agent/检索的唯一重试责任与共享预算约束，并将生产业务代表性评价门禁纳入下位追踪 | 修复 `REV-XDOC-101`、`REV-XDOC-102`，防止合成集实跑被误作生产质量放行或跨层乘法重试 |
| 3 | 2026-07-17 | 文档治理、第 11、16 节 | 回写第2轮独立复评通过结论及报告入口 | 三份 v1.0 逐文档与交叉评审均无剩余 S0/S1/S2 |
| 4 | 2026-07-17 | 文档治理、第 15～16 节 | 将编写期“未评审”表述明确限定为历史候选稿状态，并回写第3轮复评结论 | 修复 `REV-GOV-103`，消除当前治理状态与历史内部自评记录的歧义 |
| 5 | 2026-07-20 | 第 2 节 | 将 v0.1 来源链接修正为冻结历史资料的实际位置 | 修复 `REV-GOV-203`，恢复来源基线的可导航性 |
...
- 每条外部依赖只有一个内部重试责任方：Java Agent 只在调用方可见副作用尚未产生且查询契约允许时对服务入口进行有界重试；`es-query-service` 唯一负责其 Elasticsearch/Embedding 内部重试。两层全部尝试必须计入同一总 deadline、调用预算并返回尝试诊断，禁止乘法重试。
...
|---|---|---|---|---|
| `单体Agent应用与能力架构_L1_v1.0.md` | Java Agent、Python Runtime、能力体系、Adapter、执行/会话、跨运行时契约 | 逐项说明 AD-01～AD-24、DEC-01～DEC-17；非本域责任必须显式标注边界 | 契约、生命周期、QUERY/AGGREGATE、DOCUMENT 编排、Adapter、安全、状态、可观测 | v1.0 已评审/已通过 |
| `检索与索引基础设施架构_L1_v1.0.md` | 原子检索、受控文档源、索引/向量生命周期、Alias、Embedding、检索韧性 | 逐项说明 AD-01～AD-24、DEC-01～DEC-17；非本域责任必须显式标注边界 | 查询/管理契约、候选安全、文档源、表示、发布、技术状态、性能、运维、评价 | v1.0 已评审/已通过 |
...
| 生产质量代表性门禁 | 两份 L1 | v3 目标制品实际报告只能证明固定合成集质量；生产批准前还须形成经授权脱敏、分层抽样并由责任方签署的业务代表性扩展结果 |

## 15. 编写期内部自评

内部自评只检查 v1.0 的表达、完整性和与冻结 v0.1 的语义一致性，不构成正式批准。以下记录发生在正式评审之前；当前治理状态以第 16 节为准。
...
| 2 | 严格结构、稳定 ID、链接、同层边界、质量门禁和治理状态 | 严格校验0错误0警告；AD/DEC 集合与 v0.1 相等；两份 L1 追踪完整，链接有效，未发现 P0/P1/P2 | 内部自评通过；当时为候选稿，当前状态见第 16 节 |

## 16. 正式评审记录

| 日期 | 评审类型 | 结论 | 问题摘要 | 处理状态 | 报告 |
|---|---|---|---|---|---|
| 2026-07-17 | 逐文档评审 + 三文档交叉评审，第3轮复评 | 通过（`L0→L1` ready） | `REV-XDOC-101`、`REV-XDOC-102`、`REV-GOV-103` 均已关闭；无剩余 S0/S1/S2/S3 | 已完成；未实施/未生效及各 L2/生产门禁继续有效 | [L0 v1.0 评审报告](./单体Agent智能体总体架构_L0_v1.0_评审报告.md) |
