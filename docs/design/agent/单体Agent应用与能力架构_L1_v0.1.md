...
| 文档状态 | 已评审 |
| 评审状态 | 评审受阻；3 轮正式评审—修复循环已关闭全部文档发现。评审后已确认 DeepSeek 目标配置、测试环境 Provider 连通性、公开服务条款及数据使用策略、API 契约兼容性与 L1 最小效果 PoC、维护人 Dylan、应用侧字段出域机制、内部服务身份/短期能力凭据机制及 DOCUMENT 首期临时指标与阈值；`EB-L1-001` 已关闭，固定评价集工件仍未形成，当前仅剩 1 项决策/证据门禁未解除，需复评后才能改变结论 |
| 实施状态 | 未实施 |
| 生效状态 | 未生效 |
| 当前版本 | v0.1 |
...
| DOCUMENT | 流程需求已确认 | 完整证据化 RAG 编排 | 需固定评价集工件和检索查询面；首期临时指标与阈值已确认 | L2/联合门禁 |
...
- LLM 目标配置已确认为 DeepSeek、模型 `deepseek-v4-flash`、API Base URL `https://api.deepseek.com`，测试环境 LLM Provider 连通性已通过。用户已确认接受 DeepSeek 公开服务条款及其已披露的数据收集、训练/优化、中国境内存储、非固定保留期、服务提供商/集团处理和磁盘上下文缓存事实，并确认继续以 Java 目标级字段出域白名单、转换、最小化和默认拒绝作为应用侧安全边界；不得把供应商删除能力当作安全控制。API 契约4项实测和当前模型端口7项 L1 最小效果 PoC 均通过，`EB-L1-001` 已关闭。内部服务身份/短期能力凭据机制已确认为固定服务密钥 + Java 不透明短期执行凭据 + Java 服务端校验；DOCUMENT 首期临时指标与阈值已确认，但固定评价集工件尚未冻结。当前仅剩 `EB-L1-003` 这1项门禁阻塞 L1 正式通过。
...
| Java 数据出域策略执行器 | 在 Adapter/文档数据完成行级或资源级授权后，按业务域、能力、使用目的和发送目标执行硬拒绝、转换及转换后字段白名单，并输出策略版本/范围审计元数据 | auth 决策、Adapter/文档结果、版本化策略配置 | Python、DeepSeek、日志和最终用户均不能接收未经对应目标策略处理的数据；Python 不拥有或放宽策略 | 新业务域、能力和发送目标可增加独立策略而不修改既有能力流程 |
...
| Tool API | Java Tool Gateway | Python Runtime | 执行已注册的只读 QUERY、AGGREGATE 或检索操作 | 同时携带固定 Runtime 服务密钥与 Java 签发的不透明短期 `capabilityId`；服务端绑定 execution/fence/tool/租户或用户范围/过滤摘要/截止时间并失败关闭；只读重放按 tool-call 幂等 |
...
| DeepSeek `deepseek-v4-flash`（API Base URL `https://api.deepseek.com`） | DeepSeek 外部 AI 服务；目标配置、公开服务条款接受和测试环境证据由用户确认 | 理解、规划、生成和候选判断 | 输出始终视为不可信；失败不产生最终成功状态；仅可发送 Java 按 `deepseek` 目标策略生成的数据视图；不依赖供应商删除保证安全 | 基本调用链、JSON、Tool、上下文、错误语义及 L1 最小效果 PoC 已验证；仍受总截止时间/Token/调用次数约束，供应商政策变化、生产容量、备选/降级和回归验证由对应 L2/生产门禁承接 |
...
| 字段出域策略、版本和生效范围 | Java Agent 安全策略配置；Dylan 负责文档及内部模块治理 | 仅经授权的策略发布路径；业务响应不能动态扩大策略 | Java 数据出域策略执行器和安全审计读取；Python 只接收执行后的目标数据视图 | 策略按版本和范围发布、回滚；未声明业务域/能力/目的/目标或新增字段一律默认拒绝 |
...
| QUERY | 能力路由 | Python形成受约束查询计划；Java Tool 校验并选域 Adapter；业务响应先经 Java 对目标执行字段出域策略，再交 Python 组织并由 Java 终检 | Java/MySQL | 非法计划拒绝；依赖超时在总预算内有限重试；无数据不伪造成失败；策略缺失或字段未声明时拒绝出域 | 计划摘要、Tool 调用、域、策略版本、字段决策、耗时、结果规模 |
| AGGREGATE | 能力路由 | Python形成维度/指标/过滤计划；Adapter执行业务域聚合；Java验证结果契约并按目标生成最小化数据视图 | Java/MySQL | 不支持的指标/维度显式拒绝；禁止在 Agent 内对无限明细兜底聚合；策略不得由模型临时放宽 | 聚合语义、过滤摘要、策略版本、调用结果和预算 |
...
- 数据外发严格遵循 `auth-service` 行级/资源级授权 → Java Adapter/受控文档读取 → Java 字段出域白名单 → 脱敏、匿名化、聚合或截断 → Python Runtime → DeepSeek；行级/资源级授权与字段出域控制是前后相继、不可互相替代的两道门禁。
- 字段出域策略至少以业务域、能力、使用目的和发送目标为键；`deny` 优先级最高，`allow` 表示转换后的输出字段白名单，未声明字段和业务接口新增字段默认拒绝。DeepSeek、日志和最终用户必须分别评估目标策略，不能用一个目标的许可推导另一个目标获准。
...
|---|---|---|---|---|
| 授权安全 | 权限前置且不可扩权 | 所有业务/检索调用均有机器可执行授权约束；Runtime→Tool 同时具备服务级身份和逐执行能力凭据；所有出域字段均命中对应目的/目标的转换后白名单；无可信决策、凭据/过滤不可表达、策略缺失或字段未声明即拒绝 | 越权、撤权、服务密钥错误/轮换、凭据伪造/过期/重放、fence 变化、目标隔离、新增字段默认拒绝测试 | 不得后置过滤替代前置过滤；不得仅凭固定密钥授权；不得默认放行或跨目标复用许可 |
...
生产 SLO 和容量的数值阈值尚无权威输入。DOCUMENT 首期临时质量阈值已由用户确认，但尚不能脱离固定评价集、指标定义和版本形成可复现基线；这些剩余工件必须在对应 L2 正式评审前形成，并在生产批准前由责任方签署。
...
| APP-DEC-12 | Java 在授权取数后按业务域、能力、目的和目标执行版本化字段出域策略；转换后白名单与默认拒绝为强制语义 | Python 自主脱敏；接口返回什么就发送什么；黑名单优先 | 防止模型编排放宽策略及业务接口新增字段静默外泄，并支持 DeepSeek、日志、最终用户独立治理 | Adapter、Tool、Runtime、模型调用、日志、结果终检与审计 | 策略配置错误可能导致误拒绝或派生字段歧义 | 具体 Schema/发布机制由安全 L2 固化；改变强制语义时新建 ADR |
...
| APP-DEC-14 | DOCUMENT 首期采用 Recall@50、NDCG@10、事实支持、要点覆盖、引用精确/覆盖、拒答及零越权共9项临时质量门禁，所有门禁逐项通过 | 不设数值门禁；只使用加权总分 | 覆盖检索、排序、回答、引用、拒答和安全闭环，避免平均分掩盖单项失败 | DOCUMENT 编排、检索联合验收、测试与发布 | 样本偏差、指标口径不一致或数据泄漏分层被总体均值掩盖 | DOCUMENT L2 冻结公式、评价集、标注与版本；阈值变化须记录版本决策，改变安全零容忍语义时新建 ADR |
...
| DOCUMENT 应用编排与证据 L2 | 预处理、多路编排、去重合并、RRF、精排、业务校正、上下文、生成、引用/事实/完整性校验 | AD-06、AD-09、AD-15、AD-16、AD-17、AD-20；APP-DEC-07、APP-DEC-08、APP-DEC-14 | 检索查询面联合契约、文档源契约、固定且版本化的评价集工件 | 第 16.4.3 节9项门禁逐项通过；按业务域/关键场景分层报告；零越权 | 原子召回和索引实现 |
| Agent 权限、结果安全与审计 L2 | 主体/服务身份、固定服务密钥生命周期、不透明短期能力映射/撤销/轮换、能力授权、过滤收紧、字段出域策略 Schema 与求值语义、目标隔离、策略版本/范围/发布回滚、结果终检、脱敏、强制审计事实的语义与保留规则 | AD-08、AD-09、AD-16、AD-17、AD-20；APP-DEC-08、APP-DEC-12、APP-DEC-13 | auth-service、来源资源属性契约、数据分类规则与秘密注入能力 | 越权、撤权、服务密钥错误/轮换、凭据伪造/过期/重放、终态/fence 失效、多实例一致性、硬拒绝优先、新增字段默认拒绝、目标隔离、审计完整性和数据泄漏测试 | 用户和权限规则管理；审计事实和能力映射的物理表/缓存结构 |
...
| Agent 韧性、可观测、部署与扩展验证 L2 | 截止时间、重试/隔离/降级、遥测端口、就绪、配置、部署、资产隔离和未来能力结构测试 | AD-01、AD-07、AD-10、AD-11、AD-13、AD-14、AD-19；APP-DEC-01、APP-DEC-09、APP-DEC-10、APP-DEC-11、APP-DEC-13 | 环境、秘密注入/轮换能力、SLO/容量输入 | 无 alpha 依赖与名称冲突、无 `es-document-service`、双运行时拓扑、密钥轮换、故障注入、遥测替换、发布回滚、扩展样例 | 指标/追踪后端和生产 HA 专项设计 |
...
| 授权、字段出域、结果安全、隐私和审计 | 权限、结果安全与审计 | 是 | 否 | 统一字段出域策略及强制审计事实的业务语义和保留规则，不定义物理表结构 |
...
| 检查项 | 结论 | 证据/说明 |
|---|---|---|
| L0 约束承接完整 | 是 | 第4节逐项映射 AD-01～AD-20；第12节继承 DEC-01～DEC-14 的适用决策，其中评审后新增 DEC-14 由 APP-DEC-13 承接并待正式复评 |
...
| DOCUMENT 评价结果不可复现 | 首期指标与阈值已确认，但没有固定语料快照、问题样本、相关性/答案/权限标注和版本 | 同一方案在不同样本或口径下得出不可比较结论，错误答案上线 | 建立分域、分场景、版本化评价集，冻结指标公式、标注校准与报告版本 | Dylan（协调 AI、检索和业务验收方） |
| DeepSeek 公开政策变化或已接受的数据处理条件超出应用侧策略 | 供应商调整数据收集、训练/优化、存储地域、保留、子处理或缓存政策，或应用侧错误放行超出 `deepseek` 目标策略的数据 | 合规风险、数据外泄、能力关闭或 L2 返工 | 固化条款基线和变更复核；Java 目标级白名单、转换、最小化与默认拒绝；不依赖供应商删除；不满足最新策略时关闭对应模型能力 | Dylan / 安全 / Python AI 负责人 |
...
|---|---|---|---|---|
| DOCUMENT 固定评价集工件（指标与首期临时阈值已确认） | DOCUMENT 方案选择、结果复现和发布门禁 | AI/检索/业务负责人 | Agent 与检索 L1 正式评审前 | 是；仍缺语料快照、问题样本、相关性/答案/权限标注、指标公式和版本证据 |
...
|---|---|---|---|
| LLM 目标配置 | 供应商 DeepSeek；模型 `deepseek-v4-flash`；API Base URL `https://api.deepseek.com` | 2026-07-16 用户明确确认 | 选型部分已完成 |
| LLM Provider 连通性 | 测试环境连通性测试通过；只确认基本调用链可达 | 2026-07-16 用户明确确认 | 完成 `EB-L1-001` 的基础连通性部分；契约、错误语义和效果由同表后续实测证据补齐 |
| DeepSeek 服务条款与数据使用策略 | 接受 DeepSeek 公开服务条款已披露的数据收集、训练/优化、中国境内存储、非固定保留期、服务提供商/集团处理和磁盘上下文缓存事实；应用侧继续执行目标级白名单、转换、最小化、默认拒绝，并且不依赖供应商删除能力 | 2026-07-16 DeepSeek 官方隐私政策/开放平台条款/上下文缓存说明及用户明确确认 | 完成 `EB-L1-001` 的供应商处理事实与风险接受部分；供应商政策变化转为持续风险而非当前 L1 阻断 |
| DeepSeek API 契约与 L1 最小效果 PoC | JSON Output、Tool Call、多轮上下文返回成功，无效模型返回可识别的 HTTP 400 `invalid_request_error`；当前模型端口 QUERY/AGGREGATE/DOCUMENT 共7个合成场景全部通过、无修复重试，最大耗时约8.35秒 | 2026-07-16 纯合成数据实测 | 完成 `EB-L1-001` 的契约兼容性和效果可行性部分；不替代 DOCUMENT 固定评价集或生产 SLO/容量验证 |
| 文档及模块维护责任 | Dylan | 2026-07-16 用户明确确认 | 原“实名维护人未落实”阻塞已关闭；不改变外部系统各自的责任人 |
| 应用侧 LLM 数据出域机制 | `auth-service` 行级/资源级授权 → Java Adapter 获取数据 → Java 字段出域白名单 → 脱敏/匿名化/聚合/截断 → Python Runtime → DeepSeek；策略按业务域、能力、目的、目标、版本和范围管理；新增及未声明字段默认拒绝 | 2026-07-16 用户明确确认 | 完成 `EB-L1-001` 的应用侧控制部分；与供应商公开事实、风险接受、API 契约和效果实测共同形成关闭证据 |
...
| DOCUMENT 首期临时质量门禁 | `Recall@50 ≥ 0.90`、`NDCG@10 ≥ 0.80`、事实支持率 `≥ 0.90`、关键要点覆盖率 `≥ 0.85`、引用精确率 `≥ 0.98`、事实声明引用覆盖率 `≥ 0.90`、应拒答样本召回率 `≥ 0.95`、错误拒答率 `≤ 0.05`、越权引用或泄漏为0 | 2026-07-16 用户明确确认 | `EB-L1-003` 的指标与阈值部分完成；固定评价集工件仍未形成，因此该阻塞尚未关闭 |
...
应用侧必须遵循 `auth-service` 行级/资源级授权 → Java Adapter 或受控文档读取 → Java 字段出域策略 → 最小化数据视图 → Python Runtime → DeepSeek。Java 是唯一策略执行点，Python 和模型不能读取被拒绝字段或放宽策略。

字段出域策略按业务域、能力、使用目的、发送目标、版本和生效范围治理，并保持以下不变量：`deny` 硬拒绝优先；转换后的字段仍须命中白名单；未声明字段、派生字段和业务接口新增字段默认拒绝；DeepSeek、日志和最终用户的许可相互独立；审计不得记录被拒绝的原始敏感值。

安全 L2 负责冻结配置 Schema、字段路径与转换语义、策略发布/回滚、缓存一致性和失败关闭测试。2026-07-16 已完成以下门禁证据：DeepSeek 官方隐私政策、开放平台条款和上下文缓存说明确认了数据收集、训练/优化、中国境内存储、非固定保留期、服务提供商/集团处理及磁盘缓存事实；用户明确确认接受上述公开服务条款和数据使用策略；API 契约4项实测与当前模型端口7项 L1 最小效果 PoC 均通过。实测仅使用合成数据，证明接口与模型端口在 L1 层的可行性，不替代生产 SLO/容量或 DOCUMENT 固定评价集。基于“已知事实 + 明确风险接受 + 应用侧不可扩权门禁”，`EB-L1-001` 已关闭；供应商政策变化继续作为第 16.1 节持续风险管理。
...
测试未发送个人数据或业务数据；文档只保留脱敏结果摘要，不记录 API Key 和原始请求/响应正文。

#### 16.4.2 内部服务身份与短期能力凭据

首期只读 Tool 链路采用“固定服务密钥 + Java 生成的不透明短期执行凭据 + Java 服务端校验”。固定密钥证明 Python Runtime 服务身份，短期凭据绑定 execution、fence、允许 Tool、主体/租户范围、过滤约束和有效期；任一校验失败即拒绝。凭据不得进入 Prompt、模型上下文或遥测数据，未来写能力不得直接复用该只读机制。
...
`EB-L1-003` 的指标与阈值部分已完成，但仍须形成由产品、业务、安全、检索和 AI 责任方共同确认的版本化评价集工件。DOCUMENT L2 负责冻结语料与问题版本、相关性/答案/引用/权限标注、样本分层、指标公式、标注质检及报告版本；工件形成前该阻断不关闭。
