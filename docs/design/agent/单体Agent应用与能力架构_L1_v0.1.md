...
| 文档状态 | 已评审 |
| 评审状态 | 已通过；3轮历史正式评审—修复循环、`EB-L1-001～003`及`REV-L1-004`关闭记录继续有效。2026-07-17 本次三文档交叉复评已确认 `AD-23～AD-24`、`DEC-17`、`APP-DEC-17` 与检索 L1 一致，`REV-XDOC-001～002`、`REV-EVID-001` 全部关闭；文档仍未实施、未生效，各 L2 前置门禁不因 L1 通过而自动满足 |
| 实施状态 | 未实施 |
| 生效状态 | 未生效 |
| 当前版本 | v0.1 |
...
| 关联 L1 | [检索与索引基础设施架构 L1](./检索与索引基础设施架构_L1_v0.1.md)（已评审/已通过）；双方共同约束检索查询面和文档源语义，但不共享实现所有权 |
...
| 23 | 2026-07-17 | 第 5、11、13、16～17 节 | 将当前 DOCUMENT 权限安全与 Recall@50 评价基线由 v2/`2.1.0` 后续版本同步为权限语义对齐的 `3.0.0-synthetic`，明确固定包已形成但目标系统尚未实跑 | 根据用户授权修复 `RET-O-12`，使两份 L1 共享单语料库权限评价门禁；保留 v2 历史记录和系统质量实跑门禁 |
| 24 | 2026-07-17 | 第 16～17 节及评价证据入口 | 补充冻结包外部命令勘误入口，明确 `self_test.py` 仅验证 Oracle/验证器而非目标系统实际报告 | 修复 `REV-EVID-001`，防止静态包校验或 Oracle 自测被误作系统质量证据 |
| 25 | 2026-07-17 | 文档治理信息、第 3、13、16～17 节 | 回写三文档交叉复评通过结论并同步关联检索 L1 状态 | 关闭 `REV-XDOC-002`，确认同层职责、权限语义和评价门禁一致；未扩大为 L2、实现或系统质量通过 |
...
| DOCUMENT | 流程需求已确认 | 完整证据化 RAG 编排 | 权限语义对齐的固定模拟评价包 `3.0.0-synthetic` 及可重算报告契约已形成，Recall@50 有效候选池门禁已满足；仍需冻结检索查询面并在目标制品上实际跑分 | L2/联合门禁 |
...
|---|---|---|---|---|
| 检索与索引基础设施架构 L1（已评审/已通过） | 原子 BM25/Dense/可选 Sparse、受控文档源管理、检索执行、索引/向量 CRUD、Mapping、Alias、技术版本、重建和 ES 韧性 | 定义检索意图，将可信主体/租户、RBAC、DOCUMENT 能力和语料库访问权限组合为授权硬过滤，负责召回组合、结果融合、业务排序、上下文和答案验证 | Java Tool 以检索查询面提交单一语料库受约束请求并获取带稳定文档/分块标识、来源版本及得分语义的候选 | 本文不定义物理索引/打分实现；对方不维护用户/角色/RBAC，不实现问题理解、RRF、Reranker、会话、LLM 生成或业务规则 |
...
生产 SLO 和容量的数值阈值尚无权威输入。DOCUMENT 首期临时质量阈值已由用户确认；`3.0.0-synthetic` 已提供与首期单语料库权限语义一致的固定评价集、指标定义、强制分层和可重算报告契约。该包含114个语料块、30个案例，20个可回答样本的最小有效候选池为56、最少干扰项为55，并覆盖7个访问拒绝零候选案例；固定包门禁已满足。其验证报告同时明确 `target_system_quality_run_executed=false`，因此只解除 DOCUMENT/权限安全 L2 的固定包前置条件，不证明 Recall@50 或其他系统质量门禁已经通过；系统质量放行前必须在实际目标制品上跑分，生产批准前还须由责任方签署业务代表性扩展结果。
...
| DOCUMENT 应用编排与证据 L2 | 预处理、单一语料库权限约束、多路编排、去重合并、RRF、精排、业务校正、上下文、生成、引用/事实/完整性校验 | AD-06、AD-09、AD-15～AD-17、AD-20、AD-23～AD-24；APP-DEC-07、APP-DEC-08、APP-DEC-14、APP-DEC-17 | 检索查询面联合契约、受控文档源/录入清单语义及固定评价包 `3.0.0-synthetic`；目标系统实跑环境和制品版本在联合质量验收前冻结 | 语料库越权和撤权为0；第 16.4.3 节9项门禁逐项通过；报告由 v3 Schema/验证器重算并按单一语料库域/场景/标签分层 | 原子召回、索引实现和文档级 ACL |
...
| 检查项 | 结论 | 证据/说明 |
|---|---|---|
| L0 约束承接完整 | 是（已联合复评） | 第4节逐项映射 AD-01～AD-24；第12节继承 DEC-01～DEC-17 的适用决策。AD-21/DEC-15/APP-DEC-15 保留为历史记录并分别由 AD-22/DEC-16/APP-DEC-16 替代；AD-23～AD-24/DEC-17/APP-DEC-17 已联合复评通过 |
| 未侵入关联 L1 | 是（已联合复评） | 第 3.4、6.3、7.2 明确只消费检索查询面；受控录入、物理索引、原子召回和管理面归检索 L1。双方首期语义和责任边界已通过联合复评 |
...
| 模拟评价集业务代表性或目标实跑不足 | `3.0.0-synthetic` 已满足单语料库权限、零候选拒绝、Recall@50 候选池和干扰项门禁，但仍是纯模拟数据且尚未在目标制品上实跑 | 静态包校验或 Oracle 自测被误作系统质量结果，仍可能高估真实长尾、噪声、容量和生产质量 | DOCUMENT L2 使用 v3 固定包冻结报告契约和测试语义；系统质量放行前完成目标制品实跑，生产批准前再新增经授权、脱敏、分层抽样的业务代表性扩展版本并独立签署 | Dylan（协调 AI、检索、安全和业务验收方） |
...
|---|---|---|---|---|
| DOCUMENT 固定评价集工件（指标与首期临时阈值已确认） | DOCUMENT 方案选择、权限安全语义、结果复现和发布门禁 | AI/检索/业务/安全负责人 | 已完成：`EB-L1-003-DOCUMENT-EVAL` `3.0.0-synthetic`（2026-07-17） | 否；114个语料块、30个案例、单一逻辑语料库/安全等级、7个访问拒绝零候选案例、v3 Schema/验证器及 SHA-256 已冻结；`EB-L1-003` 历史关闭状态保持有效 |
| DOCUMENT v3.0 目标系统实跑与联合质量报告 | 证明实际召回、排序、回答、引用、拒答和权限执行满足9项门禁 | AI/检索/业务/安全负责人 | 固定包已形成；系统质量放行前在实际目标制品上运行 `3.0.0-synthetic` 并生成通过 v3 验证器的版本化报告 | 否（不再阻断 DOCUMENT/权限安全 L2 正式评审）；是（阻断系统质量放行）。Oracle 自测和静态包校验不得替代实际运行 |
...
| 端到端超时、并发、Token/TopK 及生产 SLO/容量/网络/HA | 量化质量预算和生产部署 | 产品/运维/架构负责人 | L2 性能基线前；生产批准前最终确认 | 否（不回退本 L1 已通过结论）；是（对应 L2 与生产批准） |
...
| 首期文档源与语料库级权限 | 受控原始存储和版本化录入清单是正文、稳定 ID、单调业务版本和录入批次权威；获授权操作人经独立服务身份脚本录入。首期一个预配置逻辑语料库，统一数据安全等级和访问策略；Java 组合可信主体/租户、RBAC、DOCUMENT 能力和语料库权限，检索只执行；不支持文档级 ACL，未来优先使用 RBAC 有效权限与文档权限码 | 2026-07-17 用户明确确认；L0 `AD-23～AD-24`、`DEC-17`；本次三文档联合复评 | `EB-RETR-L1-001` 的架构语义证据已形成且新增决策已联合复评通过；字段级录入清单、管理/查询契约和运行证据由 L2 承接 |
...
| DOCUMENT 固定模拟评价包 | `EB-L1-003-DOCUMENT-EVAL` `3.0.0-synthetic`：114个语料块、30个案例（20个可回答、10个应拒答），统一到单一 `document-default` 语料库和 `internal` 安全等级，文档级 ACL 字段为0；7个访问拒绝案例要求零候选，Recall@50 计分案例最小有效候选池56、最少干扰项55；v3 Schema/验证器重算总体/分层指标、安全事件与总门禁 | 2026-07-17 用户授权核实 `RET-O-09` 并形成 v3.0 固定包；包目录 `docs/design/agent/evidence/eb-l1-003/document-eval-v3.0.0/`，冻结包命令勘误见 `docs/design/agent/evidence/eb-l1-003/README.md` | 固定包和验证器自测通过，满足当前 L2 固定评价工件前置条件；`target_system_quality_run_executed=false`，不声明系统已跑分或生产质量达标。`self_test.py` 只验证 Oracle/验证器；实际报告必须使用 `--report` 校验。v2.0.0 仅保留为历史报告契约/旧权限语义回归输入，不得用于首期权限安全放行 |
...
所有指标逐项通过，不以加权总分抵消失败；越权引用或泄漏零容忍。结果须按单一语料库域、普通/多跳/多轮/时间版本/证据不足/无权限/撤权/提示注入/停用/围栏/策略版本等关键场景和标签分层报告。

`EB-L1-003-DOCUMENT-EVAL` `3.0.0-synthetic` 已冻结在 `docs/design/agent/evidence/eb-l1-003/document-eval-v3.0.0/`。该工件包含114个纯模拟语料块和30个案例，全部归入单一 `document-default` 语料库和 `internal` 安全等级，不含文档级 `allowed_roles`/ACL；7个 RBAC、DOCUMENT 能力、语料库权限、停用、撤权围栏和策略版本拒绝案例要求零候选。20个 Recall@50 计分案例的最小有效候选池为56、最少干扰项为55。v3 报告 Schema、可重算验证器、Oracle 防伪自测、版本记录和 SHA-256 完整性校验均已通过。

v2.0.0 的多模拟域、混合安全等级和文档级 `allowed_roles` 属于历史权限语义，且部分 Recall@50 样本候选池不足；它可保留为历史报告契约和旧语义回归输入，但不得作为首期单语料库权限安全或 Recall@50 放行证据。v3.0 固定包已解除 DOCUMENT/权限安全 L2 的评价工件前置条件，但其 `validation_report.json` 明确 `target_system_quality_run_executed=false`：静态包验证和 Oracle 自测不等于实际系统通过。系统质量放行前仍须在冻结的 Java/Python/检索/模型制品上运行 v3，生成可由 v3 验证器独立重算的报告；生产批准前还须新增经授权脱敏的业务代表性扩展版本。`EB-L1-003` 的历史关闭状态不回退。
...
| 2026-07-17 | `RET-O-12` 编写期证据同步（非正式复评） | 修订后通过（目标文档范围） | 当前规范性表述仍以 v2/`2.1.0` 作为权限安全和 Recall 后续基线，与检索 L1 及已形成的 v3.0 单语料库评价包不一致 | 已同步为 `3.0.0-synthetic` 固定包和 v3 报告门禁；静态校验不替代目标系统实跑，正式联合评审状态不变 |
| 2026-07-17 | 三文档交叉评审—第 1 轮修复后复评 | 需最小治理修订 | `REV-XDOC-001`、`REV-EVID-001` 已关闭；同层职责、权限语义和评价门禁一致；`REV-XDOC-002` 仍有4处旧“草稿/未评审”措辞 | 已按同一发现 ID 完成最小状态措辞修订，不影响架构语义 |
| 2026-07-17 | 三文档交叉评审—第 2 轮复评 | 通过（`L1→L2` ready） | 原冻结发现全部关闭，同层职责、文档源/权限语义、v3 固定包及目标系统实跑门禁一致，无新增S0/S1/S2 | 本文及检索 L1 已评审/已通过；未实施/未生效、对应 L2 前置与系统质量门禁继续有效 |
