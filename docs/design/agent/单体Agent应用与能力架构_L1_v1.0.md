...
| 文档状态 | 已评审 |
| 评审状态 | 已通过；2026-07-17 完成逐文档与三文档交叉评审，第3轮复评无剩余 S0/S1/S2/S3 |
| 实施状态 | 未实施 |
| 生效状态 | 未生效 |
| 当前版本 | v1.0 |
| 上位文档 | [单体Agent智能体总体架构_L0_v1.0.md](./单体Agent智能体总体架构_L0_v1.0.md)（同批已评审架构基线） |
| 同层文档 | [检索与索引基础设施架构_L1_v1.0.md](./检索与索引基础设施架构_L1_v1.0.md)（同批已评审架构基线） |
| 适用基线 | v0.1 冻结提交 `417c140a38d0c32f27a644ac21b1df7f3d936909`（2026-07-17） |
| 维护责任人 | Dylan |
| 目标文档位置 | `docs/design/agent/单体Agent应用与能力架构_L1_v1.0.md` |
| 替代关系 | v1.0 已评审通过并成为当前 Agent 应用与能力架构基线；v0.1 作为冻结历史保留 |

## 2. 版本迁移说明

| 项目 | 说明 |
|---|---|
| 来源文档 | [单体Agent应用与能力架构_L1_v0.1.md](./历史资料/单体Agent应用与能力架构_L1_v0.1.md) |
...
| 2 | 2026-07-17 | 第 6～7、9、12、14 节 | 明确模型调用与上下文安全单一责任，补回外部依赖及失败/重试语义，并恢复生产业务代表性评价门禁 | 修复 `REV-APP-101`、`REV-APP-102`、`REV-XDOC-101`、`REV-XDOC-102` |
| 3 | 2026-07-17 | 文档治理、第 13、15、17 节 | 回写第2轮独立复评通过结论及报告入口 | 本文与 L0、检索 L1 的逐文档和交叉评审均无剩余 S0/S1/S2 |
| 4 | 2026-07-17 | 文档治理、第 16～17 节 | 将编写期“未评审”表述明确限定为历史候选稿状态，并回写第3轮复评结论 | 修复 `REV-GOV-103`，消除当前治理状态与历史内部自评记录的歧义 |
| 5 | 2026-07-20 | 第 5、9～12 节 | 恢复 Java 等待 Python Runtime 时的事务、锁和不可重入上下文边界，并将其落实到流程、决策与 L2 门禁 | 修复 `REV-XDOC-201`，防止 L2 将远程 Runtime 调用包裹在数据库事务或持锁区间内 |
| 6 | 2026-07-20 | 第 5.1 节 | 将 `AD-07` 恢复为 `es-query-service` 收敛与 `es-document-service` 禁止建设的本域承接说明 | 修复 `REV-TRACE-202`，避免用 Adapter 拆分规则替代无关的 L0 检索服务决策 |
| 7 | 2026-07-20 | 文档治理、第 2 节 | 将上位/同层关系从候选期表述更新为已评审基线，并修正 v0.1 历史来源路径 | 修复 `REV-GOV-203`，消除状态歧义并恢复来源可导航性 |
...
| AD-07 | 本域只经 Java Tool Gateway 调用收敛后的 `es-query-service` 查询面，不建设或依赖 `es-document-service`；检索服务内部收敛由检索 L1 负责 |
| AD-08 | Java 组合 RBAC、Agent 字段策略和资源属性，执行端只能收紧 |
| AD-09 | DOCUMENT 授权过滤在候选召回前生效，后验终检不能替代前置过滤 |
| AD-10 | Java 调用或等待 Python Runtime 时不得持有数据库事务、锁或不可重入上下文；跨 Java、Python、业务服务和检索域不建立分布式事务或全局锁 |
...
| 依赖/契约权威 | 消费方 | 使用语义 | 失败、重试与降级边界 |
|---|---|---|---|
| `auth-service` | Java Agent | 获取/验证可信主体、租户和 RBAC 上界；不消费其字段规则作为长期权威 | 授权事实缺失、过期或冲突时失败关闭；不得用旧缓存扩大可见范围 |
| `employee-service` | Employee Adapter | 执行 employee QUERY/AGGREGATE 只读契约 | 保留无数据、非法条件、权限拒绝和依赖失败语义；仅在契约确认无调用方可见副作用且总 deadline 有余量时由 Java 有界重试 |
| `mq-procedure-service` | Transaction Adapter | 执行 transaction QUERY/AGGREGATE 只读契约 | 保持分页、聚合和错误语义可验证；缺少 Agent 所需语义时显式失败，不得猜测或改用其他域接口 |
| `es-query-service` 查询面 | Java Tool Gateway | 提交逻辑语料库、原子通道、最终硬过滤、预算和 deadline，接收已授权候选与诊断 | 授权、过滤、围栏或兼容状态不确定时失败关闭；Java 只对查询面安全失败有界重试，服务内部依赖重试由检索域唯一负责 |
| 获准 LLM / Reranker 端口 | Python Runtime | 仅在 Java 下发的 `ModelAccessDecision`、安全上下文和预算内完成规划、DOCUMENT 精排/生成 | 禁止自行换目标或跨信任等级回退；失败只能按能力策略受控降级或失败，不得绕过安全/证据门禁 |
...
1. Java 完成身份/租户/能力校验，以短事务创建执行、设置 deadline 并提交事务。
2. Java 在数据库事务、锁和不可重入上下文之外形成模型准入决策与最小安全上下文，再调用 Runtime。
...
1. Java 校验 DOCUMENT 能力和语料库权限，以短事务创建执行与短期 Tool 凭据并提交事务。
2. Java 在数据库事务、锁和不可重入上下文之外裁决模型目标并投影问题/历史；Python 选择获准原子召回通道和预算。
3. Tool Gateway 重验服务身份、执行凭据、deadline 与最终硬过滤，调用检索查询面。
4. Python 仅在 Java 已授权候选和预算内完成融合、精排与证据打包；Java 独占权限/安全投影和最终候选校验权威，Runtime 不得扩大候选、字段或模型可见范围。
...
每条依赖只有一个内部重试责任方。Java 负责 Runtime、业务服务和检索查询面入口的有界重试；其中检索查询只在确认没有调用方可见副作用且剩余预算允许时重试，Elasticsearch/Embedding 的内部重试只由 `es-query-service` 负责。所有层级尝试均计入同一总 deadline 和调用预算，必须返回尝试诊断，禁止乘法重试。

## 10. 核心架构机制与质量属性

### 10.1 一致性、幂等与副作用

- Java 只在本地短事务内创建执行、推进状态或提交终态；调用或等待 Runtime、Adapter、检索及模型等远端依赖时不得持有数据库事务、锁或不可重入上下文。
- 不建立跨服务分布式事务或全局锁；本地持久化与远端只读调用通过状态机、幂等、栅栏和审计闭环。
...
| APP-DEC-02 | Java 在本地短事务提交执行起点后，于事务、锁和不可重入上下文之外调用 Runtime，并作为唯一持久状态权威 | 防止 Python Checkpointer/双写导致状态分叉，同时避免远程等待占用数据库连接或扩大锁窗口 |
...
| 执行生命周期与持久状态 | APP-DEC-02/09 | 状态机、短事务提交点、远程调用事务外边界、幂等、栅栏、回调、SQL 迁移责任 | Python 持久恢复 |
...
| Employee Adapter | APP-DEC-06 | `employee-service` 查询/聚合契约、字段/操作符/错误语义 | transaction 规则 |
| Transaction Adapter | APP-DEC-06 | `mq-procedure-service` 查询/聚合契约、分页/聚合/错误语义 | employee 规则 |
| DOCUMENT 编排与证据闭环 | APP-DEC-07/14/17 | `es-query-service` 查询面契约、融合/精排、引用与拒答公式、评价运行入口；生产批准前增加业务代表性扩展结果 | ES 索引生命周期 |
...
1. v0.1 与证据包继续冻结；三份 v1.0 已作为同批架构设计通过评审。后续任一文档发生实质变更时，必须重新执行受影响文档的逐文档与交叉复评。
...
| v3 合成集业务代表性 | 未完成 | 即使目标系统实跑通过也不能单独支持生产质量批准；生产批准前需增加经授权脱敏、分层抽样并由责任方签署的业务代表性扩展版本 |
...
- 本文与 L0、检索 L1 已于 2026-07-17 按同一版本批次完成正式评审；编写期内部自评仍只作为候选稿质量控制，不替代该正式记录。

## 16. 编写期内部自评

内部自评仅检查表达、完整性和与冻结 v0.1 的语义一致性，不构成正式批准。以下记录发生在正式评审之前；当前治理状态以第 17 节为准。
...
| 2 | 严格结构、APP-DEC/上位追踪、Agent↔检索边界、评价门禁与治理状态 | 0错误0警告；APP-DEC-01～17 与 v0.1 集合相等；AD-01～24、DEC-01～17 均可追踪；未发现 P0/P1/P2 | 无需进一步修复 | 内部自评通过；当时为候选稿，当前状态见第 17 节 |

## 17. 正式评审记录

| 日期 | 评审类型 | 结论 | 问题摘要 | 处理状态 | 报告 |
|---|---|---|---|---|---|
| 2026-07-17 | 逐文档评审 + 三文档交叉评审，第3轮复评 | 通过（`L1→L2` ready） | `REV-APP-101`、`REV-APP-102`、`REV-XDOC-101`、`REV-XDOC-102`、`REV-GOV-103` 均已关闭；无剩余 S0/S1/S2/S3 | 已完成；第14章列出的 L2/生产门禁继续有效 | [Agent L1 v1.0 评审报告](./单体Agent应用与能力架构_L1_v1.0_评审报告.md) |
