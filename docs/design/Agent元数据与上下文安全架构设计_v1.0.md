...
- 本文高于 D02/D04/D03 对应 L2、配置、数据库和代码。
...
| Coordinator、CoordinationPlanner、Run/Task/Attempt、ResultRef、Task State Boundary | `Multi-Agent协调与任务架构设计_v1.0.md` | 只定义 TASK 如何复用主体、授权、Context 和 Domain 边界，不定义调度与归并 |
...
| Context 生命周期 | TTL、清理、并发更新不完整 | 加密、TTL、版本 CAS、成功写入、随 Conversation/Run 清理 |
...
| Authorization / Result Security Boundary | 计算 Effective Profile/Scope、冻结 Snapshot、执行当前权限复检，并按同一有效范围过滤/mask 类型化候选 | 编排 Planning/Execution、执行 Handler、持久化 Invocation、信任 Handler 自报授权 |
...
```text
Profile Definition + Policy + Current User Permission + optional Delegation
  ...
  → Authorization Snapshot bound into ExecutablePlanningResult
  → Execution current-permission recheck
...
| Planning Effective Scope | Effective Profile、Current User Permission 与可选 Delegation Constraint 的请求级交集 | 最终执行许可、跨 Invocation 复用 |
| User Permission | 外部权威系统给当前主体的权限事实 | Profile/Policy 配置副本 |
| Delegation Constraint | TASK 可选的委派收紧边界；CHAT 使用全集中性语义 | 新权限、调用方 JWT、Task Graph |
...
| Available Capability | 单个 capability 在 Registration、Profile、Policy、Permission、Delegation 和 Domain 能力下的请求级可用交集 | 静态事实、最终执行授权 |
...
Effective Profile、Available Capability、Domain Projection、Runtime Domain Schema、Context View、Adapter Availability Projection、Adapter Execution Binding、缓存和查询投影都不是新的事实来源。
...
- Multi-Agent 未来可用的委派边界引用；具体委派语义由 Multi-Agent L1 定义。
...
- D06 后对 Multi-Agent L1 所定义委派/资源约束的部署级收紧引用。
...
该计算只收紧 Profile，不形成用户授权结论。随后 Planning Effective Scope 再与 User Permission 和可选 Delegation Constraint 求交：

```text
Planning Effective Scope
  = Effective Profile limits
  ∩ Current User Permission
  ∩ Delegation Constraint (TASK only; CHAT = universal neutral constraint)
```
...
- Run Owner Reference：D06 后 Run/Task 所属的稳定 Owner；CHAT 中与认证 Conversation Owner 规则一致；
- Invocation Scope：ConversationScope 或 RunScope；
- Delegation Constraint：TASK 可选收紧，CHAT 使用中性全集；
...
- Agent Profile/Policy/User Permission/Delegation 的精确版本或证据引用；
- 允许的 capability、domain、field、operator/function、Context read/write 范围；
- 风险、执行模式、结果过滤、预算和 TTL 上限引用；
...
Planning 在计算 Catalog 前必须捕获同一 Profile/Policy/Permission/Delegation 解析链的不可变授权证据与版本链，并用其完成 Route/Plan 全部安全投影。每次向 Runtime 发送 Route/Plan 前以及冻结 Snapshot 时，都必须确认该证据仍有效；一旦变化就 fail closed，不重新读取并混入新版本继续当前 Planning。capability 确定后，Planning 只从该证据链收敛一个 capability-scoped Authorization Snapshot。禁止调用方并列传入可替换的 capability/domain/Context 权限副本。Snapshot 与 Available Capability Snapshot、Context Snapshot 和 Resolved Registration 必须共享同一主体、范围、版本链和 request correlation；Planning 完成后的最新变化由 Core 当前复检处理。
...
Effective Execution Scope
  ...
  ∩ Current Delegation Constraint
```

复检前必须确认主体、Owner、Invocation Scope、Profile/Policy/Delegation 版本仍有效；“bound Profile version”不得替换为最新 Profile 版本。任何撤权、版本不匹配、权威源不可用、范围扩大请求或无法确认都 fail closed。复检可以保持或缩小 Planning 范围，永远不能扩大。
...
结果过滤使用同一次 Core 复检得到的 Effective Execution Scope 和 output ContractRef：
...
其中 Planning Effective Scope 已唯一包含 Effective Profile（Profile ∩ Policy）、Current User Permission 和 optional Delegation Constraint。Catalog 必须按声明和交集通用计算，禁止重复读取 Policy/Permission，或增加 capabilityId/domain/agentId 专用 if/switch、硬编码清单和 Prompt 分支。
...
D04 必须删除配置、Prompt、Adapter 自报清单和 Java 常量中的 Domain 执行事实副本；Adapter 的必要映射实现保留，但不再作为 metadata 来源。D04 形成：
...
Context key boundary
  = Owner Reference
  + Invocation Scope (ConversationScope | RunScope)
  + contextType
  + schema version compatibility
```

- CHAT 只读取/写入认证 Owner 的 ConversationScope。
- D06 TASK 只读取/写入 Run Owner 的 RunScope；Task 间显式结果通过 ResultRef，不通过“最新 Context”传递。
- Owner/Scope 只从 Invocation Handle 和 Authorization Snapshot 派生，不接受 Runtime、Handler、Adapter 或 payload 自报。
- 不允许跨 Conversation/Run 回退到同用户“最近 Context”。
...
- Context 随 Conversation/Run 清理；清理幂等且不得留下可继续读取的孤儿投影。
...
任一适用层拒绝或无法确认即 fail closed。CHAT 中 Delegation Constraint 使用中性全集，不新增 Chat 专用公式。
...
- Profile/Policy/Permission/Delegation 证据引用；
...
单 Agent D03 中，Lifecycle 在同一 Agent DB finalization transaction 中原子提交：
...
sequenceDiagram
                    ...
                    alt Registration resolved
                        ...
                        alt Context and Domain projection valid
                            ...
                            alt Valid ExecutablePlan
                                P->>AU: Freeze capability-scoped Snapshot from same captured evidence
                                AU-->>P: immutable Snapshot bound to all versions or deny if evidence changed
    ...
    alt ExecutablePlanningResult
        ...
        EC->>V: Raw Plan + Execution Validation Context with same Binding projection
        V-->>EC: immutable Validated Plan or safe failure
        EC->>H: Registration bridge: Validated Plan + Execution Context
        ...
        EC->>AU: Filter/mask typed candidate under same Effective Execution Scope
...
- optional Delegation version/reference；
...
9. 任一不一致拒绝相应版本整体生效，不允许部分加载或静默降级。
...
- capability/domain 被 Policy、Permission、Delegation、Adapter availability 拒绝的分类计数；
...
- CHAT/TASK 使用同一 Profile、Policy、Permission、Capability Catalog、Canonical Catalog、Context 和 Authorization / Result Security Boundary。
...
| MS-03 | Effective Profile = Profile ∩ Policy；请求授权再与 Permission/Delegation 求交 | L0 第 5、7、9 节 |
...
| MS-10 | Context 以 Owner + ConversationScope/RunScope + type/version 隔离 | L0 第 5.4、9.2 节 |
| MS-11 | Context 最小化、加密、TTL、版本 CAS 并随 Conversation/Run 清理 | L0 第 9.3 节 |
...
本文评审通过后，与已评审的能力执行内核 L1 共同闭合 D02 的 L1 前置；D02 仍必须遵守 L0 的阶段顺序。D04 也以本文为直接架构基线。
...
D03 必须与契约、Planning、Capability Kernel、Persistence、Adapter metadata、API/UI 和旧路径删除一起纵向切换。禁止提前发布新的 Profile/Catalog/Context/Adapter metadata 半链，或保留旧 query context、旧权限判断和新边界并行运行。
...
3. Profile、Policy、Permission、Delegation 的交集公式唯一且 fail closed。
...
6. Capability Catalog 按 Registration/Profile/Policy/Permission/Delegation/Domain 能力通用计算，无 capability/domain 专用分支。
...
16. Context 以 Owner + ConversationScope/RunScope + contextType/version 隔离，不跨 Conversation/Run 回退“最近值”。
17. Context payload 最小化、静态加密、设置最严 TTL，并随 Conversation/Run 清理。
...
29. CHAT/TASK 复用同一 Profile、Authorization / Result Security、Catalog、Context 和 Domain 边界，不建立第二权限体系。
