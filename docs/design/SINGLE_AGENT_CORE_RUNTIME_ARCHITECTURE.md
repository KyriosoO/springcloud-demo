...
> 本文是 P2 第一份 L1 架构草稿。作者自审、校验通过不等于正式评审、实施或生效；进入本文治理的 L2 前仍须关闭 `CR-GATE-001`。
...
| `SA-C-010` | 新能力通过能力实现、Adapter、配置和组合根注册接入；核心无业务域条件分支 | 6、10、12 | 新增模拟能力及依赖扫描测试 | 无 |
...
| `SA-C-018` | 核心只接受 Knowledge 返回的证据充分性和出域状态，不得将失败或证据不足改写为肯定答案 | 7、9、10 | Knowledge 契约替身、拒答和模型零调用测试 | 无 |
...
flowchart LR
    ...
    subgraph LogicalAgent["一个逻辑 Agent"]
        ...
        subgraph Runtime["agent-runtime<br/>Python 运行进程"]
            Entry["受控运行时入口"]
            Graph["LangGraph<br/>唯一编排与请求状态"]
            Core["agent-core<br/>确定性执行约束"]
            Registry["能力注册运行时<br/>启动校验、运行只读"]
            Api["agent-capability-api<br/>稳定能力语义"]
            ModelPort["DeepSeek 模型端口"]
            Root["组合根"]
        end
    end

    Knowledge["Knowledge Capability / Adapter<br/>关联 L1"]
    Business["Employee / Transaction Adapter<br/>关联 L1"]
...
`agent-runtime` 是部署和组合边界，LangGraph 是其中唯一的 Agent 编排权威。`agent-core` 是由图节点调用的确定性领域无关服务，不形成第二张图或第二套状态机。关联能力通过 `agent-capability-api` 和注册运行时接入，不反向依赖 Spring 接入层。
...
| DeepSeek 模型端口 | 隔离结构化动作和回答生成的供应商语义 | 受控提供方实现 | 不接受未通过出域边界的业务或知识载荷 | 支持测试替身和未来供应商替换 |
| 组合根 | 在启动期装配图、核心、注册表、模型实现和关联能力 | 全部内部模块 | 是唯一知道具体实现集合的地方 | 新能力只增加装配，不改核心业务逻辑 |

### 6.3 同层边界

| 关联 L1 | 对方权威范围 | 本文责任 | 交互语义 | 禁止侵入事项 |
|---|---|---|---|---|
| Knowledge L1 | `knowledge.query` 内部流水线、知识配置、Knowledge Adapter、检索/BGE 集成和知识出域 | 提供能力 API、注册、执行上下文、模型端口和通用结果语义 | 将 Knowledge 作为一个能力执行；接收其证据充分性及出域状态 | 核心不得定义知识域、索引、召回、重排或文档策略 |
      ...
      → LangGraph
          → agent-core
              ...
              → agent-capability-api
                  ← 具体 Capability / Adapter
          → DeepSeek 模型端口
              ← DeepSeek 提供方实现
```

- Spring 不依赖 LangGraph 图、核心实现、能力实现或 Adapter。
- 核心不依赖具体 Capability、Adapter、业务客户端或供应商 SDK。
- 具体能力实现依赖能力 API，不依赖核心执行实现。
- 组合根可以知道具体实现以完成装配，但业务调用不得经组合根形成旁路。
...
| 能力执行契约 | 具体 Capability / Adapter | `agent-core` | 接收受控请求和执行上下文，返回统一结果状态及允许结果 | 必须保留 JWT 和总时限；不得暴露原始协议异常供模型自由解释 |
...
| DeepSeek 模型契约 | DeepSeek 提供方，经模型端口隔离 | 结构化动作候选和受控回答生成 | 超时、无效结构或提供方失败必须与无结果区分 | 用户问题也属于外部模型载荷；敏感问题受 `CR-GATE-003` 控制；调用消耗图内预算且默认不自动重试 |
...
- `model_egress_denied`、未分类或没有安全模型载荷时，DeepSeek 调用次数必须为零。
...
sequenceDiagram
    ...
    participant A as 单个 Capability/Adapter
...
| 模型出域拒绝 | 对应能力策略判定 | 返回 `model_egress_denied` 或受控非模型结果，DeepSeek 零调用 | 核心扩大字段或忽略未知策略 | 修正策略/数据分类后新请求 |
...
不得修改 `agent-core` 增加业务域分支，不得修改其他能力实现，也不得扩展为动态插件平台。
...
- 用户原始问题在发送 DeepSeek 前也必须执行最小化和外部模型输入边界检查；`CR-GATE-003` 未关闭时只允许非敏感测试问题或本地测试替身，不得以“尚未查询业务数据”为由默认外发敏感文本。
...
- `agent-service` 对外，`agent-runtime` 仅暴露受控内部入口；Capability 和 Adapter 与 Python 运行时同进程部署。
...
- 稳定扩展点只有能力 API、启动注册入口、模型端口和运行时入口契约。
- 组合根增加模块依赖属于允许的装配变更，不属于核心业务侵入。
...
| 安全 | 全查询用户身份、业务最终授权、模型数据最小化 | 100% 请求须有用户 JWT；零服务身份回退；出域拒绝时模型零调用 | 401/403、JWT 透传、模型零调用和日志脱敏测试 | 不得以个人项目为由跳过 |
...
| 扩展性 | 新能力不侵入现有能力 | 新增模拟能力只新增实现、配置、装配和测试，不改核心业务分支 | 扩展验收和依赖检查 | 不演化为动态插件平台 |
...
| 《单体 Agent 核心执行与能力注册 L2》 | LangGraph 请求状态、核心确定性闸门、能力 API、执行上下文、统一结果、组合根、注册校验与冻结、模拟能力扩展测试 | `SA-C-002/005/008/009/010/012/014/018/019`；`CR-AD-002/003/006/007` | 本文正式评审 | `CR-GATE-001`；实现前 `CR-GATE-002` | Knowledge 流水线、业务 DTO、字段策略和供应商协议 |
...
| 能力 API、注册、组合根和结果语义 | 核心执行与能力注册 | 是 | 否 | 具体能力描述和域内配置仍由关联 L1/L2 拥有 |
...
|---|---|---|---|---|---|
| P2-L1 | 核心与运行边界可评审 | 完成本 L1 并与 L0/索引同步 | `CR-GATE-001` | 回退文档草稿 | 无 |
...
|---|---|---|---|---|---|---|---|---|
| `CR-GATE-001` | `design_decomposition` | 本 L1 → 核心运行 L2 | 开始并定版本文治理的三份 L2 | 本文独立评审无未关闭 S0/S1；项目维护者确认职责、同层边界、能力契约和 L2 分解 | 项目维护者、独立评审方 | 核心运行 L2 开始前 | 允许事实核实和 PoC 计划；禁止把本文草稿当成已批准实现依据 | 三份核心运行 L2 |
...
| `SA-GATE-006` | `integration` | 领域真实或敏感数据进入 DeepSeek | 使用知识证据或业务真实结果调用外部模型 | 关联能力出域 L2 完成、未分类/冲突失败关闭及外部模型零调用测试通过 | 项目维护者、模型提供方、关联能力 | 首次真实数据联调前 | 只允许非敏感测试数据或本地替身；禁止真实未获准载荷外发 | DeepSeek L2 及关联能力 L2 |

门禁仅控制所列动作，不阻塞另外两份 L1 的事实核实和草稿编写。本文完成后，Knowledge/业务查询 L1 可以引用能力 API 的架构语义；在 `CR-GATE-001` 关闭前不得把字段级契约视为稳定实现基线。
...
| 敏感数据进入模型或日志 | 用户问题未最小化、能力未完成出域裁剪，或模型端口/日志记录原始载荷 | 数据泄露 | 输入最小化、默认拒绝、模型调用前置闸门、零调用及日志脱敏测试 | 项目维护者、关联能力 |
...
| EXT-01/02/03 | 9.4、10.7、11 | 新增模拟能力和依赖扫描 | 已设计，未实施 |
...
5. 是否接受三份最小 L2 的覆盖和顺序，以及 `CR-GATE-001/002` 的阶段约束。
6. 是否接受第一阶段不持久化会话或图状态、单实例重启恢复和在途请求失败语义。

### 17.3 正式评审记录

正式评审记录只能根据独立评审或项目维护者明确结论填写。当前本文为未评审草稿，作者自审不写入正式评审记录。

| 日期 | 评审类型 | 结论 | 问题摘要 | 处理状态 |
|---|---|---|---|---|
| — | 尚未正式评审 | 未评审 | 待执行 L1 独立评审并关闭 `CR-GATE-001` | 待评审 |
