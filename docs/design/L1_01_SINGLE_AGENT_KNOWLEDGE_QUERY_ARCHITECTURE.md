...
> 本文是 P2 第二份 L1 架构基线。v0.2 已完成五轮独立评审—修订—复核，`REV-KQ-001`～`REV-KQ-008` 均已关闭且无未关闭 S0/S1；项目维护者已授权按评审结论原子同步，`KQ-GATE-001` 已关闭。该结论只允许开始本文治理的三份 L2，不表示 L2 已定版、代码实施已开始、真实检索/模型链路已获准或知识效果已经验证。
...
| Knowledge Adapter | 实现 Retrieval Port，完成逻辑域到受控别名/索引映射、用户读取上下文传递和提供方协议转换 | 不拥有读取授权规则、知识策略、内容真相、域选择或答案生成 |
...
| `L2_01_01` Knowledge 检索与本地模型接入详细设计 | Retrieval Port/Adapter、逻辑域到受控物理资源的映射、当前用户读取上下文及候选返回前授权契约、`es-query-service` 面向 Knowledge 的类型化只读契约、统一候选/失败、必要的最小提供方公开接口改造设计、Embedding/Rerank Provider、融合/去重/重排算法和运行限制 | 本文、`L2_01_00` 的逻辑域目录、外部提供方现状、知识读取权威 | 接口/字段、域映射、明确拒绝/判定不可验证/权威失败的类型化结果、兼容策略、双方映射及最小改造触点、授权先于正文返回/BGE 的负向测试、超时、候选上限、契约测试和管理接口不可达测试 | 逻辑域语义、读取规则所有权、Employee/Transaction 结构化查询、`es-query-service` 全面重构、DeepSeek 通用协议和出域策略决定 |
...
|---|---|---|---|---|---|---|---|---|
| `KQ-GATE-001` | `design_decomposition` | 本 L1 → Knowledge L2 | 开始本文治理的三份 L2 | **已满足（2026-07-25）**：v0.2 完成五轮独立评审—修订—复核，`REV-KQ-001`～`REV-KQ-008` 均关闭，无未关闭 S0/S1；项目维护者已授权按评审结论原子同步 | 项目维护者、独立评审方 | 任一 Knowledge L2 开始前 | 已关闭；允许开始三份 Knowledge L2；禁止据此开始代码实施、声明 L2 已定版或绕过其余开放门禁 | 三份 Knowledge L2 |
