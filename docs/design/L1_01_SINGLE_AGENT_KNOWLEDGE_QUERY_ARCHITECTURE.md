...
| 当前版本 | v1.23 |
| 日期 | 2026-09-10 |
| 本次增量 | KQ-AD-014原问关键词与分域改写向量互补保持；仅同步L2_01_00 §8.12当前源码11/7/v3绑定及定向non-live状态，不增加路径或模型调用；未宣称部署或真实专项通过 |
...
| 实施状态 | 在线生产源码单绑定Rewrite V11、Summary V7、质量策略V3和阶段A离线语料处理已实施；当前对象图定向non-live已验证，未部署新版，模型选域、适用性证明及完整真实专项仍未通过；具体Gate/UAT、证据与评审状态由P3/UAT_01管理 |
...
| 版本 | 日期 | 变更原因 | 变更内容 |
|---|---|---|---|
| v1.23 | 2026-09-10 | 两路共用改写会同时丢失原问的词面线索 | 有界安全原问用于keyword，分域改写用于vector；长问题保留既有表达，旧计划兼容；本增量实施/验收独立记录 |
| v1.22 | 2026-09-10 | 每域复制全部原问条件与聚焦子问题冲突 | 复用请求级证据需求表达条件归属，保持原问题、全局未分配约束和各域所属条件；不增加模型字段、调用或索引变更 |
| v1.21 | 2026-09-07 | 政策向量结构获得追加授权，短条款可能缺少检索上下文 | 离线工具可构造标题/现有章节/原文的派生向量输入；不改引用正文、在线接口或读取/出域边界，不以局部排名改善批准发布 |
...
- 图谱、语料再导入、未评审的索引重建与公共 DTO/公开枚举变更；§4.7仅为追加授权的政策向量表示增量，具体候选构建/发布仍须先满足L2合同；
...
条件保持不等于每个域复制整道问题。当前实现沿用V9引入的语义：由同次模型在既有必要证据需求中声明各子问题的条件，本域query保留本域需求条件；适用于多域的条件应在各域需求中明确，原问未分配给任何需求的受保护条件保守作为全局条件。归属声明必须先通过原问约束子集及输入安全校验；不接受新事实，不以query反推或修补需求，不因查询失败扩域。局部校验只能证明声明与表达的一致性，不能证明模型对任意中文作用域的理解；全局条件被错误归到单域等语义错误仍由原问题、原文判据和专项UAT约束。L2定义有界校验、兼容和失败语义；历史规则保留，不与当前V11同时注册或作为后备。

`KQ-AD-014`：每个已选中域仍只执行keyword/vector两路。当前增量目标为keyword保留安全且符合检索长度上限的原问，vector使用该域已校验改写；原问过长时在检索前固定沿用该域改写，不截断、不拆分、不追加路径，也不因结果失败再切换表达。两路同表达保留历史兼容，不再作为新生产策略的强制条件。读取授权先于正文返回。最多4路、80条原始候选；RRF不变，V2请求内按域顺序串行、每域最多一次BGE重排，跨域只比较域内排名而非不同query的裸分数。质量策略V2只保留每个非空域的语义重排首位，再按各域语义排名轮转填充；关键词候选仍参与召回与融合，但不强制占据最终前排。最终20条、Evidence8条和32768字节上限不变，不再以父文档相同为由额外排除相关条款。当前需求排序V3按§4.6执行，域覆盖不等于答案充分；摘要仍必须拒绝缺失证据。

此增量不改Rewrite的语义选域、需求或每域一个query输出；本地仅选择两条既定路径的文本来源，不能依据文号、行业、case或gold生成query。与扩大候选窗口、追加原问第三路、继续增加文号正则相比，同预算双表示的修改最小，且不改变RRF路径权重。代价是完整原问在多域中可能引入旁支词面噪声，长问题不能获得原问保留收益；必须通过跨题、双域及完整候选相关性验证，不把必要召回提升等同整体准确率。原问安全与长度规则、内部计划来源校验和旧调用兼容由L2_01_00/01治理；模型/Java接口、正文授权、出域、Summary和在线流程均不变。部署、回滚和真实效果由P3/UAT管理。

V2是同一流水线内的版本化质量策略，不增加模型、检索、输入预算或出域字段。V1的关键词/语义双锚点、交错填充及每文档3条保留历史解释责任；legacy仍为每文档2条，不作为当前V2约束。取消独立父文档配额的风险是错误高排名或单文档占比上升，由域轮转、总数/字节硬上限及专项原文覆盖验收控制，而非引入新模型选择器、内容黑名单或文档特判。

方案比较：仅保留原配额不能处理同一规范性文件的第四条必要条款；两遍“先多样性、再填充”仍可能在8条内先耗尽预算，缺少证据支持其必要性；采用“域覆盖＋语义相关性＋总预算”作为最小方案。此取舍允许下位设计版本化选择规则，不授权索引、读取/出域、引用validator或gold变化。V2在完成本增量设计评审前不得实施，实施与真实效果分别由P3/UAT治理。
...
`KQ-AD-016`：阶段 B 不原位修改 Summary V4、读取策略、三层出域、原文子串或引用校验；经用户确认的摘要语义增量仅通过KQ-AD-017的新任务版本实现。不能利用未知时效元数据认定当前有效。功能与专项质量验收独立于历史 P5 运行，不能用新算法改判旧结果。

`KQ-AD-017`：与答案相关的显式分类上下文属于须证明的回答要点，不能把用户给定的类别归属直接当作已证事实。若问题在某分类下询问对象定义，应从本次允许证据同时支持定义及该归属；不推导用户未提出且与回答无关的上位分类，不使用模型常识补链。仅一段连续原文足以证明全部要点时仍只引用一次；否则选择不同来源片段，在现有引用和长度预算内无法完整证明时输出证据不足。语义理解归Summary模型；本地validator继续只证明引用/结构完整性，检索coverage不代表答案语义完整。通过新Summary V5指令和唯一组合根绑定实施，不添加模型复核轮次、行业特判、gold依赖或第二链路；真实效果必须另行验证。

该增量经三轮内审及只读设计复评通过，允许下位合同及非live实施；已实施并通过定向non-live验证，不继承此前功能UAT的语义效果结论。下位合同、验证与当前状态分别见L2_01_02、P3及UAT_01。

当前生产源码绑定为Rewrite V11（按L2_01_00 §8.11/8.12使用输出专用Schema信封及固定启用域槽位；无损映射后复用V7必要证据合同、V8澄清优先规则及§8.9分域校验）、Summary V7覆盖声明及需求排序V3；保留既有每域表达与显式条件保护。此前任务验证不继承为新版真实效果通过。实现入口、代码复核和专项验收状态只在P3/UAT_01记录。本轮仅同步当前实现绑定，职责决策不变，不表示部署已更新。
...
|---|---|---|---|
| 再补Prompt示例 | 可能改善模型意图 | 下游仍无共同的必要证据清单；不能本地核对遗漏 | 不作为完整修复 |
| 增加topK/Evidence | 降低部分窗口损失 | 不解决漏域与漏证明，增加正文暴露及成本 | 不采用，保持现有硬上限 |
| 请求内强类型证据需求 | 同一需求从规划贯穿排序、Evidence与摘要 | 新内部合同及最多4次本地重排；语义正确性仍依赖模型和UAT | 最小可验证方案，不引入规则引擎或新模型轮次 |

一次Rewrite在读取前同时给出查询目的、必要证据需求和每域查询。资料查阅按原问题提出有限要点；选择单一适用结论时，需求必须包括对象/分类范围、规则和时效依据，不把用户给定类别当成已证事实。模型负责语义、需求归域及检索表达，本地只校验类型、有限角色、数量、域白名单和原问条件；不按行业、词面或gold补需求/扩域。即使属于同一域，也可存在多个必要证明角色；不强制所有问题双域。

检索仍最多两域、四路、80条原始候选。新质量策略V3对每个证据需求在其已授权域内候选池作一次本地重排，并保留该需求首位候选；这些保留项只是覆盖机会，不是“已证明”标记。最多4次重排，最多160次候选评分，仍受原20秒检索阶段deadline；不提高每路20、最终20、Evidence8或32KiB上限，不增加检索轮次。摘要在原问题及冻结需求下逐项声明引用；本地额外核对完整需求ID、来源归域和实际引用对应，之后仍执行原有唯一引用及连续子串校验。任一需求无法证明则证据不足，不返回部分肯定结论。

Knowledge内部计划/阶段数据可增加版本化字段；公共Core、Spring/Runtime HTTP及ES业务DTO、三个Stage Protocol方法签名不变。禁止借query字符串、全局变量或隐藏上下文传递需求。逻辑域/需求来自已安全处理的问题，不扩大文档模型可见字段；既有读取与三层出域仍独立拒绝优先。旧任务与策略源码不变，当前Rewrite11/Summary7/quality-v3在代码中成对唯一绑定，经L2_01_00 §8.12精确表示映射后仍使用原必要证据输出合同，不允许运行时降级。

L2_01_00独占需求类型、规划校验与请求内传递；L2_01_01独占需求导向排序及候选保留；L2_01_02独占Evidence预算、出域载荷和摘要覆盖校验。共享ID只用于请求内机械追踪，不是图谱、长期记忆或新的知识事实。模型可能遗漏/误归类要点、错误选域或虚报语义支持；局部校验不能证明自然语言蕴含，须以固定原文判据和非酒店保留问题实测，设计通过不等于核心P0或阶段B完成。

### 4.7 向量检索表示与引用原文分离（KQ-AD-019）

政策短条款的向量可以由离线Corpus工具加入其自身已有标题和章节，提高检索可辨识性；这些元数据只是检索表示，不成为补造的原文或答案事实。引用正文、contentHash实际校验、chunk/document标识、条款关系、权限和出域元数据保持不变；Agent仍只得到经授权的原文候选，不接触拼接向量输入、物理字段或索引管理。

| 方案 | 收益与风险 | 取舍 |
|---|---|---|
| 仅重编码正文 | 消除部分存量向量与当前本地模型差异，但短条款仍缺上下文 | 保留为离线对照，不能冒称结构修复 |
| 自身标题＋现有章节＋原文 | 复用已存在数据、确定性构造，不引入事实或层级推断 | 推荐最小试验及表示合同；效果与发布另行验证 |
| 推断祖先层级、换ANN或扩大窗口 | 可能引入错误父子关系或无证据复杂度；局部近似搜索对照未显示主要损失 | 暂不实施，不因个别gold排名低硬编码补救 |

本切片只准入可独立验证的纯向量输入构造器；候选复制、重编码、mapping、快照和发布是后续有直接依赖的实施切片，不因构造器通过而成为已完成。后续必须保留共享物理索引中的law全部记录及其现行向量，policy范围由既有Profile确定；新索引不覆盖旧索引，发布继续遵守既有授权/出域快照和回滚规则。不得从用户问题、gold、case ID、模型答案或推测时效生成索引内容。

实际本地模型身份/版本、向量输入版本及哈希必须可绑定；仅服务health正常不证明模型快照。离线余弦对照不等同新索引ANN、typed检索、rerank、摘要或完整UAT通过。新增结构不能修复“缺必要条件却执行检索”的独立语义问题，也不产生新的付费执行授权。
...
- 默认Runtime仍禁用Knowledge；当前源码显式启用路径唯一绑定Rewrite V11、Summary V7及质量策略V3。旧Rewrite V1～V10、Summary V1～V6保留历史兼容和哈希追溯；V11仅精确解码槽位并无损转接原严格decoder，不注册历史任务。
...
- `true`：当前实现在同一Model Gateway唯一注册Rewrite V11、Summary V7，在同一Registry唯一追加`knowledge.query`；旧版本不作为自动后备。当前实施及验收进度由P3管理，已接线不代表真实效果通过。
...
v1.22仅KQ-AD-013/018条件归属增量完成三轮内审和分离修改阶段后的只读设计复核；未新增职责、公开合同或模型调用，允许L2 DR-KFLOW-028非live实施。S0=0/S1=0/未处理S2=0，接受模型可能错误声明条件归属的语义限制，仍须专项UAT；同一执行者不冒充外部独立评审。实施/真实结论由P3/UAT管理。

v1.21聚焦设计复评：KQ-AD-019纯表示与原文/权限/在线流程分离，候选与发布非隐含批准；三轮内审及分离编辑只读复评通过，同一执行者不冒充外部独立人员，S0=0/S1=0/未处理S2=0；不代表候选迁移或效果通过。
