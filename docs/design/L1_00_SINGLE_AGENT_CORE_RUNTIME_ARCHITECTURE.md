...
| 关联 L1 | [`L1_01`《单体 Agent 知识查询能力架构 L1》](L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) v0.2（已评审/已通过，`KQ-GATE-001` 已关闭）；计划中的 `L1_02`《单体 Agent 业务查询适配架构 L1》 |
...
| `L2_00_02`《单体 Agent DeepSeek 模型接入与受控生成 L2》 | 模型端口、`deepseek-v4-pro` 契约、凭证映射、用户问题最小化与输入闸门、结构化动作 PoC、响应校验、模型预算/重试边界和领域出域调用闸门集成 | `SA-C-005/009/011/014/018`；`CR-AD-005` | 核心执行语义；关联 L1 的出域判定语义 | `CR-GATE-001`；定版与真实实现受 `SA-GATE-002` 控制；敏感问题受 `CR-GATE-003` 控制；领域真实数据受 `SA-GATE-006` 控制 | Knowledge/业务出域策略计算、领域 Prompt 和检索模型 |
...
下位交付以三份 L2 为最小集合，不再拆分独立注册、错误、观测或部署文档。建议先稳定 `L2_00_01` 核心执行与能力注册语义，再并行细化 `L2_00_00` Spring 协同和 `L2_00_02` DeepSeek 接入；这不改变三份 L1 的文档顺序。
...
| `SA-GATE-002` | `design_decomposition` / `slice_implementation` | DeepSeek 接入 | 定版 `L2_00_02` 并进入真实模型实现 | Spring/LangGraph 内部契约、DeepSeek 契约和受控结构化动作 PoC 完成 | 项目维护者、模型提供方 | `L2_00_02` 定版前 | 允许模型无关 L2 和测试替身；禁止真实模型实现 | `L2_00_02` |
...
| `CR-GATE-003` | `integration` | 用户问题进入 DeepSeek | 向外部模型发送可能含敏感信息的原始或改写问题 | `L2_00_02` 固化问题最小化、允许/拒绝边界和零调用测试；未分类时失败关闭 | 项目维护者、模型提供方 | 首次敏感问题联调前 | 只允许非敏感测试问题或本地替身；禁止默认外发敏感文本 | `L2_00_02` |
...
|---|---|---|---|---|
| Spring→Python 采用何种内部传输协议及契约源 | 跨语言兼容、取消和本地调试 | `L2_00_00` / 项目维护者 | `CR-GATE-001` 后、该 L2 定版前 | 不阻塞本文；阻塞该 L2 定版 |
| 运行时请求接受、断连和重复提交的精确语义 | 防止不确定状态下重放 | `L2_00_00` | `CR-GATE-002` 前 | 不阻塞本文 |
...
| Agent 对外响应与 L0 状态的协议映射 | 调用方兼容性 | `L2_00_00` | 外部契约定版前 | 不阻塞本文 |
...
- 任何代码、配置或真实模型链路已经实现。
- `L1_01` 已完成 L1 正式评审，但 `L1_02` 尚未创建，且任何关联实施、真实模型/数据集成或效果门禁均未因此关闭。
