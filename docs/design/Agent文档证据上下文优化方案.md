...
| 是否可作为实现依据 | 否；完成本次目标态复审并与代码对齐后再转为 Approved |
...
| 序号 | 类型 | 内容 | 影响 | 建议处理 | 是否阻塞 |
|---:|---|---|---|---|---:|
| 1 | 代码对齐 | 当前代码仍可能包含旧配置 setter 和旧索引 fallback | 与目标态不一致 | 在实施阶段按第 13 节删除并补测 | 是，阻断转 Approved |
| 2 | 契约 | 当前 `DocumentGenerationResult` 是否完整携带 citation bindings 和 operation metadata 需代码对齐确认 | 可能无法证明生成文本可信 | 补充 Java 契约与测试后复审 | 是，阻断转 Approved |
| 3 | 数据 | 现有索引是否保存派生字段 provenance 需 dry-run 确认 | citation span 可能不准确 | 不满足时不得切换索引别名 | 是，阻断索引切换 |
| 4 | Multi-Agent | 未来 ResultRef 如何承载 Document typed result 尚未设计 | 不影响当前单 Agent | D06 Multi-Agent L1 定义，不在本文提前实现 | 否 |

## 16. 评审记录

| 轮次 | 日期 | 评审结论 | 发现问题数 | 修正问题数 | 遗留问题 | 说明 |
|---:|---|---|---:|---:|---|---|
| 1 | 2026-07-13 | 有条件通过 | 6 | 6 | 3 个实施对齐阻塞项 | 已完成文档内部修订；正式转 Approved 前需按第 15 节验证代码与索引 |

## 17. 实施对齐检查

| 检查项 | 设计要求 | 实现位置 | 当前结论 | 说明 |
|---|---|---|---|---|
| 预算唯一性 | 三阶段使用同一有效预算证据 | Validator、Handler、ResultSecurityProjector | 待核验 | 不以本文推断代码已满足 |
| 生成候选安全 | 类型化 candidate + citation/evidence 校验 | `DocumentGenerationPort`、`DocumentResultSecurityProjector` | 待核验 | 必须补充失败路径测试 |
| ACL 窗口 | 同文档、同 ACL、同 tenant 扩展 | retrieval mapper、context packer | 待核验 | 跨边界测试是门禁 |
| 兼容删除 | 不保留旧参数和旧索引 fallback | `AgentProperties`、application.yml、mapper | 待核验 | D03 目标态删除 |
| Multi-Agent 范围 | 不创建 TASK/ResultRef 空壳 | 本文全部实现落点 | 满足设计 | 当前仅单 Agent |
