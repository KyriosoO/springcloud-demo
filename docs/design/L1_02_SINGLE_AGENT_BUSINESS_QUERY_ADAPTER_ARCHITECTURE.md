...
> 本文是 P2 第三份 L1 架构基线。v0.2 已完成五轮独立评审—修订—复核，`REV-BQ-001`～`REV-BQ-006` 均已关闭且无未关闭 S0/S1；项目维护者已授权按评审结论原子同步，`BQ-GATE-001` 已关闭。该结论只允许开始本文治理的三份 L2，不表示最终有限动作清单已经确认、业务接口或共享安全契约需要或已经获准变更、L2 已定版、代码实施已开始、角色授权闭环已完成或真实业务数据已经允许进入外部模型。
...
| `BQ-FACT-007` | L0 已确认 Employee/Transaction 首批共同允许语义角色为 `admin`、`viewer`，并确认 `dylan` 通过 `admin` 访问；当前仓库 `auth-users.yml` 已将 `dylan` 配置为 `ADMIN`、`viewer_t` 配置为 `VIEWER`，用户到语义角色的分配与目标一致，但配置值仍为大写且尚无统一 Authority 映射及有效运行配置证据 | 不再设计 `dylan` 用户角色迁移；仍须核实大小写/前缀/集合映射、有效配置来源和端到端授权，不得在 Adapter 增加用户特例 |
...
| 角色声明与 Authority | JWT 有 `role` 集合，但缺少统一映射，当前配置值仍需按统一规范解释 | `common-security` 提供单一规范映射，缺失/错误失败关闭；业务查询只消费 | 消费语义、有效配置和端到端矩阵待固化；提供方改造须由其权威另行承接 | `L2_02_00`；`SA-GATE-004/005` |
| 业务最终授权 | 当前主要只校验用户令牌类型，部分候选入口无专用守卫，宽响应缺少动作级可见性契约 | 每个实际映射动作由目标业务服务执行本域角色授权并保证成功响应的数据范围 | 方法/路径授权、响应可见性及允许/拒绝测试待补齐 | 域 L2、P3/P4；`SA-GATE-004/005` |
...
- 若核实后必须修改 `auth-service` 或 `common-security` 的实现/公共契约，应先形成精确差距、调用方影响和回滚范围，并取得对应文件和动作的单独授权；在提供方证据完成前 `SA-GATE-004/005` 保持开启。
...
- 若候选接口无法证明其成功响应已经满足当前用户的数据可见性契约，该接口不得成为真实启用动作；可以继续使用替身完成隔离设计，但对应 `SA-GATE-004/005` 保持开启。
...
|---|---|---|---|---|
| `L2_02_00` 业务查询公共约束、配置与出域详细设计 | 业务动作公共输入/结果/出域结构、最小有效用户结果语义、强类型配置公共约束、字段交集、代码实现的有限转换枚举及配置选择规则、结构化模型安全载荷与事实绑定语义、JWT 透传客户端原则、业务查询对统一 `role` → Authority 外部契约的消费语义/差距/联调前提、业务敏感输入类别、公共失败矩阵、组合根与公共测试替身 | 本文、L1_00 能力契约、`auth-service`/`common-security` 现状与外部权威 | DTO/配置/枚举、公共校验、角色映射消费契约与差距记录、必需结果与状态保持测试、业务文本数据隔离及事实追踪场景、向 `L2_00_02` 提供的业务输入/模型输出负向场景、模型零调用与模拟第三域测试 | 全局问题输入/模型 Provider 策略、Employee/Transaction 精确动作/字段/端点/业务规则，以及 `auth-service`/`common-security` 私有实现和未经授权的公共契约变更 |
| `L2_02_01` Employee Adapter 与业务授权联调详细设计 | Employee 最终动作目录、现有接口映射、参数/结果字段及最小有效用户结果、敏感字段分类、出域配置、协议客户端、错误映射、Employee 方法授权、响应数据可见性契约和联调矩阵 | 本文、`L2_02_00`、`employee-service` 当前契约 | 动作 ID、DTO/配置、接口映射、契约测试、`admin/viewer` 允许及拒绝场景、成功响应数据范围、必需字段缺失失败、Employee 指令性字段/无依据事实负向场景、禁止接口零调用 | Transaction、Knowledge、Employee 业务规则重写、以 Adapter 投影替代业务授权和未授权新接口 |
| `L2_02_02` Transaction Adapter 与业务授权联调详细设计 | Transaction 最终动作目录、现有接口映射、参数/结果字段及最小有效用户结果、出域配置、协议客户端、错误映射、Transaction 方法授权、响应数据可见性契约和联调矩阵 | 本文、`L2_02_00`、`mq-procedure-service`/`transaction-api` 当前契约 | 动作 ID、DTO/配置、接口映射、契约测试、`admin/viewer` 允许及拒绝场景、成功响应数据范围、必需字段缺失失败、Transaction 指令性字段/无依据事实负向场景、聚合/写入零调用 | Employee、Knowledge、Transaction 业务规则重写、以 Adapter 投影替代业务授权和未授权新接口 |
...
| 阶段 | 目标 | 允许产出 | 禁止越过 |
|---|---|---|---|
| P2-L1 | 固定业务查询 L1 边界 | v0.2 已完成五轮正式评审并与相关架构文档原子同步 | 不声明动作、接口或权限已生效 |
...
| `SA-GATE-004` | `integration`（继承 L0） | Employee 真实查询 | 启用真实 Employee 动作 | 最终只读动作映射确认；`admin/viewer` 允许、其他角色拒绝；业务服务成功响应的数据范围符合本域可见性契约；统一 Authority、参数/字段收紧、契约和异常测试通过 | `auth-service`、`common-security`、`employee-service`、项目维护者 | Employee P4 联调前 | 允许 Adapter/替身设计与实现；禁止真实 Employee 动作启用 | `L2_02_00/01` |
| `SA-GATE-005` | `integration`（继承 L0） | Transaction 真实查询 | 启用真实 Transaction 动作 | 最终只读动作映射确认；`admin/viewer` 允许、其他角色拒绝；业务服务成功响应的数据范围符合本域可见性契约；聚合/写入不可达；统一 Authority、参数/字段收紧和契约测试通过 | `auth-service`、`common-security`、`mq-procedure-service`、项目维护者 | Transaction P4 联调前 | 允许 Adapter/替身设计与实现；禁止真实 Transaction 动作启用 | `L2_02_00/02` |
| `SA-GATE-006` | `integration`（继承 L0） | 真实业务结果进入 DeepSeek | 将 Employee/Transaction 真实结果发送外部模型 | 字段默认拒绝、结果/模型交集、代码有限转换、结构化数据/指令隔离、回答事实绑定、未分类/冲突失败关闭和模型零调用测试通过 | 项目维护者、对应业务域、模型提供方 | 对应域首次真实数据模型联调前 | 只允许非敏感测试数据或本地替身；禁止真实未获准业务载荷外发 | `L2_02_00/01/02`、`L2_00_02` |

`BQ-GATE-001` 已关闭且只控制 L1→L2；`BQ-GATE-002` 按三个 L2 切片分别判定；`CR-GATE-003` 控制敏感用户问题外发；`SA-GATE-004/005` 分别控制两个真实业务域；`SA-GATE-006` 只控制业务结果外发。任一门禁不能替代另一个，也不应阻塞无直接依赖的文档或测试替身工作。
...
| 统一角色映射与有效用户配置未闭环 | `role` claim 到 Authority 的大小写/前缀规则及有效环境配置尚未形成已验证外部契约 | 业务服务可能无法正确允许 `admin/viewer` | `L2_02_00` 只固化消费语义、差距和联调矩阵；提供方设计/修改由 `auth-service`、`common-security` 权威另行承接和授权；未完成失败关闭 | 两域真实授权 |
| 业务服务最终授权与响应可见性未具备 | 当前主要只校验 `token_type=user`，部分接口无专用守卫，候选宽响应也未形成动作级数据可见性契约 | 任意已认证用户可能访问本应受限数据，或 Adapter 被误用为授权补丁 | `SA-GATE-004/005`；域 L2 定义方法授权、响应数据范围和允许/拒绝矩阵 | 两域真实集成 |
...
以下记录对应本次已授权的五轮独立评审—修订—复核；它只证明本文的 L1→L2 分解已通过，不替代 L2 评审、实现验证、外部契约变更授权、真实集成或模型出域证据。
