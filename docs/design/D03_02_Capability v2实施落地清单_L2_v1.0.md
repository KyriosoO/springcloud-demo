...
3. 把当前已产生的 D03 本地 diff 标记为已实现、待验证或待修正。
4. 定义继续编码前必须先修正的签名、Bean、事务和测试问题。
5. 给出后续评审、修复、复审清单，避免继续临场补设计。
...
Maven 命令均在 `D:\codex\serviceCenter` 执行；Python、`rg`、`git` 命令均在 `D:\codex` 执行。第 9.1 到 9.4 是目标门禁命令，若其中测试类当前尚不存在，必须先按第 8 节补齐测试，再执行对应命令，不能把 `surefire.failIfNoSpecifiedTests=false` 当作豁免。
