...
| [`L0_00`](../design/L0_00_SINGLE_AGENT_ARCHITECTURE.md) | v2.4 | 系统边界和下位治理 | Approved |
| [`L1_00`](../design/L1_00_SINGLE_AGENT_CORE_RUNTIME_ARCHITECTURE.md) | v2.9 | Runtime/Model/Core、Knowledge 可选接线、组合根及完整意图边界 | Approved |
...
| [`L1_01`](../design/L1_01_SINGLE_AGENT_KNOWLEDGE_QUERY_ARCHITECTURE.md) | v1.7 | Knowledge 能力、candidate-05 根因、Summary V4 与效果口径 v2 | Approved |
| [`L2_01_00`](../design/L2_01_00_SINGLE_AGENT_KNOWLEDGE_QUERY_FLOW_CONFIGURATION_DETAILED_DESIGN.md) | v1.8 | 单动作、域目录 v2、默认关闭接线、当前任务/Provider 装配 | Approved |
| [`L2_01_01`](../design/L2_01_01_SINGLE_AGENT_KNOWLEDGE_RETRIEVAL_LOCAL_MODEL_DETAILED_DESIGN.md) | v1.7 | typed retrieval、读取授权、RRF/rerank 和 client 生命周期 | Approved |
| [`L2_01_02`](../design/L2_01_02_SINGLE_AGENT_KNOWLEDGE_EVIDENCE_EGRESS_SUMMARY_EFFECTIVENESS_DETAILED_DESIGN.md) | v1.8 | Evidence/出域、Summary V4、效果口径 v2、诊断和候选 | Approved |
| [`UAT_00`](UAT_00_SINGLE_AGENT_ACCEPTANCE_TEST_PLAN.md) | v1.18 | Business 35/35 追踪、当前测试计数与正式 Python 入口 | Reviewed |
| [`UAT_01`](UAT_01_SINGLE_AGENT_KNOWLEDGE_ACCEPTANCE_TEST_PLAN.md) | v1.7 | Knowledge 功能与效果验收 | Reviewed |
...
| `WP-K-EFFECT-OPT-06` | Knowledge 最小效果优化 | `L1_01 v1.7`、`L2_01_00/02 v1.8`、`UAT_01 v1.7` | 实施 Summary V4 与效果口径 v2；不调整 retrieval/validator/dataset/gold/权限 | `WP-K-EFFECT-DIAG-06` | - | 新任务、evaluator 语义、组合根和测试 | Knowledge 全回归、E2E、strict mypy、历史 hash | 禁止放宽阈值/安全；V1～V3 不修改 | Done |
| `WP-K-EFFECT-CANDIDATE-06-PREP` | 新效果候选非 live 冻结 | 新优化版本与 candidate-04/05 历史 | 新 run/manifest/hash/reference/预算/快照/失败关闭 | `WP-K-EFFECT-OPT-06` | - | candidate-06 准备资产和一次性授权模板 | fake budget、首 outbound、retry/resume=0、历史 hash | 不读取密钥或产生 outbound | Ready |
...
| `DEP-KQ-001` | `WP-K-BASELINE-03` | `WP-K-RUNTIME-WIRING-03` | contract | 生产接线只能依据评审通过的默认关闭与生命周期合同 | `L1_00` v2.9；`L2_01_00` v1.8；`L2_01_01` v1.7 |
...
| 30 | `WP-K-EFFECT-CANDIDATE-06-PREP` | Ready | `WP-K-EFFECT-OPT-06` | 只准备非 live 冻结资产，不读取密钥或 outbound |
...
| `WP-K-EFFECT-CANDIDATE-06-PREP` | 更新后的 `DR-KEV-015` | candidate-06 non-live assets | preparation/history/budget | `GATE-076` | Ready |
...
总计 32 个工作包：既有 Business 16 个、Knowledge 9 个均为 Done；本轮新增 7 个收口包中，文档纠偏、Python 正式入口、candidate-05 诊断和最小优化已 Done，candidate-06 非 live 准备 Ready，live 与最终收口按依赖 Blocked。`GATE-071～075` 与 `GATE-UAT-008` 已关闭；`GATE-076～078` 保持 Open。candidate-05 结论保持 `partially_effective`，Business v3/v4/run03 与 Knowledge candidate-01～05 历史均保持不可变。

## 15. 后续实施建议

Employee/Transaction 保持已完成。文档纠偏与评审、Python 正式入口、candidate-05 只读诊断和设计评审后的最小优化均已完成；当前只执行 candidate-06 非 live 冻结。完成后必须暂停并输出 `GATE-077` 精确授权模板；不得重用 `GATE-072`、读取密钥或产生真实 outbound。
