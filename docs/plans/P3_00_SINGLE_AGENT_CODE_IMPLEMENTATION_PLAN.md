...
| `WP-BQ-CONTROLLED-LIVE-02` | 受控模型与业务联调 | `REQ_00`; `L2_00_02`; 两域 L2 | 有限固定场景、敏感值内存化、四次失败历史不可变 | `WP-BQ-NONLIVE-E2E-02`, `WP-EMP-DETAIL-RETIRE-02`, `WP-EMP-ES-AUTH-02` | `GATE-070` | 新的真实三动作 finite evidence，不覆盖既有失败证据 | 一计划/一业务调用与真实权限矩阵 | 失败即停止，先修复根因，不复用失败结果路径 | Blocked |
...
| 11 | `WP-BQ-CONTROLLED-LIVE-02` | Blocked | `GATE-070` | controlled-run04 已失败；真实向量返回 10/20 hits 且 1 条缺姓名，先完成 partial coverage/记录卫生 non-live 修复和四项历史保护 |
...
| `WP-BQ-CONTROLLED-LIVE-02` | `DR-MODEL-104`; `DR-BQCOM-107/108`; `DR-EMP-105/107/108`; `DR-TXN-105` | action 独立超时、partial page/记录卫生、受控 runner、四次不可变失败及新的独立有限结果 | 有限三动作 live 矩阵 | `GATE-070` 重新关闭证据 | Blocked |
...
总计 12 个工作包、15 条直接依赖：Done 10 个，Ready 0 个，Blocked 2 个。filters 合同、统一三动作配置、v3 Model 逻辑目录、Employee search/semantic Adapter、端点级共享 JWT converter/真实 Servlet 安全链、Transaction 四字段/Date/Decimal/分页 Adapter、唯一三动作生产组合根、旧 detail 目标入口退役核实与新版完整 non-live E2E 已完成。真实上海员工查询已返回 20 条；semantic 独立 10000ms 合同已落地，但零模型诊断证明现有向量接口 `total=20`、实际 hits=10、其中 1 条缺失姓名，当前 Adapter 对 partial page 和历史脏记录过严。403、Feign 服务发现、旧 semantic timeout 和真实响应合同四项失败分别冻结 SHA-256=`fdc37b16e45d58733ede0a468e90b4db5242de8c84bcda7cca18ef07bd368607`、`121814993c53c2f0b4910bb5efe8b35bfe3da65dc395bd3270aa1c57b6eb5a08`、`737d76c296d7803618f74c370a4478b73e2a65a3bbec66ffee3d2d577b4a467d`、`3582693a77b4b791eabdc7253778936ac76ae7a779c09fad1edb3057bc7c14de`；旧 manifest SHA-256=`974228e060383324255a393d3f1107506510b515d0577b44c2671ea24d3a7d90` 不变，新 manifest SHA-256=`3da2d9f250253b142e43f690d5dc4e7ff8cf9bfe57f2e52ff6d248ec2c8d75d2`。`GATE-067/068/069` Closed；`GATE-070/GATE-UAT-007` Open；先完成独立 non-live 修复，不新增门禁。

## 15. 后续实施建议

先实施 `DR-BQCOM-108/DR-EMP-108` 的 bounded partial hits 与记录级卫生，并通过全量 non-live、四项历史哈希和计划 DAG 复核后重新关闭 `GATE-070`。随后使用已冻结配置 snapshot/manifest 和独立 controlled-run05 执行六场景真实联调；真实三动作及权限矩阵通过后，按证据开放正式 UAT。若现有接口无法安全表达目标，应停止对应扩展并报告，不得自行新建接口或扩大权限。
