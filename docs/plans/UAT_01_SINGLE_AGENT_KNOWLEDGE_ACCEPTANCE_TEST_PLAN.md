...
### 14.55 有限失败观察的non-live兼容证据

P3 §20.86核实既有§14.34～14.35观察器未接入代表集v1/v2 Model失败路径。新增12项当前Rewrite9/生产根fake兼容测试，验证有限phase/code/cause、公开结果与观测不变、调用次数不变、失败零检索及取消清理；HTTP响应头等旧白名单未覆盖的原因仍为unknown。复用已有工具，不新增诊断接口或付费运行入口，不改变任何任务、Prompt、validator、索引、历史执行器或原件。

本轮没有真实调用，§14.54.1五通过/一失败/三未执行和累计31 E2E/80模型保持。测试只能证明未来独立运行可复用的诊断能力，不能事后重建017输出或补写原因，更不能代替该题通过证据。Business35项与Knowledge37项追踪已在本轮相关回归复核；阶段B专项及人工usefulness仍未完成。后续接线必须在独立运行前验证，失败批次不重启、不补跑、不复用剩余授权；本节本身不授权新模型请求。

### 14.56 剩余噪声与真实验收缺口分离

P3 §20.87对§14.53.1已有142条Evidence进行零网络分解：49条grade0均为需求锚点之后的可选项，涉及17题；21条要求不符、19条主体不符、9条其他文件，13条已经位于最终候选前三名。它们并非强制锚点保留；分数准入与相关性不是同一判据。原记录未保存具体分数，不能据此确定新阈值或重放完整selector，不修改旧分级/结果、当前0.5准入或索引。

沿用§14.47：残余噪声作为明确质量风险，不以“必须清零”增加无限诊断前置，也不豁免必要原文、语义和实际引用验收。本次只有旧记录算术和相应反证，没有真实模型/检索请求或新usefulness结果；§14.54.1的五通过、一失败、三未执行保持。017失败发生在检索前，不能归因于这些离线噪声。既有35/37功能追踪及专项未完成状态不变。

### 14.57 四项独立验收与有限失败观察

2026-09-11用户明确批准：准备、冻结并执行独立4项批次，最多12次模型请求，累计上限35次E2E/92次模型；失败即停，禁止重试、续跑和自动追加批次。此授权仅用于`knowledge-representative-uat-v3-20260911-03`，reference=`UAT_01:14.57`；不恢复§14.54已消费批次，也不沿用其剩余预算。执行前已知累计31 E2E/80模型，严格冻结当前提交、manifest、代码/制品、dataset、Prompt、配置、索引和本地模型身份后执行。

顺序固定KRB-017、019、021、023，问题、域、必要来源及原文锚点逐字沿用§14.51，分别为vehicle_price:1、environment:2、stamp_price:1、lost_invoice:1；均为既有holdout，不称新盲评。已通过的5题不重复。任务仍为selection-v4/Rewrite9/Summary7，未修改Prompt、生产排序、候选窗口、Evidence准入、索引、gold或通过条件。

全批上限4 E2E、12模型、16 search、8 embedding、16 rerank，另1次本地启动预热；单题仍为3/4/2/4。Business、旧answer、索引写入、retry/resume均0。复用已有真实Spring→当前Runtime入口及固定Knowledge接口、内存JWT、预算前置检查、独占started/consumed/journal、任一失败停止和核实PID/原始日志清理。绑定或依赖未通过时不发送模型请求。

V3仅修改批准的批次元数据并接入§14.55已验证的现有有限观察器；每题增加`modelFailure={records:[{phase,code,cause}],overflowed}`。只投影观察器已有严格枚举，未知保持unknown，不读异常消息、局部变量或原始模型响应；最多8项，溢出停止，不改变公开响应、任务调用、validator或判据。记录按当前case边界隔离，异常/取消也保留已有有限诊断；观察器退出后恢复原回调。旧执行器、测试及全部历史原件字节不变。
...
依据L2_01_00 §8.12和P3 §20.94，已新增Rewrite11替换queries数组表示；设计三轮内审与两轮分离复评通过，当前源码11/7/v3及直接fake/Spring验证已落实，未部署或产生新版真实测量。固定启用域槽位不改变现有case、预期域、需求、原文anchor、gold、阈值或人工rubric，也不证明run-07真实失败一定是重复域。§14.62.1的10/7/v3历史终态继续有效，不作为11/7真实验证。
