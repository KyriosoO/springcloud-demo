...
当前结论：真实来源同池必要覆盖、原文哈希、策略及字节验证完成；完整相关性分级、生产接线生效和真实后置拒绝仍未完成。默认legacy、文号配置默认关闭及所有历史运行终态不变。沿用§14.44生效判据，不创建新候选或改变gold/阈值；本条不是新的付费运行授权。测试与评审计数见P3 §20.69。

#### 14.44.3 相关性核对进度（12/24题）

沿§14.40/DR-KEV-033，以旧/新top20并集逐题核对。当前记录`retrieval_relevance.review.v1.jsonl`包含001～012的243项，全部483项中仍有240项未核对；不是独立盲评或外部专家批准。旧问题、gold和原始结果不变，既有结果中的null不被覆盖；额外分级复算由`retrieval_relevance_review.py`提供，整个24题的Precision/nDCG仍不汇总。三批分别追加81、82、80项，以首6/10/15行哈希保护既有判断，具体来源及验证见P3 §20.70。
...
分级1属于帮助解释范围、导航或程序的背景，不是直接作答依据；Precision按已定义grade>0包含这些项。004的候选2条含一条指定文件的直接定义和一条不同历史文件的平行定义，不能把后者提升为等价来源。高nDCG只表示已标注相关项在这个有限池中的排序，不证明无关尾部消失或Summary正确。001/004尾部减少不抵消002/003的无关Evidence；候选策略及阶段B不能据此通过。后续依原编号核对剩余题，保留留出分组、不调阈值或追加付费调用。运行核对、测试及当前实施状态见P3 §20.70。
